...
For PLAN, name the observable result, independent acceptance, relevant regression and failure checks, and concrete maintainability risks in that existing session record. Implement and verify those criteria. At DECIDE, report the accepted outcome before efficiency; for measured work, use [outcome measures](outcome-measures.md), retaining failed attempts, unknown usage and zero-completion cases. Resource targets never replace the required review.
...
Light mode changes planning depth, not required review. An eligible direct-light task may be implemented in the active context and still requires the independent review defined above; self-review is not independent verification. The presence of AFP worker seats alone does not force delegation.

When the selected path delegates implementation, BUILD implements and repairs, and fresh TEST independently verifies the candidate without BUILD's conversation. Keep the intended behavior and verification plan readable from the assigned authority. Do not create a standalone direct-light state machine or invent a worker result for direct coordinator work.
