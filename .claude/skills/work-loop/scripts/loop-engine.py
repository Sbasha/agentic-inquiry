...
def cmd_transition(args: argparse.Namespace) -> int:
    ...
    expect_run_id = getattr(args, "expect_run_id", None)
    expect_sequence = getattr(args, "expect_sequence", None)
    if (expect_run_id is None) != (expect_sequence is None):
        return stop("transition requires --expect-run-id and --expect-sequence together")
    if expect_sequence is not None:
        if expect_sequence < 0:
            return stop("transition --expect-sequence must be non-negative")
        sequence = state.get("transition_sequence")
        if type(sequence) is not int or run_id != expect_run_id or sequence != expect_sequence:
            return stop("transition run or sequence changed; inspect current workflow state")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument("--expect-run-id", default=None)
    sp.add_argument("--expect-sequence", type=int, default=None)
