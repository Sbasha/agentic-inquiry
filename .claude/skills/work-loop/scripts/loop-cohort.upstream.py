...
def _review_operation_gate(
    ...
    *,
    expect_run_id: str,
    spec_name: str,
    expect_round_count: int | None = None,
) -> tuple[str, int | None]:
    ...
    """
    if expect_round_count is not None and (expect_round_count < 0 or operation_id is None):
        return "refuse", stop(
            "review record: --expect-review-round-count must be non-negative "
            "and requires --operation-id"
        )
    ...
    if expect_round_count is not None:
        rounds = non_negative_int(state, "review_round_count", 0)
        if isinstance(rounds, str):
            return "refuse", stop(f"review record: {rounds} for {spec_name}")
        if rounds != expect_round_count:
            return "refuse", stop(
                "review record: review round count changed; inspect current workflow state"
            )
    return "record", None
...
def cmd_review_record(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "all_skipped", False):
        ...
        outcome, code = _review_operation_gate(
            ...
            expect_round_count=getattr(args, "expect_review_round_count", None),
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument("--expect-run-id", required=True, dest="expect_run_id")
    sp.add_argument(
        "--expect-review-round-count",
        type=int,
        default=None,
        help="require this count before a new round; matching recorded operations remain no-ops",
    )
