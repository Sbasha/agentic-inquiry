...
When an approved delivery breakdown already exists and the request is only to prepare or publish tracker tickets, use `to-tickets` when installed. This skill owns shaping child intents and delivery contracts; it does not need to repeat that work for a tracker projection.
