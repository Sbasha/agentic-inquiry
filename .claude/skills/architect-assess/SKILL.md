...
Use `graphify` only when optional graph traversal materially helps a scoped relationship or blast-radius question and generation is authorized. Verify graph edges against source; an extracted graph does not replace this assessment's current-state model or evidence coverage.
