...
description: Create, revise, or review editable Excalidraw diagrams with clear hierarchy, spacing, labels, connectors, grouping, and visual flow. Use when the user explicitly requests Excalidraw output or an existing `.excalidraw` artifact; report when the required Excalidraw integration is unavailable instead of silently substituting another format.
---

# Creating Excalidraw Diagrams

Use the local file workflow when an editable file is sufficient; an unavailable inline integration does not prevent local file creation. Resolve `scripts/generate_excalidraw.py` from the absolute directory of this installed skill. Choose a new caller-owned output path, verify it does not already exist or traverse an unrelated symlink, and preserve existing artifacts. The helper currently writes directly to its target; do not use it for replacement until that specific replacement is authorized and preservation is established.


When creating Excalidraw diagrams, ensure you:

- Generate the layout with correct spacing
- Make consistent and logical use of colors, fonts, and shapes
...
- Call `read_me` before your first `create_view` call each session - it provides the color palette, camera sizes, layout rules, and element format reference.
- If inline preview is requested and no Excalidraw integration is available, report the missing preview capability. The local file workflow remains available; installing an integration is a separate action.
...
|----------|---------|---------|
| `labeled_rect(id, x, y, w, h, bg, stroke, label, ...)` | `[rect, text]` | Shape with centered bound text (handles all binding and field generation) |
...
skill_dir = Path("<absolute-installed-skill-directory>").resolve()
