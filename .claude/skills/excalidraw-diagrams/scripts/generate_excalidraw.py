...
def txt(
    ...
    Args:
        id: Unique element ID.
        x, y: Position (left edge for standalone, approximate center for bound).
...
def labeled_rect(
    ...
    """Create a rectangle with centered bound text. Returns [rect, text].
...
def write_scene(elements: list[dict[str, Any]], path: str) -> None:
    ...
    # Preserve trailing-separator behavior for caller-supplied paths.
    pathlib.Path(os.path.dirname(path) or ".").mkdir(exist_ok=True, parents=True)  # noqa: PTH120
...
if __name__ == "__main__":
    ...
    # Two labeled boxes connected by a bound arrow (moves with shapes)
