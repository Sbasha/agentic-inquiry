...
description: "Create, update or syntax-check Mermaid diagrams of systems, integrations, flows, states, data models, deployment topologies and plans. Use for a diagram as the deliverable. Use excalidraw-diagrams for editable Excalidraw, graphify for source graph analysis, architect-design for a design decision and architect-review for substantive architecture critique."
metadata:
...
This skill owns Mermaid creation, update and syntax review. Use `excalidraw-diagrams` for an explicitly requested editable Excalidraw canvas; use `graphify` for an optional source-bound graph analysis. A Graphify result is evidence to inspect, not a diagram specification.

Read [Mermaid syntax and label guidance](references/mermaid-diagrams/reference.md) when preparing Mermaid text. Preserve the project's visual conventions instead of applying the reference palette automatically. Render locally with an available renderer where verification matters. Sending private diagram or project content to an external renderer requires authorization for that upload.
