...
description: Apply defensive, portable shell-scripting practices with simple control flow, safe quoting, explicit error handling, predictable cleanup, and ShellCheck-aware structure. Use when creating, reviewing, debugging, or hardening Bash or POSIX shell scripts that operate on real files, processes, remote commands, installers, or automation workflows.
---

# Bash Scripting Best Practices

Guidance for writing reliable, maintainable bash scripts following modern best practices. Emphasizes simplicity, automated tooling, and defensive programming without over-engineering.
...
setup() {
    # Dependency checks, variable initialization
    command -v jq >/dev/null 2>&1 || die "jq required"
}
