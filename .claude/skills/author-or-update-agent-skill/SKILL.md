...
Repository instruction files such as `CLAUDE.md` and Claude scoped rules belong to `authoring-claude-md` when installed. A one-off supplied prompt is not a reusable skill; use the available prompt-revision workflow for it.
