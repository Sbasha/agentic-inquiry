...
`.afp-bundle-install-marker.toml` at the install's scope root; the
...
scopes' `.afp-bundle-install-marker.toml` files are absent, the skill
...
1. **State files.** Read `<repo>/.afp-bundle-state.toml` (if
   present) and `~/.afp-bundle/state.toml` (if present). These
   ...
   `afp bundle init-state --migrate` as the prereq for
...
3. **Install markers.** Read `<repo>/.afp-bundle-install-marker.toml` and
   `~/.afp-bundle/.afp-bundle-install-marker.toml` if present. Prepend
   each entry to the session-internal proposal queue. After consuming
   each scope's entries, delete that scope's marker file.

4. **Discovery files.** Read `<repo>/.afp-bundle-discovery.toml` and
   `~/.afp-bundle/.afp-bundle-discovery.toml` if present. The repo-scope
...
6. **Proactive cache scan.** Scan
   ...
   *active case* - an adopter who proactively
   invokes the adapt-to-project skill in session 1 before the
...
`[markers]` in the repo-scope `<repo>/.afp-bundle-discovery.toml`; never
...
```
afp bundle adapt --values-from <repo>/.afp-bundle-discovery.toml
```
...
**Doctrinal self-check.** After writing `<repo>/.afp-bundle-discovery.toml`,
...
Regenerate `.afp-bundle-pending.md` at each scope where deferred work
...
- **Never write outside the adopter's per-scope jail.** Repo-scope
  ...
  (`.claude/`, `.afp-bundle/` for the Claude Code adapter).
  *Enforcement boundary:* class-1 substitution shells out to
  `afp bundle adapt`, where `safety.write_jailed` enforces the
...
- **Never write `[markers]` to the user-scope `.afp-bundle-discovery.toml`.**
  Markers are repo-only; the typed loader refuses
  `[markers]` at user scope.
- **Never write `.afp-bundle-discovery.toml` in any shape other than the
...
- **Never shell out to anything other than `afp bundle adapt`** for
