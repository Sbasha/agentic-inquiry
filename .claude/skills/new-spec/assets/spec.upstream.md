...
The body describes the current contract; decision history lives in the
repository's decision records and revision history in Git. The plan follows the
same rule: while it is `Drafting` it is rewritten in place, and after approval
execution observations go to the verification ledger. -->
