...
class TestStrategyExecutionOrder:
    ...
    async def test_strategy_execution_order_property(self, resolver, temp_project):
        ...
        async def mock_database(
            target_name, target_type, source_file, source_language, import_path
        ):
        ...
        async def mock_import_path(
            target_name, target_type, source_file, source_language, import_path
        ):
            call_order.append("import_path")
            return None  # Fail to continue to next strategy

        async def mock_exact_match(
            target_name, target_type, source_file, source_language, import_path
        ):
            call_order.append("exact_match")
            return None  # Fail to continue to next strategy

        async def mock_module_path(
            target_name, target_type, source_file, source_language, import_path
        ):
            call_order.append("module_path")
            return None  # Fail to continue to next strategy

        async def mock_proximity(
            target_name, target_type, source_file, source_language, import_path
        ):
    ...
    @pytest.mark.asyncio
    async def test_strategy_order_with_different_success_points(
        self, resolver, temp_project
    ):
        ...
        async def mock_database_success(
            target_name, target_type, source_file, source_language, import_path
        ):
...
class TestStrategyStatistics:
    ...
    async def test_strategy_statistics_property(self, resolver, temp_project):
        ...
        async def mock_import_success(
            target_name, target_type, source_file, source_language, import_path
        ):
            return ("import_file.py", "class", 1.0)

        async def mock_proximity_success(
            target_name, target_type, source_file, source_language, import_path
        ):
    ...
    async def test_each_resolution_increments_one_counter(self, resolver, temp_project):
        ...
        assert len(incremented_strategies) == 1, (
            f"Expected exactly 1 strategy incremented, got {incremented_strategies}"
        )
        ...
        for strategy in [
            ...
            assert differences[strategy] == 0, (
                f"Strategy {strategy} should not have been incremented"
            )
