...
async def test_context_manager_propagates_exceptions(
    mock_db_manager, tmp_path, mock_watcher
):
...
async def test_aexit_returns_false(mock_db_manager, tmp_path, mock_watcher):
    ...
    result = await pipeline.__aexit__(
        exc_type=ValueError, exc_val=ValueError("test"), exc_tb=None
    )
...
@pytest.mark.asyncio
async def test_aexit_returns_false_on_normal_exit(
    mock_db_manager, tmp_path, mock_watcher
):
    ...
    result = await pipeline.__aexit__(exc_type=None, exc_val=None, exc_tb=None)
