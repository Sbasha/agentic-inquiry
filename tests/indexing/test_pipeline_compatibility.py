...
class TestParsedDocumentCompatibility:
    ...
    def test_parsed_document_required_fields(self):
        ...
        doc = ParsedDocument(
            doc_id="test_doc_123", file_path="/path/to/file.py", chunks=[]
        )
...
class TestParserChunkToDocumentChunkMapping:
    ...
    async def test_basic_chunk_mapping(self, pipeline):
        ...
        doc = ParsedDocument(doc_id="test_doc", file_path="test.py", chunks=[chunk])
        ...
        chunks = await pipeline.db_manager.advanced_filter(
            "document_chunks", filters={"doc_id": "test_doc"}
        )
    ...
    async def test_code_symbols_mapping(self, pipeline):
        ...
        entities = await pipeline.db_manager.advanced_filter(
            "graph_entities", filters={"doc_id": "test_doc"}
        )
        ...
        code_entity_names = {
            e["name"] for e in entities if e.get("type") not in ("file", "code_module")
        }
    ...
    async def test_relationships_mapping(self, pipeline):
        ...
        doc = ParsedDocument(doc_id="test_doc", file_path="test.py", chunks=[chunk1])
        ...
        relationships = await pipeline.db_manager.advanced_filter("graph_relationships")
        ...
        assert all(
            "source_id" in rel and "target_id" in rel for rel in relationships
    ...
    async def test_metadata_mapping(self, pipeline):
        ...
        doc = ParsedDocument(doc_id="test_doc", file_path="test.md", chunks=[chunk])
    ...
    async def test_sentinel_values(self, pipeline):
        ...
        chunk = ParserChunk(content="Some text content", fts_text="Some text content")

        doc = ParsedDocument(doc_id="test_doc", file_path="test.txt", chunks=[chunk])
...
""")
        ...
        try:
            ...
            assert all(
                chunk.content and len(chunk.content.strip()) > 0
            ...
            chunks = await pipeline.db_manager.advanced_filter(
                "document_chunks", filters={"file_path": str(test_file)}
            )
            assert len(chunks) > 0
            ...
            # Verify graph entities were created
            entities = await pipeline.db_manager.advanced_filter(
                "graph_entities", filters={"file_path": str(test_file)}
            )
...
class TestLanceDBStorage:
    ...
    async def test_document_chunks_table(self, pipeline):
        ...
        chunk = ParserChunk(content="Test content", fts_text="Test content")
    ...
    async def test_graph_entities_for_document_sections(self, pipeline):
        ...
        doc = ParsedDocument(
            doc_id="test_md_doc", file_path="test.md", chunks=[chunk1, chunk2]
        )
        ...
        entities = await pipeline.db_manager.advanced_filter(
            "graph_entities", filters={"doc_id": "test_md_doc"}
        )
        ...
        assert len(doc_section_entities) == 2, (
            f"Expected 2 doc_section entities, got {len(doc_section_entities)}: {[e.get('name') for e in doc_section_entities]}"
        )
    ...
    @pytest.mark.asyncio
    async def test_markdown_file_creates_graph_entities(
        self, pipeline, temp_project_root
    ):
...
""")
        ...
        doc_section_chunks = [
            c for c in parsed_doc.chunks if c.element_type == "doc_section"
        ]
        assert len(doc_section_chunks) >= 2, (
            f"Parser should create doc_section chunks: {[c.element_type for c in parsed_doc.chunks]}"
        )
        ...
        # Verify graph entities were created
        entities = await pipeline.db_manager.advanced_filter(
            "graph_entities", filters={"doc_id": parsed_doc.doc_id}
        )
        ...
        entity_names = {e["name"] for e in doc_section_entities}
        assert "Main Title" in entity_names or any("Main" in n for n in entity_names), (
            f"Missing Main Title in {entity_names}"
        )
    ...
    @pytest.mark.asyncio
    async def test_document_relationships_contains_follows(
        self, pipeline, temp_project_root
    ):
...
""")
        ...
        doc_relationships = [
            r for r in relationships if r.get("type") in ("contains", "follows")
        ]
        ...
        assert len(contains_rels) >= 1, (
            f"Expected contains relationships, got {len(contains_rels)}"
        )
        ...
        assert len(follows_rels) >= 1, (
            f"Expected follows relationships, got {len(follows_rels)}"
        )
        ...
        for rel in contains_rels:
            ...
            assert "::" in rel["source_id"], (
                f"source_id should be a qualified entity ID: {rel['source_id']}"
            )
    ...
    async def test_vector_embeddings(self, pipeline):
        ...
        chunk = ParserChunk(
            content="Test content for embedding", fts_text="Test content for embedding"
        )
