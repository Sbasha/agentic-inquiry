...
class TestResolutionFailureLogging:
    ...
    async def test_logs_no_entities_found(
        ...
        assert result is None
        assert log_capture.has_message_containing("No entities found for symbol", "DEBUG")
    ...
    async def test_logs_database_errors(
        ...
        mock_db_manager.query_entities.side_effect = Exception("Database connection failed")
        ...
        assert log_capture.has_message_containing("Error", "DEBUG") or \
               log_capture.has_message_containing("Error", "ERROR")
...
class TestResolutionSuccessLogging:
    ...
    async def test_logs_successful_exact_match(
        ...
        assert any("exact match" in msg.lower() or "improved symbol resolution" in msg.lower() 
                  for msg in messages)
    ...
    async def test_logs_resolution_strategy_used(
        ...
        strategy_mentioned = any(
            "fuzzy match" in msg.lower() or 
            "proximity" in msg.lower() or
            "exact match" in msg.lower() or
            "improved symbol resolution" in msg.lower()
            for msg in messages
        )
