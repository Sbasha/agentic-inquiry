...
async def test_end_to_end_async_indexing(pipeline):
    ...
    chunks = await pipeline.db_manager.advanced_filter(
        "document_chunks", filters={"file_path": "test_file.py"}
    )
    assert len(chunks) == 3

    # Verify entities were indexed
    entities = await pipeline.db_manager.advanced_filter(
        "graph_entities", filters={"file_path": "test_file.py"}
    )
    # Expect: 1 file entity + 3 code entities (one per symbol)
    assert len(entities) >= 3
    code_entities = [
        e for e in entities if e.get("type") not in ("file", "code_module")
    ]
...
async def test_concurrent_document_processing(pipeline):
    ...
    num_docs = 20
    docs = [
        create_test_document(f"test_file_{i}.py", num_chunks=3) for i in range(num_docs)
    ]
    ...
    for i in range(num_docs):
        chunks = await pipeline.db_manager.advanced_filter(
            "document_chunks", filters={"file_path": f"test_file_{i}.py"}
        )
        assert len(chunks) == 3
...
async def test_no_event_loop_blocking(pipeline):
    ...
    await asyncio.gather(pipeline.process_document(doc), check_loop_responsiveness())
...
async def test_concurrent_processing_with_cache(pipeline, tmp_path):
    ...
    try:
        ...
        docs = [
            create_test_document(file_path, num_chunks=2) for file_path in sample_files
        ]
...
async def test_performance_improvement(pipeline):
    ...
    num_docs = 50
    docs = [
        create_test_document(f"perf_test_{i}.py", num_chunks=2) for i in range(num_docs)
    ]
    ...
    for i in range(num_docs):
        chunks = await pipeline.db_manager.advanced_filter(
            "document_chunks", filters={"file_path": f"perf_test_{i}.py"}
        )
    ...
    assert throughput > 5.0, f"Throughput too low: {throughput:.1f} docs/sec"
...
async def test_semaphore_limits_concurrency(pipeline):
    ...
    docs = [
        create_test_document(f"concurrent_test_{i}.py", num_chunks=1) for i in range(30)
    ]
    ...
    assert max_concurrent <= 10, f"Exceeded semaphore limit: {max_concurrent} > 10"

    # Should have some concurrency (not purely sequential)
    assert max_concurrent >= 2, f"No concurrency detected: {max_concurrent}"
