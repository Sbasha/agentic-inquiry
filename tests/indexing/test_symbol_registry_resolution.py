...
def registry():
    ...
    return SymbolRegistry(
        project_root="/tmp/test_project", project_id="test_project", cache_size=128
    )
...
def test_resolution_exact_match(registry):
    ...
    async def run():
        ...
        result = registry.resolve_with_cache(
            "MyClass", "class", "/tmp/test_project/main.py"
        )
...
def test_resolution_with_frequency_data(registry):
    ...
    async def run():
        ...
        for i in range(10):
            await registry.track_import(
                "DataProcessor",
                f"/tmp/test_project/file{i}.py",
                "/tmp/test_project/utils.py",
            )
        await registry.track_import(
            "DataProcessor",
            "/tmp/test_project/file_x.py",
            "/tmp/test_project/helpers.py",
        )

        # Resolve from a new file - should prefer utils.py due to much higher frequency
        result = registry.resolve_with_cache(
            "DataProcessor", "class", "/tmp/test_project/new_file.py"
        )
...
def test_resolution_with_co_occurrence_data(registry):
    ...
    async def run():
        ...
        await registry.track_import(
            "Config", source_file, "/tmp/test_project/config.py"
        )
        await registry.track_import(
            "Config", source_file, "/tmp/test_project/config.py"
        )
        ...
        assert (
            registry.get_co_occurrence_score(source_file, "/tmp/test_project/config.py")
            == 3
        )
        assert (
            registry.get_co_occurrence_score(
                source_file, "/tmp/test_project/settings.py"
            )
            == 0
        )
...
def test_resolution_with_file_naming_pattern(registry):
    ...
    async def run():
        ...
        result = registry.resolve_with_cache(
            "UserManager", "class", "/tmp/test_project/main.py"
        )
        ...
        naming_score = registry.score_file_naming_pattern(
            "UserManager", "/tmp/test_project/user_manager.py"
        )
...
def test_resolution_with_directory_structure(registry):
    ...
    async def run():
        ...
        score1 = registry.score_directory_structure(
            "AuthService", "/tmp/test_project/auth/service.py"
        )
        registry.score_directory_structure(
            "AuthService", "/tmp/test_project/services/auth_service.py"
        )
...
def test_resolution_no_match(registry):
    ...
    async def run():
        ...
        result = registry.resolve_with_cache(
            "NonExistent", "class", "/tmp/test_project/main.py"
        )
...
def test_resolution_low_confidence(registry):
    ...
    async def run():
        ...
        result = registry.resolve_with_cache(
            "Obscure", "function", "/tmp/test_project/other.py"
        )
...
def test_resolution_cache_behavior(registry):
    ...
    async def run():
        ...
        # First resolution
        result1 = registry.resolve_with_cache(
            "CachedClass", "class", "/tmp/test_project/main.py"
        )
        ...
        result2 = registry.resolve_with_cache(
            "CachedClass", "class", "/tmp/test_project/main.py"
        )
...
def test_resolution_type_mismatch(registry):
    ...
    async def run():
        ...
        result = registry.resolve_with_cache(
            "helper", "class", "/tmp/test_project/main.py"
        )
...
def test_resolution_composite_scoring(registry):
    ...
    async def run():
        ...
        await registry.track_import(
            "Service", "/tmp/test_project/file1.py", "/tmp/test_project/utils.py"
        )
        await registry.track_import(
            "Service", "/tmp/test_project/file2.py", "/tmp/test_project/utils.py"
        )
...
def test_async_protocol_methods(registry):
    ...
    async def run():
        ...
        await registry.track_import(
            "TestClass", "/tmp/test_project/main.py", "/tmp/test_project/test.py"
        )
