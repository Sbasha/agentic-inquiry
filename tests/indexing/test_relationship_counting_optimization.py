...
async def create_graph_builder_setup():
    ...
    mock_db_manager = InMemoryLanceDBManager(
        uri=f"memory://test-property-{uuid.uuid4().hex[:8]}"
    )
...
async def test_chunk_relationships_called_once_per_chunk(chunks):
    ...
    # Patch the method
    with patch.object(
        graph_builder, "_chunk_relationships", side_effect=tracked_chunk_relationships
    ):
        # Process the chunks
...
async def test_relationship_count_matches_pending_queue():
    ...
    expected_count = sum(
        len(chunk.relationships) for chunk in chunks if chunk.relationships
    )
