...
class TestEventEmissionLatency:
    ...
    async def test_concurrent_event_emission_latency(self, tmp_path: Path):
        ...
        system = await EventSystem.from_config(
            config, project_id="perf_test_concurrent"
        )
        async with system:
            # Warm up
            await system.emit("test.warmup", source="perf_test")
            ...
            # Flatten results
            all_latencies = [
                lat for task_latencies in results for lat in task_latencies
            ]
...
class TestBatchWriteThroughput:
    ...
    async def test_batch_write_throughput(self, tmp_path: Path):
        ...
        system = await EventSystem.from_config(
            config, project_id="perf_test_throughput"
        )
        async with system:
...
class TestQueryPerformance:
    ...
    async def test_operation_query_performance(self, tmp_path: Path):
        ...
        store = await EventStore.from_config(
            config=config, project_id="perf_test_query", db_path=db_path
        )
    ...
    async def test_type_filter_query_performance(self, tmp_path: Path):
        ...
        store = await EventStore.from_config(
            config=config, project_id="perf_test_type_query", db_path=db_path
        )
        await store._ensure_initialized()
    ...
    async def test_time_range_query_performance(self, tmp_path: Path):
        ...
        store = await EventStore.from_config(
            config=config, project_id="perf_test_time_query", db_path=db_path
        )
        await store._ensure_initialized()
...
class TestMemoryUsage:
    ...
    async def test_memory_usage_under_load(self, tmp_path: Path):
        ...
        async with system:
            ...
            baseline_memory = sum(
                stat.size for stat in baseline_snapshot.statistics("lineno")
            )
