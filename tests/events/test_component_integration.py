...
async def test_indexing_pipeline_has_event_system(tmp_path: Path):
    ...
    try:
        ...
        assert hasattr(pipeline, "event_system"), (
            "IndexingPipeline missing event_system"
        )
        assert isinstance(pipeline.event_system, EventSystem), (
            "event_system is not EventSystem instance"
        )
        assert pipeline.event_system.project_id == "test_project", (
            "EventSystem has wrong project_id"
        )

        # Verify it can emit events
        await pipeline.event_system.emit(
            "test.event", source="test", test_field="test_value"
        )
...
async def test_search_service_has_event_system(tmp_path: Path):
    ...
    try:
        ...
        # Verify EventSystem is initialized
        assert hasattr(search_service, "event_system"), (
            "SearchService missing event_system"
        )
...
async def test_parser_chain_has_event_system(tmp_path: Path):
    ...
    try:
        ...
        assert parser_chain.event_system.project_id == "test_project", (
            "EventSystem has wrong project_id"
        )
...
async def test_file_watcher_has_event_system(tmp_path: Path):
    ...
    try:
        watcher = FileWatcher(
            config=config, project_id="test_project", event_system=event_system
        )
        ...
        assert watcher.event_system.project_id == "test_project", (
            "EventSystem has wrong project_id"
        )
...
async def test_event_system_persistence(tmp_path: Path):
    ...
    try:
        ...
        await event_system.emit(
            "test.operation", source="test", status="success", test_field="test_value"
        )
