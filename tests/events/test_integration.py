...
async def test_multi_project_isolation(tmp_path: Path):
    ...
    try:
        ...
        success = await AsyncTestHelper.wait_for_async_condition(
            lambda: system_a.store.get_operation_events(
                operation_id, project_id="project_a"
            ),
        ...
        success = await AsyncTestHelper.wait_for_async_condition(
            lambda: system_b.store.get_operation_events(
                operation_id, project_id="project_b"
            ),
...
async def test_track_operation_context_manager(tmp_path: Path):
    ...
    try:
        ...
        store = await EventStore.from_config(
            db_path=tmp_path / "test_events.db", project_id="test_project"
        )
...
async def test_concurrent_operations(tmp_path: Path):
    ...
    try:
        ...
        tasks = [simulate_operation(op_id, 10) for op_id in operation_ids]
    ...
    try:
        ...
        for i in range(10):
            event = Event(
                ...
                if i < 5
                else EventTypes.Search.QUERY_STARTED,
                status=EventStatus.PROGRESS,
                source="query_test",
                metadata={"index": i},
            )
        ...
        indexing_events = await store.get_events_by_type(
            EventTypes.Indexing.PROGRESS, project_id="test_project"
        )
        assert len(indexing_events) == 5

        search_events = await store.get_events_by_type(
            EventTypes.Search.QUERY_STARTED, project_id="test_project"
        )
        ...
        early_events = await store.get_events_by_time_range(
            start_time=base_time, end_time=mid_time, project_id="test_project"
        )
        ...
        type_count = await store.count_events(
            event_type=EventTypes.Indexing.PROGRESS, project_id="test_project"
        )
        assert type_count == 5
...
async def test_mcp_server_event_persistence_end_to_end(tmp_path: Path):
    ...
    try:
        ...
        events_by_operation = await event_system.store.get_operation_events(
            operation_id
        )
        ...
        our_progress_events = [
            e for e in progress_events if e.operation_id == operation_id
        ]
        ...
        our_events_in_range = [
            e for e in events_by_time if e.operation_id == operation_id
        ]
        ...
        latest_events = await event_system.store.get_latest_events(
            limit=10, project_id="test_project"
        )
