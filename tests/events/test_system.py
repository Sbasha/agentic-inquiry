...
class TestEventEmission:
    ...
    async def test_emit_with_correlation_id_auto_extraction(
        ...
        with correlation_context() as corr_id:
            await temp_event_system.emit(
                "test.event", source="test", status=EventStatus.STARTED
            )
    ...
    async def test_emit_increments_metrics(self, temp_event_system: EventSystem):
        ...
        for i in range(5):
            await temp_event_system.emit(
                f"test.event.{i}", source="test", status=EventStatus.PROGRESS
            )
...
class TestBackgroundWriter:
    ...
    async def test_flush_interval_timeout(self, temp_event_system: EventSystem):
        ...
        await temp_event_system.emit(
            "test.event.1", source="test", status=EventStatus.PROGRESS
        )
        await temp_event_system.emit(
            "test.event.2", source="test", status=EventStatus.PROGRESS
        )
    ...
    async def test_error_handling_in_writer_loop(
        ...
        monkeypatch.setattr(
            temp_event_system.store, "store_events", failing_store_events
        )
        ...
        success = await AsyncTestHelper.wait_for_condition(
            lambda: len(store_events_calls) >= 1, timeout=5.0
        )
    ...
    async def test_exponential_backoff_retry_logic(
        ...
        # Wait for writer to process and recover
        success = await AsyncTestHelper.wait_for_condition(
            lambda: len(store_events_calls) >= 4, timeout=5.0
        )
        ...
        assert len(retry_delays) >= 2, (
            f"Expected at least 2 retry delays, got {retry_delays}"
        )
    ...
    async def test_successful_write_resets_retry_count(
        ...
        monkeypatch.setattr(
            temp_event_system.store, "store_events", intermittent_failure_store_events
        )

        # Emit first batch
        for i in range(3):
            await temp_event_system.emit(
                f"test.event.batch1.{i}", source="test", status=EventStatus.PROGRESS
            )
        ...
        for i in range(3):
            await temp_event_system.emit(
                f"test.event.batch2.{i}", source="test", status=EventStatus.PROGRESS
            )

        # Wait for second batch to be processed
        success = await AsyncTestHelper.wait_for_condition(
            lambda: len(store_events_calls) >= 3, timeout=5.0
        )
    ...
    async def test_batch_dropped_after_max_retries(
        ...
        monkeypatch.setattr(
            temp_event_system.store, "store_events", always_failing_store_events
        )
        ...
        assert len(store_events_calls) >= 6, (
            f"Expected at least 6 attempts, got {len(store_events_calls)}"
        )
...
class TestLifecycleManagement:
    ...
    async def test_async_context_manager(self, tmp_path: Path):
        ...
        config.events = EventsConfig(flush_interval_seconds=0.05)
        ...
        async with system:
            ...
            # Emit events
            await system.emit("test.event", source="test", status=EventStatus.STARTED)
...
class TestMetrics:
    ...
    async def test_queue_depth_property(self, temp_event_system: EventSystem):
        ...
        success = await AsyncTestHelper.wait_for_condition(
            lambda: temp_event_system.queue_depth <= depth_after_emit, timeout=5.0
        )
    ...
    @pytest.mark.asyncio
    async def test_sampling_metrics(self, temp_event_system_with_sampling: EventSystem):
...
class TestSamplingStrategy:
    ...
    async def test_sampling_preserves_failed_events(
        ...
        await system.emit(
            "test.failed", source="test", status=EventStatus.FAILED, error="Test error"
        )

        # Fill queue with progress events
        for i in range(10):
            await system.emit(
                f"test.progress.{i}", source="test", status=EventStatus.PROGRESS
            )

        # Wait for writer to process and verify failed event was stored
        success = await AsyncTestHelper.wait_for_async_condition(
            lambda: system.store.get_latest_events(limit=20),
            lambda events: len([e for e in events if e.status == EventStatus.FAILED])
            >= 1,
            timeout=5.0,
        )
    ...
    async def test_sampling_preserves_started_completed_events(
        ...
        await system.emit("test.started", source="test", status=EventStatus.STARTED)
        await system.emit("test.completed", source="test", status=EventStatus.COMPLETED)
...
class TestPerformanceMonitoring:
    ...
    async def test_timing_warning_logged_when_write_exceeds_threshold(
        ...
        monkeypatch.setattr(temp_event_system.store, "store_events", slow_store_events)
        ...
        timing_warnings = [
            ...
            if "EventStore write took" in msg and "threshold: 50ms" in msg
        ]
    ...
    async def test_no_timing_warning_when_write_under_threshold(
        ...
        assert len(timing_warnings) == 0, (
            f"Unexpected timing warning: {timing_warnings}"
        )
    ...
    async def test_timing_warning_includes_batch_size(
        ...
        timing_warnings = [
            msg
            for msg in log_messages
            if "EventStore write took" in msg and "batch of 3 events" in msg
        ]
        assert len(timing_warnings) > 0, (
            f"Expected timing warning with batch size, got: {log_messages}"
        )
    ...
    async def test_alert_logged_when_dropped_events_exceed_threshold(
        ...
        drop_alerts = [
            msg for msg in log_messages if "High event drop rate detected" in msg
        ]
    ...
    async def test_alert_includes_recommendation(
        ...
        alert_with_recommendation = [
            msg for msg in drop_alerts if "queue_max_size" in msg or "sampling" in msg
        ]
        assert len(alert_with_recommendation) > 0, (
            f"Expected recommendation in alert, got: {drop_alerts}"
        )
    ...
    async def test_alert_only_triggered_once(
        ...
        for i in range(50):
            await temp_event_system.emit(
                f"test.event.extra.{i}", source="test", status=EventStatus.PROGRESS
            )
        ...
        assert len(drop_alerts) == 1, (
            f"Expected exactly 1 alert, got {len(drop_alerts)}: {drop_alerts}"
        )
...
class TestCleanupScheduling:
    ...
    async def test_cleanup_task_created_in_start(self, tmp_path: Path):
        ...
        config.events = EventsConfig(cleanup_interval_hours=24, retention_days=30)
    ...
    async def test_cleanup_old_events_called_with_correct_retention(
        ...
        success = await AsyncTestHelper.wait_for_condition(
            lambda: len(cleanup_calls) > 0, timeout=10.0
        )
    ...
    @pytest.mark.asyncio
    async def test_vacuum_called_after_cleanup(self, tmp_path: Path, monkeypatch):
        ...
        success = await AsyncTestHelper.wait_for_condition(
            lambda: len(cleanup_calls) > 0 and len(vacuum_calls) > 0, timeout=10.0
        )
    ...
    async def test_cleanup_loop_handles_errors_gracefully(
        ...
        success = await AsyncTestHelper.wait_for_condition(
            lambda: len(cleanup_calls) >= 2, timeout=15.0
        )
    ...
    async def test_cleanup_loop_logs_purged_count(
        ...
        cleanup_logs = [
            msg for msg in log_messages if "purged" in msg.lower() and "42" in msg
        ]
        assert len(cleanup_logs) > 0, (
            f"Expected cleanup log with count 42, got: {log_messages}"
        )
