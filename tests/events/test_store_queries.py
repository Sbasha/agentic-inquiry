...
class TestEventStoreQueries:
    ...
    async def test_get_events_by_type_multiple_results(
    ...
    async def test_get_events_by_type_with_limit(self, populated_store: EventStore):
        ...
        events = await populated_store.get_events_by_type("indexing.started", limit=1)
    ...
    async def test_get_events_by_time_range(self, populated_store: EventStore):
        ...
        events = await populated_store.get_events_by_time_range(
            start_time=base_time, end_time=base_time + 3
        )
    ...
    async def test_get_events_by_time_range_with_type_filter(
        self, populated_store: EventStore
    ):
        ...
        events = await populated_store.get_events_by_time_range(
            start_time=base_time, end_time=base_time + 5, event_type="indexing.progress"
        )
    ...
    async def test_count_events_with_type_filter(self, populated_store: EventStore):
        ...
        count = await populated_store.count_events(event_type="indexing.progress")

        assert count == 1

    async def test_count_events_with_status_filter(self, populated_store: EventStore):
        """Test counting events filtered by status."""
        count = await populated_store.count_events(status=EventStatus.FAILED)
    ...
    async def test_count_events_with_time_range(self, populated_store: EventStore):
        ...
        count = await populated_store.count_events(
            start_time=base_time, end_time=base_time + 2
        )

        assert count == 3

    async def test_count_events_with_multiple_filters(
        self, populated_store: EventStore
    ):
    ...
    async def test_count_events_no_matches(self, populated_store: EventStore):
        ...
        count = await populated_store.count_events(event_type="nonexistent.type")
