...
class TestFileTrackerSchema:
    ...
    async def test_legacy_schema_migration(self, temp_db_path):
        ...
        async with aiosqlite.connect(tracker.db_path) as conn:
            ...
            await cursor.execute(
                "SELECT project_id, file_path, content_hash FROM file_states"
            )
...
class TestProjectIsolation:
    ...
    async def test_different_projects_isolated(self, temp_db_path, sample_file):
        ...
        tracker1 = await FileTracker.from_config(
            config=config1, project_id="project1", db_path=temp_db_path
        )
        ...
        tracker2 = await FileTracker.from_config(
            config=config2, project_id="project2", db_path=temp_db_path
        )
...
class TestBackwardCompatibility:
    ...
    async def test_explicit_path_overrides_config(self, temp_db_path, mock_config):
        ...
        tracker = await FileTracker.from_config(
            config=mock_config, project_id="test_project", db_path=temp_db_path
        )
