...
class TestSpecialCharacterSearch:
    ...
    async def test_search_with_question_mark(self, search_service, mock_db_manager):
        ...
        mock_db_manager.fts_search.return_value = _make_search_results(
            mock_dicts, source="fts"
        )
    ...
    @pytest.mark.asyncio
    async def test_search_with_multiple_special_chars(
    ...
    @pytest.mark.asyncio
    async def test_hybrid_search_with_special_chars(
        self, search_service, mock_db_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_sanitization_can_be_disabled(
        ...
        query = "How does this work?"
        _results = await service.fts_search(query_fts=query, limit=10)
...
class TestSanitizationLogging:
    ...
    @pytest.mark.asyncio
    async def test_sanitization_is_logged(
        self, search_service, mock_db_manager, caplog
    ):
        ...
        # Execute search with special characters
        query = "How does this work?"
        await search_service.fts_search(query_fts=query, limit=10)
    ...
    @pytest.mark.asyncio
    async def test_no_log_when_query_unchanged(
        self, search_service, mock_db_manager, caplog
    ):
        ...
        sanitization_logs = [
            r for r in caplog.records if "Query sanitized" in r.message
        ]
