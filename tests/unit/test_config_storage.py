...
class TestPathResolution:
    ...
    def test_nested_path_resolution(self, temp_storage_root):
        ...
        assert config.get_file_tracker_path() == temp_storage_root / "data" / "tracker.db"
...
class TestEnvironmentVariableOverrides:
    ...
    def test_typo_detection_invalid_section(self, monkeypatch, caplog):
        ...
        assert config.search.default_limit != 50, "Invalid env var should not have been applied"
...
@pytest.mark.filterwarnings("ignore:ensure_storage_directories.*deprecated:DeprecationWarning")
class TestDirectoryCreation:
    """Test directory creation via ensure_storage_directories."""

    @pytest.mark.filterwarnings("default:ensure_storage_directories.*deprecated:DeprecationWarning")
    def test_ensure_storage_directories_emits_deprecation_warning(self, temp_storage_root):
        ...
        with pytest.warns(DeprecationWarning, match="ensure_storage_directories.*deprecated"):
...
class TestHybridSearchWeightValidation:
    ...
    def test_invalid_weights(self, minimal_config_base, vector_weight, fts_weight, expected_substrings):
...
class TestRerankerTypeValidation:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("rrf", {}),
            ("rrf", {"k": 60}),
            ("cross_encoder", {"model_name": "cross-encoder/ms-marco-MiniLM-L-6-v2", "batch_size": 32}),
        ],
    )
...
class TestStorageConfigValidation:
    ...
    def test_schema_validation_error_raises_configuration_error(self, minimal_config_base):
    ...
    def test_missing_jsonschema_logs_warning(self, minimal_config_base, caplog, monkeypatch):
        ...
        try:
            ...
            assert any("jsonschema package not installed" in record.message 
                      for record in caplog.records)
        finally:
...
class TestConfigIntegration:
    ...
    def test_config_roundtrip(self, temp_storage_root):
        ...
        assert restored_config.storage.default_project_id == original_config.storage.default_project_id
        assert restored_config.storage.lancedb.path == original_config.storage.lancedb.path
        assert restored_config.storage.file_tracker.path == original_config.storage.file_tracker.path
        assert restored_config.storage.document_cache.enabled == original_config.storage.document_cache.enabled
        assert restored_config.storage.document_cache.path == original_config.storage.document_cache.path
