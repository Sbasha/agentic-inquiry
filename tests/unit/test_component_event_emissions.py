...
class TestMemorySystemEvents:
    ...
    @pytest.mark.asyncio
    async def test_store_emits_event(self, test_config, mock_event_system):
        """Test that store emits memory.stored event."""
        # Arrange
        import numpy as np

        mock_embedding_service = MagicMock()
        mock_embedding_service.embed_async = AsyncMock(
        ...
        await memory_system.store(
            content="Test memory content", context=context, importance=0.8
        )
    ...
    @pytest.mark.asyncio
    async def test_retrieve_emits_event(self, test_config, mock_event_system):
        ...
        # Act
        await memory_system.retrieve(query="test query", context=context, limit=5)
    ...
    @pytest.mark.asyncio
    async def test_consolidate_emits_event(self, test_config, mock_event_system):
