...
from agentic_inquiry.cache.document_cache import (
    DocumentCache,
    RestrictedUnpickler,
    safe_pickle_loads,
)
...
class TestCacheCorruptionRecovery:
    ...
    @pytest.mark.asyncio
    async def test_corrupted_index_json_recovery(
        self, disk_cache_config, sample_document, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_truncated_index_file_recovery(
        self, disk_cache_config, sample_document, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_corrupted_pickle_entry_recovery(
        self, disk_cache_config, sample_document, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_missing_entry_file_recovery(
        self, disk_cache_config, sample_document, tmp_path
    ):
...
class TestPartialWriteRecovery:
    ...
    @pytest.mark.asyncio
    async def test_temp_file_cleanup_on_failed_write(
        self, disk_cache, sample_document, tmp_path
    ):
        ...
        with patch(
            "aiofiles.os.replace", side_effect=OSError("Simulated write failure")
        ):
    ...
    @pytest.mark.asyncio
    async def test_interrupted_write_recovery(
        self, disk_cache_config, sample_document, tmp_path
    ):
...
class TestGracefulDegradation:
    ...
    @pytest.mark.asyncio
    async def test_fallback_to_memory_on_permission_error(
        self, tmp_path, sample_document
    ):
    ...
    @pytest.mark.asyncio
    async def test_cache_survives_disk_write_failures(
        self, disk_cache, sample_document, tmp_path
    ):
        ...
        with patch.object(
            disk_cache, "_save_to_disk", side_effect=Exception("Disk full")
        ):
    ...
    @pytest.mark.asyncio
    async def test_cache_survives_index_update_failures(
        self, disk_cache, sample_document, tmp_path
    ):
        ...
        with patch.object(
            disk_cache, "_update_index", side_effect=Exception("Index update failed")
        ):
    ...
    @pytest.mark.asyncio
    async def test_cache_operations_with_readonly_filesystem(
        self, tmp_path, sample_document
    ):
    ...
    @pytest.mark.asyncio
    async def test_cache_with_deleted_directory(
        self, disk_cache, sample_document, tmp_path
    ):
...
class TestIOErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_load_with_corrupted_entries_cleanup(
        self, disk_cache_config, sample_document, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_corrupted_entries_with_missing_index(
        self, disk_cache_config, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_with_file_deletion_error(
        self, disk_cache_config, sample_document, tmp_path
    ):
...
class TestEdgeCases:
    ...
    async def test_very_large_cache_entry(self, disk_cache, tmp_path):
        ...
        large_chunks = [ParserChunk(content="x" * 100000) for _ in range(100)]
    ...
    @pytest.mark.asyncio
    async def test_cache_with_special_characters_in_path(
        self, disk_cache, sample_document, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_cache_with_unicode_in_path(
        self, disk_cache, sample_document, tmp_path
    ):
