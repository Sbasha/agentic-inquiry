...
class TestElementTypeMapping:
    ...
    def test_docx_heading_types_mapped_to_doc_section(self):
        ...
        for i in range(1, 7):
            ...
            assert (
                UNSTRUCTURED_TO_ENTITY_TYPE[heading_style]
...
class TestHeadingExtraction:
    ...
    @pytest.mark.asyncio
    async def test_heading_chunks_have_correct_element_type(
        self, parser, outline_md_path
    ):
    ...
    async def test_heading_element_names_populated(self, parser, outline_md_path):
        ...
        for chunk in heading_chunks:
            assert chunk.element_name, (
                f"Heading chunk missing element_name: {chunk.content[:50]}"
            )
        ...
        assert any(
            "Presentation Outline" in name or "Agentic Inquiry" in name
    ...
    @pytest.mark.asyncio
    async def test_section_chunks_have_correct_element_type(
        self, parser, outline_md_path
    ):
        ...
        non_heading_chunks = [
            c for c in result.chunks if c.element_type != "doc_section"
        ]
...
class TestRelationshipExtraction:
    ...
    async def test_contains_relationships_in_chunks(self, parser, outline_md_path):
        ...
        for chunk in chunks_with_rels:
            contains_rels.extend(
                [r for r in chunk.relationships if r.type == "contains"]
            )
    ...
    async def test_follows_relationships_in_chunks(self, parser, outline_md_path):
        ...
        for chunk in heading_chunks:
            if chunk.relationships:
                follows_rels.extend(
                    [r for r in chunk.relationships if r.type == "follows"]
                )
    ...
    @pytest.mark.asyncio
    async def test_hierarchy_level_in_relationship_metadata(
        self, parser, outline_md_path
    ):
        ...
        for rel in rels_with_level:
            ...
            assert isinstance(level, int), (
                f"hierarchy_level should be int, got {type(level)}"
            )
...
class TestHeadingLevelDetection:
    ...
    async def test_markdown_heading_level_detection(self, parser, outline_md_path):
        ...
        assert len(levels_found) >= 2, (
            f"Expected multiple heading levels, found: {levels_found}"
        )
...
class TestMultipleDocumentFormats:
    ...
    async def test_consistent_behavior_across_formats(self, parser, outline_md_path):
        ...
        heading_chunks = [
            c for c in md_result.chunks if c.element_type == "doc_section"
        ]
        for chunk in heading_chunks:
...
class TestDocumentParserQuality:
    ...
    @pytest.mark.asyncio
    async def test_relationships_reference_valid_entities(
        self, parser, outline_md_path
    ):
...
""")
        ...
        chunk_a = next(
            (c for c in heading_chunks if c.element_name == "Section A"), None
        )
        chunk_b = next(
            (c for c in heading_chunks if c.element_name == "Section B"), None
        )
        chunk_c = next(
            (c for c in heading_chunks if c.element_name == "Section C"), None
        )
        ...
        a_follows_rels = [
            r for r in (chunk_a.relationships or []) if r.type == "follows"
        ]
        assert len(a_follows_rels) == 1, (
            f"Section A should have 1 'follows' rel, got {len(a_follows_rels)}"
        )
        ...
        b_follows_rels = [
            r for r in (chunk_b.relationships or []) if r.type == "follows"
        ]
        assert len(b_follows_rels) == 1, (
            f"Section B should have 1 'follows' rel, got {len(b_follows_rels)}"
        )
        ...
        c_follows_rels = [
            r for r in (chunk_c.relationships or []) if r.type == "follows"
        ]
...
""")
        ...
        parent_chunk = next(
            (c for c in heading_chunks if c.element_name == "Parent Section"), None
        )
        child_chunk = next(
            (c for c in heading_chunks if c.element_name == "Child Section"), None
        )
        ...
        parent_contains_rels = [
            r for r in (parent_chunk.relationships or []) if r.type == "contains"
        ]
        ...
        child_contains_rels = [
            r for r in (child_chunk.relationships or []) if r.type == "contains"
        ]
...
""")
        ...
        rel_identifiers = [
            (r.source_name, r.target_name, r.type) for r in all_relationships
        ]
        ...
        for chunk in result.chunks:
            if chunk.relationships and chunk.element_name:
                for rel in chunk.relationships:
                    ...
                    source_matches = (
                        rel.source_name == chunk.element_name
                        or rel.source_type
                        == "document"  # Document-level relationships OK
                    )
...
class TestPDFHeadingClassification:
    ...
    def test_long_text_not_classified_as_heading(self, parser):
        ...
        long_text = (
            "This is a very long piece of text that goes on and on for many characters "
            * 2
        )
        element_type = parser._classify_pdf_element(
            text=long_text,
...
class TestPDFStructureExtraction:
    ...
    async def test_pdf_creates_relationships(self, parser, pdf_sample_path):
        ...
        assert len(all_rels) > 0, "PDF should have relationships between sections"
    ...
    async def test_pdf_chunks_have_metadata(self, parser, pdf_sample_path):
        ...
        chunks_with_font = [
            c for c in result.chunks if c.metadata and "font_size" in c.metadata
        ]

        assert len(chunks_with_font) > 0, "PDF chunks should include font_size metadata"
