...
def test_example_script_runs(script_path: str):
    ...
    # Check exit code
    assert exit_code == 0, f"Example script {script_path} failed with exit code {exit_code}"
...
def test_basic_parsing_example_output():
    ...
    exit_code, stdout, stderr = run_example_script("examples/parsers/01_basic_parsing.py")
...
def test_parser_chain_example_output():
    ...
    exit_code, stdout, stderr = run_example_script("examples/parsers/02_parser_chain.py")
    
    assert exit_code == 0, "Parser chain example failed"
    
    # Should mention default priority
    assert "Default priority" in stdout or "priority order" in stdout, "Missing priority info"
...
def test_cache_watcher_example_output():
    ...
    exit_code, stdout, stderr = run_example_script("examples/parsers/03_cache_and_watcher.py")
    
    assert exit_code == 0, "Cache and watcher example failed"
    
    # Should show cache statistics
    assert "Cache Statistics:" in stdout or "cache" in stdout.lower(), "Missing cache info"
    
    # Should mention hits and misses
    assert "hit" in stdout.lower() or "miss" in stdout.lower(), "Missing cache hit/miss info"


def test_error_handling_example_output():
    """Test specific output from error handling example."""
    exit_code, stdout, stderr = run_example_script("examples/parsers/04_error_handling.py")
    ...
    assert "graceful" in stdout.lower() or "handled" in stdout.lower(), "Missing graceful handling"
