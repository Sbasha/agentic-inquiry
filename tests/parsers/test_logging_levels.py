...
async def test_parser_chain_logs_individual_failure_at_warning(caplog):
    ...
    try:
        chain = ParserChain(
            ...
            parser_names=["failing_parser", "success_parser"],
        )
...
async def test_parser_chain_logs_all_failures_at_error(caplog):
    ...
    try:
        ...
        assert "parser1" in error_record.message or hasattr(
            error_record, "parser_errors"
        )
        assert "parser2" in error_record.message or hasattr(
            error_record, "parser_errors"
        )
...
async def test_parser_chain_provides_actionable_information(caplog):
    ...
    failing_parser = MockFailingParser(
        "test_parser", "Specific error: missing dependency"
    )
    ...
    try:
        chain = ParserChain(
            event_system=_create_mock_event_system(), parser_names=["test_parser"]
        )
...
async def test_debug_logs_not_required_for_production(caplog):
    ...
    try:
        chain = ParserChain(
            event_system=_create_mock_event_system(), parser_names=["debug_test_parser"]
        )
