...
async def test_parser_chain_handles_unregistered_parsers():
    ...
    try:
        ...
        chain = ParserChain(
            event_system=_create_mock_event_system(),
            parser_names=["nonexistent_parser", "parser2"],
        )
...
async def test_parser_chain_with_parser_without_can_parse():
    ...
    try:
        chain = ParserChain(
            event_system=_create_mock_event_system(), parser_names=["simple"]
        )
        result = await chain.parse("/test/file.txt")
...
async def test_parser_chain_logs_all_failures_at_error_level(caplog):
    ...
    try:
        ...
        assert error_record.parser_errors == [
            "parser1: parser1 failed to parse",
            "parser2: parser2 failed to parse",
        ]
...
async def test_parser_chain_does_not_catch_keyboard_interrupt():
    ...
    try:
        chain = ParserChain(
            event_system=_create_mock_event_system(), parser_names=["interrupting"]
        )
...
async def test_parser_chain_does_not_catch_system_exit():
    ...
    try:
        chain = ParserChain(
            event_system=_create_mock_event_system(), parser_names=["exiting"]
        )
