...
def mock_element(draw, element_type=None):
    ...
    if element_type is None:
        element_type = draw(
            st.sampled_from(["Title", "NarrativeText", "ListItem", "Table", "Image"])
        )
...
def element_list(draw):
    ...
    for _ in range(num_elements):
        ...
        if is_heading:
            ...
            elem_type = draw(
                st.sampled_from(["NarrativeText", "ListItem", "Table", "Image"])
            )
...
class TestElementTypeDelegation:
    ...
    async def test_heading_elements_call_process_heading(self, elements):
        ...
        with (
            patch.object(parser, "_process_heading_element") as mock_heading,
            ...
            heading_count = sum(
                1 for elem in elements if elem.__class__.__name__ == "Title"
            )
    ...
    async def test_non_heading_elements_call_process_non_heading(self, elements):
        ...
        # Mock the processing methods
        with (
            patch.object(parser, "_process_heading_element"),
            patch.object(parser, "_process_non_heading_element") as mock_non_heading,
        ):
            # Call the method under test
            parser._extract_relationships(elements, file_path)
    ...
    async def test_only_non_headings_no_heading_calls(self):
        ...
        elements = [
            create_mock_element("NarrativeText", f"Paragraph {i}") for i in range(5)
        ]
    ...
    @pytest.mark.asyncio
    async def test_delegation_count_matches_element_types(
        self, num_headings, num_non_headings
    ):
