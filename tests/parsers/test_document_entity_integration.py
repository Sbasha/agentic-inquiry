...
def test_document_parser_entity_extraction_integration():
    ...
    async def run():
        ...
        config.storage = StorageConfig(
            root="/tmp/project", default_project_id="test_project"
        )
        ...
        print(
            f"Registered {registered_count}/{len(expected_headings)} expected headings"
        )
        ...
        doc_entities = [
            ...
            if e.get("type") in ["doc_section", "section", "table", "figure"]
        ]
    ...
    async def run():
        ...
        for heading_name in test_headings:
            ...
            if results:
                ...
                assert metadata.entity_type == "doc_section", (
                    f"Type should be 'heading', got {metadata.entity_type}"
                )
                assert metadata.file_path.endswith("outline.md"), (
                    f"File path should end with outline.md: {metadata.file_path}"
                )
                assert metadata.is_exported is True, (
                    "Document entities should be exported"
                )
...
def test_6_2_relationship_creation_in_lancedb():
    ...
    async def run():
        ...
        assert len(all_relationships) > 0, (
            "Should have created relationships in database"
        )
...
def test_6_3_search_returns_document_entities():
    ...
    async def run():
        ...
        search_service = SearchService(
            storage=storage, config=config, event_system=mock_event_system
        )
        ...
        for query in search_queries:
            results = await search_service.fts_search(query_fts=query, limit=10)
            ...
            if results:
                ...
                if entity_results:
                    ...
                    assert sample.get("file_path", "").endswith("outline.md"), (
                        "Should be from outline.md"
                    )
    ...
    async def run():
        ...
        if heading_entities:
            ...
            for rel in contains_rels[:5]:  # Limit to first 5
                ...
                if target_id in entities_by_id:
                    ...
                    print(
                        f"    - Child: {child.get('name')} (type: {child.get('type')})"
                    )
        ...
        if follows_rels:
            ...
            if source_entity and target_entity:
                ...
                print(
                    f"    - Source: {source_entity.get('name')} (type: {source_entity.get('type')})"
                )
                print(
                    f"    - Target: {target_entity.get('name')} (type: {target_entity.get('type')})"
                )
            ...
            if descendants_count > 0:
                ...
                for entity_id in list(visited)[1:6]:  # Skip root, show first 5
                    if entity_id in entities_by_id:
                        ...
                        print(
                            f"    - {entity.get('name')} (type: {entity.get('type')})"
                        )
