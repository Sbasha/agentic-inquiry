...
async def run(
    ...
    check(
        ...
        severity="CRITICAL",
        fail_msg="No entities found via list_entities or find_similar",
    )
    ...
    check(
        ...
        len(class_ents) > 0 and len(func_ents) > 0,
        {
    ...
    if total_ents > 0:
        note_adoption(
            journal,
            f"extracted {total_ents} function/class/method entities — structured code intelligence vs raw text",
            "positive",
        )
    else:
        note_adoption(
            journal,
            "entity extraction returned nothing — no structured code intelligence available",
            "blocker",
        )
    ...
    if similar:
        ...
        if candidate and candidate != "BaseService":
            ...
            inh = r.get("inherits", []) or (
                r.get("dependencies") if isinstance(r.get("dependencies"), dict) else {}
            ).get("inherits", [])
    ...
    check(
        results,
        issues,
        "T1_2_relationship_types",
        has_imports or has_calls or has_defines,
        {
    ...
    if rel_count >= 2:
        note_adoption(
            journal,
            f"verified {rel_count}/4 relationship types (imports/calls/defines/inherits) — relationship graph grep cannot build",
            "positive",
        )
    elif rel_count == 0:
        note_adoption(
            journal,
            "no relationship types verified — graph is empty, no advantage over grep",
            "negative",
        )
    ...
    check(
        ...
        entity_found,
        {
            ...
            "elapsed_s": round(t, 2),
            "dep_keys": list(r.get("dependencies", {}).keys())
            if isinstance(r.get("dependencies"), dict)
            else [],
        },
    )
    if entity_found:
        dep_keys = (
            list(r.get("dependencies", {}).keys())
            if isinstance(r.get("dependencies"), dict)
            else []
        )
        note_adoption(
            journal,
            f"traced import chain for LanceDBManager ({len(dep_keys)} dep categories) — cross-file dependency tracing grep can't do",
            "positive",
        )
    ...
    check(
        ...
        issues,
        "T2_2_cross_file",
        impact_radius > 0 or len(affected) > 0,
        {
    ...
    r, t = await call_tool(
        ...
        services=services,
        session_id=session_id,
        entity="search_knowledge",
        include_dependencies=True,
    )
    entity_found = "error" not in r and r.get("entity") is not None
    calls_data = r.get("calls", []) or (
        r.get("dependencies") if isinstance(r.get("dependencies"), dict) else {}
    ).get("calls", [])
    check(
        results,
        issues,
        "T3_1_call_relationships",
        entity_found,
        {
            "entity_found": entity_found,
    ...
    entity_found = "error" not in r and r.get("entity") is not None
    deps = r.get("dependencies", {})
    internal = r.get("internal_calls", []) or (
        deps.get("calls", []) if isinstance(deps, dict) else []
    )
    check(
        results,
        issues,
    ...
    if impact_radius > 0 or len(affected) > 0:
        note_adoption(
            journal,
            f"blast radius analysis found {max(impact_radius, len(affected))} affected entities — impact analysis without manual tracing",
            "positive",
        )
    else:
        note_adoption(
            journal,
            "blast radius returned 0 affected entities — no impact analysis available",
            "negative",
        )
    ...
    check(
        results,
        issues,
        "T5_1_entity_resolution",
        len(config_ents) > 0 or len(process_ents) > 0,
        {
    ...
    if len(config_ents) > 1 or len(process_ents) > 1:
        note_adoption(
            journal,
            f"resolved ambiguous names: {len(config_ents)} Config classes, {len(process_ents)} process functions — intelligent disambiguation vs grep's literal matching",
            "positive",
        )
    elif len(config_ents) > 0 or len(process_ents) > 0:
        note_adoption(
            journal,
            "entity resolution found results but limited disambiguation data",
            "neutral",
        )
    ...
    if perf.get("depth1_pass", False):
        note_adoption(
            journal,
            f"graph traversal completed in {depth1_s:.1f}s at depth 1 — relationship navigation at scale",
            "positive",
        )
    else:
        note_adoption(
            journal,
            f"graph traversal took {depth1_s:.1f}s or returned empty — too slow or broken for interactive use",
            "negative",
        )
    if depth1_s > 10.0:
        note_adoption(
