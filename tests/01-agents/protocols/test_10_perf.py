...
async def run(
    ...
    try:
        ...
        search_files = [
            ...
            if any(
                kw in res.get("file_path", "") + res.get("content", "").lower()
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="database query vector similarity search latency N+1 query async await",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
        ...
        check(
            results,
            issues,
            "T1_2_bottleneck_characterization",
            found_db,
            {
        ...
        check(
            results, issues, "T1_2_bottleneck_characterization", False, fail_msg=str(e)
        )
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="chunk embedding dimension limit result count search candidates rerank",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="search result deduplication data structure aggregation merge scores",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        ...
        cache_patterns = [
            ...
            for res in results_list[:5]
            if any(
                kw in res.get("content", "").lower() + res.get("file_path", "").lower()
        ...
        log(
            test_id,
            f"T3.1: {count} results, {len(cache_patterns)} cache patterns found",
        )
        if found_cache:
            note_adoption(
                journal,
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="batch size chunk bulk insert operation concurrent parallel async",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
        ...
        log(
            test_id,
            f"T3.2: {count} results, {len(batch_patterns)} batch patterns found",
        )
    except Exception as e:
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="pool_size max_overflow connection pool limit config performance tuning",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="MIN_VECTOR_SCORE MIN_FTS_SCORE threshold filter pre-filter limit candidates",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="RRF reciprocal rank fusion BM25 approximate nearest neighbor HNSW index",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
        ...
        log(
            test_id,
            f"T5.1: {count} results, {len(alt_algorithms)} algorithm alternatives found",
        )
        if found_alts:
            note_adoption(
                journal,
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="SentenceTransformer embedding model vector search LanceDB PostgreSQL pgvector AlloyDB",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
        ...
        log(
            test_id,
            f"T5.2: {count} results, {len(lib_alts)} library alternatives found",
        )
    except Exception as e:
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="IDF weighted reranking score boost normalization RRF k=30 dual_source_bonus",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="index optimization vector index creation maintenance HNSW IVF performance improvement",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
        ...
        log(
            test_id,
            f"T6.2: {count} results, {len(quick_wins)} quick win opportunities found",
        )
    except Exception as e:
    ...
    try:
        ...
        check(
            ...
            severity="HIGH",
            fail_msg=f"Search latency too high: avg={avg_latency:.1f}s",
        )
