...
async def run(
    ...
    log(
        test_id,
        f"Code indexing done: chunks={idx.get('chunks_created', '?')}, "
    ...
    log(
        test_id,
        f"Doc indexing: completed={idx_docs.get('completed')}, "
    ...
    log(
        test_id,
        f"Thresholds OK: chunks={total_chunks}, entities={entities_count}, verify={verify_count}",
    )
    ...
    check(
        ...
        t11_pass,
        {
            ...
            "top5": [
                {
                    "file": c.get("file_path", c.get("name", "")),
    ...
    log(
        test_id,
        f"T1.1: total={total_11}, decreasing={scores_decreasing} -> {'PASS' if t11_pass else 'FAIL'}",
    )
    ...
    query_tests = [
        ...
        ("search", "short"),
        (
            "implement a caching layer for database queries to improve performance",
            "long",
        ),
    ]
    ...
    for q, qtype in query_tests:
        ...
        top_score = (
            all_items[0].get(
                "similarity",
                all_items[0].get("score", all_items[0].get("relevance_score", 0)),
            )
            if all_items
            else 0
        )
        top_file = (
            all_items[0].get("file_path", all_items[0].get("name", ""))
            if all_items
            else ""
        )
        log(
            test_id,
            f"  [{qtype}] -> {cnt} results, top={top_score:.3f} ({str(top_file)[:50]})",
        )
        t12_details[qtype] = {
            "count": cnt,
    ...
    check(
        results,
        issues,
        "T1_2_multiple_queries",
        all_returned,
        {
            "queries": t12_details,
            "all_returned_results": all_returned,
            "queries_with_results": sum(
                1 for v in t12_details.values() if v["count"] > 0
            ),
        },
    ...
    log(
        test_id,
        f"T1.2: {sum(1 for v in t12_details.values() if v['count'] > 0)}/{len(query_tests)} queries returned results -> {'PASS' if all_returned else 'FAIL'}",
    )
    ...
    cross_file = (
        len(set(e.get("file_path", e.get("name", i)) for i, e in enumerate(all_lance)))
        > 1
    )
    ...
    check(
        ...
        t21_pass,
        {
            ...
            "top_lance": [
                {
                    "name": e.get("name", e.get("file_path", "")),
                    "type": e.get("type", e.get("entity_type", "")),
                    "score": e.get("similarity", 0),
                }
    ...
    log(
        test_id,
        f"T2.1: lance={lance_total}, search={search_total}, cross_file={cross_file} -> {'PASS' if t21_pass else 'FAIL'}",
    )
    ...
    conf_total = r_conf.get(
        "total",
        len(r_conf.get("similar_entities", []))
        + len(r_conf.get("similar_content", [])),
    )

    code_content = [c for c in embed_content if c.get("content_type") == "CODE"]
    doc_content = [
        c for c in embed_content if c.get("content_type") not in ("CODE", None)
    ]
    ...
    check(
        ...
        t22_pass,
        {
            ...
            "note": "SG-001: hard CODE filter may block doc results"
            if sg001_active
            else "mixed results",
        },
    ...
    log(
        test_id,
        f"T2.2: total={total_embed}, sg001_active={sg001_active} -> {'PASS' if t22_pass else 'FAIL'}",
    )
    ...
    for thresh in [0.8, 0.5, 0.3]:
        ...
        t_cnt = r_t.get("total", 0) or len(r_t.get("similar_entities", [])) + len(
            r_t.get("similar_content", [])
        )
        if t_cnt == 0:
        ...
        top_score = (
            items_3[0].get("similarity", items_3[0].get("score", 0)) if items_3 else 0
        )
        log(
            test_id,
            f"  thresh={thresh}: {t_cnt} results, top={top_score:.3f}, time={elapsed:.2f}s",
        )
        thresh_results[str(thresh)] = {
            "count": t_cnt,
            "top_score": round(top_score, 3),
            "time_s": round(elapsed, 2),
        }
    ...
    log(
        test_id,
        f"T3.1: monotonic={monotonic} ({c8}<={c5}<={c3}) -> {'PASS' if t31_pass else 'FAIL'}",
    )
    ...
    ns_count = r_ns.get(
        "total",
        len(r_ns.get("similar_entities", [])) + len(r_ns.get("similar_content", [])),
    )
    ns_error = r_ns.get("error")
    log(
        test_id,
        f"  Nonsense query -> {ns_count} results, error={ns_error}, time={t_ns:.2f}s",
    )
    ...
    ht_count = r_ht.get(
        "total",
        len(r_ht.get("similar_entities", [])) + len(r_ht.get("similar_content", [])),
    )
    ht_error = r_ht.get("error")
    log(
        test_id,
        f"  High thresh 0.99 -> {ht_count} results, error={ht_error}, time={t_ht:.2f}s",
    )
    ...
    log(
        test_id, f"  'caching and memoization' -> {cache_cnt} results in {t_cache:.2f}s"
    )
    ...
    log(
        test_id,
        f"  'input validation and sanitization' -> {val_cnt} results in {t_val:.2f}s",
    )
    ...
    log(
        test_id,
        f"T4.1: cache={cache_cnt}, validation={val_cnt} -> {'PASS' if t41_pass else 'FAIL'}",
    )
    ...
    log(
        test_id,
        f"  chunks={ctx_chunks}, tokens={ctx_tokens}, error={ctx_error}, time={t_ctx:.2f}s",
    )
    ...
    log(
        test_id,
        f"T4.2: has_content={has_content}, chunks={ctx_chunks} -> {'PASS' if t42_pass else 'FAIL'}",
    )
    ...
    for limit_val, thresh_ms in [(10, 10000), (50, 15000), (100, 20000)]:
        ...
        log(
            test_id,
            f"  limit={limit_val}: {ms:.0f}ms (thresh={thresh_ms}ms) -> {'PASS' if ok_p else 'FAIL'} ({cnt} results)",
        )
        perf[f"limit_{limit_val}"] = {
            "ms": round(ms),
    ...
    check(
        ...
        severity="LOW",
        fail_msg="Performance thresholds exceeded",
    )
    if t51_pass:
        perf_summary = ", ".join(
            f"limit={k.split('_')[1]}:{v['ms']}ms" for k, v in perf.items()
        )
        note_adoption(
            journal,
    ...
    log(
        test_id,
        f"T5.1: {sum(1 for v in perf.values() if v['ok'])}/{len(perf)} within threshold -> {'PASS' if t51_pass else 'FAIL'}",
    )
