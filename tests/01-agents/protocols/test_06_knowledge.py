...
async def run(
    ...
    patterns = [
        ("Async-first architecture", "All database and I/O operations use async/await", ["architecture", "async"]),
        ("Facade storage pattern", "StorageFacade abstracts multiple backends (LanceDB, PostgreSQL, AlloyDB)", ["architecture", "storage"]),
        ("Tree-sitter parsing", "Code parsing uses tree-sitter for AST extraction", ["parsing", "ast"]),
        ("Hybrid search pipeline", "Search combines vector similarity + FTS with RRF reranking", ["search", "algorithm"]),
        ("Event-driven indexing", "Indexing emits events (started/completed/failed) for async coordination", ["indexing", "events"]),
    ]
    ...
    check(results, issues, "T1_1_pattern_storage", len(stored_ids) >= 3, {
    ...
    decisions = [
        ("AlloyDB chosen for production", "AlloyDB selected over PostgreSQL for server-side embeddings and managed scaling", ["decision", "storage"]),
        ("RRF over simple ranking", "Reciprocal Rank Fusion chosen to combine vector + FTS results effectively", ["decision", "search"]),
        ("MCP over REST API", "Model Context Protocol chosen for Claude Code integration over REST", ["decision", "api"]),
    ]
    ...
    check(results, issues, "T1_2_decision_storage", len(decision_ids) >= 2, {
    ...
    gotchas = [
        ("AlloyDB connection pool exhaustion", "Running parallel tests with separate MCPServer instances exhausts connection pools. Must share one server.", ["gotcha", "alloydb"]),
        ("Python 3.13 closure scoping", "Nested async functions in 3.13 need default parameter capture for closure variables", ["gotcha", "python"]),
        ("Server-side embedding init order", "ai.initialize_embeddings() must run before any rows have embeddings (4MB Vertex limit)", ["gotcha", "alloydb", "embedding"]),
    ]
    ...
    check(results, issues, "T1_3_gotcha_storage", len(gotcha_ids) >= 2, {
    ...
    for query, check_name in recall_queries:
        ...
        check(results, issues, check_name, len(memories) > 0, {
    ...
    memories = r.get("memories", [])
    has_gotcha = any("pool" in str(m).lower() or "connection" in str(m).lower() for m in memories)
    check(results, issues, "T2_2_contextual_recall", has_gotcha, {
        "recalled": len(memories),
    ...
    note_adoption(journal,
        ...
        if has_gotcha else
        "contextual recall missed the pool exhaustion gotcha — knowledge retrieval unreliable for edge cases",
    ...
    if session2_id:
        ...
        memories = r.get("memories", [])
        check(results, issues, "T3_session_continuity", len(memories) > 0, {
        ...
        note_adoption(journal,
            ...
            if len(memories) > 0 else
            "recall returned 0 memories in new session — agent loses institutional knowledge across sessions",
            "positive" if len(memories) > 0 else "blocker")
    else:
        check(results, issues, "T3_session_continuity", False,
    ...
    check(results, issues, "T4_knowledge_coverage", len(all_memories) >= 5, {
    ...
    check(results, issues, "T5_search_memory_integration", has_both, {
    ...
    note_adoption(journal,
        ...
        if has_both else
        f"search returned {len(search_results)} results, memory returned {len(memory_results)} — incomplete knowledge integration",
