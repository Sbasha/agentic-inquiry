...
async def run(
    ...
    try:
        ...
        index_ok = entities > 0 or health in (
            "healthy",
            "unknown",
        )  # alloydb may scope differently
        check(
            results,
            issues,
            "setup_index_health",
    ...
    try:
        ...
        check(
            ...
            True,
            {
                ...
                "elapsed_s": round(t, 2),
                "top_result": results_list[0].get("file_path", "")
                if results_list
                else "EMPTY",
            },
        )
        log(
            test_id,
            f"T1.1: query='SearchService' -> {result_count} results, relevant={relevant}",
        )
    ...
    try:
        ...
        check(
            results,
            issues,
            "T1_2_reproduction_storage_facade",
            True,
            {
                "query": "StorageFacade",
                "result_count": count2,
        ...
        log(
            test_id,
            f"T1.2: query='StorageFacade' -> {count2} results, relevant={relevant2}",
        )
    except Exception as e:
        check(
            results, issues, "T1_2_reproduction_storage_facade", False, fail_msg=str(e)
        )
    ...
    try:
        ...
        fts_files = [
            ...
            for res in results_list
            if any(
                kw in res.get("file_path", "") + res.get("content", "")
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="hybrid_search search_knowledge entry point vector FTS combine",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="query normalization tokenization preprocessing before search",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="full text search FTS implementation PostgreSQL to_tsquery plainto_tsquery",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="search_knowledge calls hybrid search uses results code impact",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="onboarding entity impact lineage uses search results downstream",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="camelCase split regex pattern word boundary search token",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
    ...
    try:
        r, t = await call_tool(
            ...
            services=services,
            session_id=session_id,
            query="test search quality hybrid FTS unit test pytest",
            limit=10,
        )
        results_list = r.get("results", [])
        count = len(results_list)
