...
async def run(
    ...
    try:
        ...
        if health_ok:
            note_adoption(
                journal,
                f"Server responds in {t:.1f}s — fast enough for interactive use",
                "positive",
            )
        else:
            note_adoption(
                journal,
                f"Server slow ({t:.1f}s) or unhealthy — agent would retry or skip",
                "negative",
            )
        ...
        note_adoption(
            journal,
            f"Server unreachable: {e} — agent cannot use this tool at all",
            "blocker",
        )
        return summarize(
            test_id,
    ...
    try:
        ...
        note_adoption(
            journal,
            "Session created and retrievable — state management works",
            "positive",
        )
    except Exception as e:
        check(
    ...
    if result_count > 0:
        note_adoption(
            journal,
            f"Search returned {result_count} results in {t:.1f}s for 'session management' — grep would need exact string match",
            "positive",
        )
    else:
        note_adoption(
    ...
    has_path = bool(
        top and (top.get("file_path") or top.get("metadata", {}).get("file_path"))
    )
    check(
        results,
        issues,
    ...
    if has_path and top:
        note_adoption(
            journal,
            f"Top result has file path and score={top.get('score', '?')} — actionable for code navigation",
            "positive",
        )
    if t > 5.0:
        note_adoption(
            journal,
            f"search took {t:.1f}s — for simple keyword lookups, grep is faster and has zero setup cost",
            "neutral",
        )
    elif t > 2.0:
        note_adoption(
            journal,
            f"search took {t:.1f}s — acceptable for semantic queries but grep returns exact matches instantly",
            "neutral",
        )
    ...
    if len(memories) > 0:
        note_adoption(
            journal,
            "Memory round-trips work — agent can accumulate knowledge across queries",
            "positive",
        )
    else:
        note_adoption(
            journal,
            "Memory recall returned nothing — agent loses context between queries",
            "negative",
        )
