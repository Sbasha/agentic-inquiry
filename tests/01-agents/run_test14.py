...
OUTPUT_DIR = PROJECT_ROOT / "test_results" / "agv" / RUN_ID / "semantic_graph"
...
async def main():
    ...
    from protocols import test_14_semantic_graph as mod

    print(f"Loading config from: {ENV_PATH}")
    config = Config.load(ENV_PATH)

    print("Creating MCP services...")
    services = await create_mcp_services(config, f"agv_test14_semantic_{RUN_ID}")

    print(f"Running TEST_14 semantic graph, output: {OUTPUT_DIR}")
