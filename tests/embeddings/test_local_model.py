...
class TestModelMetadata:
    ...
    def test_metadata_creation_with_valid_data(self, valid_metadata_dict):
        ...
        assert (
            metadata.max_sequence_length == valid_metadata_dict["max_sequence_length"]
        )
        assert metadata.tokenizer_type == valid_metadata_dict["tokenizer_type"]
        assert (
            metadata.normalize_embeddings == valid_metadata_dict["normalize_embeddings"]
        )
    ...
    def test_metadata_validation_negative_max_sequence_length(
        ...
        with pytest.raises(
            ValueError, match="max_sequence_length must be a positive integer"
        ):
            ModelMetadata(**valid_metadata_dict)
    ...
    def test_metadata_validation_empty_tokenizer_type(self, valid_metadata_dict):
        ...
        with pytest.raises(
            ValueError, match="tokenizer_type must be a non-empty string"
        ):
            ModelMetadata(**valid_metadata_dict)

    def test_metadata_validation_invalid_normalize_embeddings(
        self, valid_metadata_dict
    ):
    ...
    def test_load_reads_valid_metadata(self, valid_metadata, tmp_path):
        ...
        assert (
            loaded_metadata.normalize_embeddings == valid_metadata.normalize_embeddings
        )
...
class TestModelLoader:
    ...
    def test_load_model_format_mismatch_warning(
        ...
        assert any(
            "differs from metadata format" in record.message
...
@pytest.fixture
def embedder_with_mocks(
    tmp_path, mock_model_loader, mock_onnx_session, mock_tokenizer, mock_metadata
):
    """Create LocalModelEmbedder with mocked dependencies."""
    model_path = tmp_path / "test_model"
    model_path.mkdir()

    embedder = LocalModelEmbedder(model_path=model_path, normalize=True, batch_size=32)
...
class TestLocalModelEmbedder:
    ...
    def test_initialization_with_custom_parameters(self, tmp_path):
        ...
        embedder = LocalModelEmbedder(
            model_path=model_path, ndims=768, normalize=False, batch_size=16
        )
    ...
    def test_initialization_with_workspace_relative_path(
        self, mock_temp_config, tmp_path
    ):
        ...
        embedder = LocalModelEmbedder(
            model_path="models/test_model", config=mock_temp_config
        )
    ...
    def test_initialization_with_absolute_path(self, tmp_path, mock_temp_config):
        ...
        embedder = LocalModelEmbedder(model_path=absolute_path, config=mock_temp_config)
    ...
    def test_ndims_with_explicit_ndims_parameter(self, tmp_path):
        ...
        embedder = LocalModelEmbedder(model_path=tmp_path / "model", ndims=768)
    ...
    def test_ensure_loaded_validates_dimensions(
        self, tmp_path, mock_model_loader, mock_onnx_session, mock_tokenizer
    ):
        ...
        with patch.object(ModelLoader, "__init__", return_value=None):
            with patch.object(
                ModelLoader,
                "load_model",
                return_value=(mock_onnx_session, mock_tokenizer, metadata),
            ):
                embedder._loader = ModelLoader(tmp_path)

                with pytest.raises(
                    ValueError,
                    match="Model dimensions .* do not match expected dimensions",
                ):
                    embedder._ensure_loaded()
    ...
    def test_mean_pooling(self, embedder_with_mocks):
        ...
        token_embeddings = np.random.randn(batch_size, seq_length, hidden_size).astype(
            np.float32
        )
