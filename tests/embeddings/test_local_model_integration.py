...
@pytest.fixture
def integration_embedder(
    tmp_path, mock_onnx_session_for_integration, mock_tokenizer_for_integration
):
...
class TestLocalModelEmbedderPipelineIntegration:
    ...
    async def test_embedder_with_embedding_service(self, integration_embedder):
        ...
        embeddings = await service.generate_embeddings_batch(
            texts=texts, embedder=integration_embedder, operation_name="test_embedding"
        )
    ...
    def test_embedder_with_multiple_table_configurations(self, integration_embedder):
        ...
        doc_embedder, doc_dims = service.get_embedder_configuration(
            "document_chunks", "vector"
        )
        graph_embedder, graph_dims = service.get_embedder_configuration(
...
class TestBatchSizePerformance:
    ...
    async def test_async_batch_performance(self, integration_embedder):
        ...
        results = await asyncio.gather(
            *[integration_embedder.generate_async(batch) for batch in batches]
        )
        elapsed_time = time.time() - start_time
