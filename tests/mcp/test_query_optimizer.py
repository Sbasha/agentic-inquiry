...
def test_start_and_end_query():
    ...
    # End query
    optimizer.end_query(query_id=query_id, rows_returned=15, status="success")
...
def test_query_duration_tracking():
    ...
    query_id = optimizer.start_query(query_type="search", table_name="document_chunks")
...
def test_query_statistics_update():
    ...
    for i in range(5):
        query_id = optimizer.start_query(
            query_type="search", table_name="document_chunks"
        )
        optimizer.end_query(
            query_id=query_id,
            rows_returned=10 + i,
    ...
    # Check statistics
    stats = optimizer.get_query_stats(query_type="search", table_name="document_chunks")
...
def test_get_query_stats_filtered():
    ...
    for table in ["document_chunks", "graph_entities"]:
        for i in range(3):
            query_id = optimizer.start_query(query_type="search", table_name=table)
            optimizer.end_query(query_id=query_id, rows_returned=10)
...
def test_index_recommendations():
    ...
    filter_recommendations = [
        r for r in recommendations if "column" in r and r["table"] == "document_chunks"
    ]
...
def test_query_profile_complete():
    ...
    profile = QueryProfile(
        query_type="search", table_name="document_chunks", start_time=time.time()
    )
...
def test_query_statistics_to_dict():
    ...
    stats = QueryStatistics(query_type="search", table_name="document_chunks")
...
def test_success_rate_calculation():
    ...
    for i in range(10):
        ...
        optimizer.end_query(
            query_id=query_id, rows_returned=10, status="success" if i < 8 else "error"
        )
