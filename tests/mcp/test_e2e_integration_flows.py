...
async def test_empty_search_flow_complete(integration_services, tmp_path):
    ...
    assert "0 chunks" in index_state["message"] or "no indexed content" in index_state["message"].lower()
...
async def test_limit_parameter_variations(integration_services):
    ...
    result_default = await get_server_info(
        services=integration_services
    )
    ...
    result_limited = await get_server_info(
        services=integration_services,
        limit=2
    )
    ...
    result_large = await get_server_info(
        services=integration_services,
        limit=100
    )
    ...
    result_one = await get_server_info(
        services=integration_services,
        limit=1
    )
...
async def test_sort_by_parameter_variations(integration_services):
    ...
    result_name = await get_server_info(
        services=integration_services,
        sort_by="name"
    )
...
async def test_all_error_messages_include_actionable_suggestions(
    ...
    assert "different" in suggestions_text2.lower() or "synonyms" in suggestions_text2.lower()
