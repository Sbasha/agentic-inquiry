...
@pytest.mark.asyncio
async def test_single_file_indexing_creates_relationships(
...
@pytest.mark.asyncio
async def test_directory_indexing_creates_relationships(
    mcp_services, tmp_path, monkeypatch
):
...
''')
    ...
    assert result["status"] == "started", (
        f"Expected started status for directory, got: {result}"
    )
    assert "operation_id" in result, (
        "Expected operation_id in directory indexing result"
    )
