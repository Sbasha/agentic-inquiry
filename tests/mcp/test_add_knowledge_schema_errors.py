...
def mcp_services():
    ...
    async def mock_add_event(session_id, event_type, data):
        ...
        session_manager._events.append({"event_type": event_type, "data": data})
...
@pytest.mark.asyncio
async def test_add_knowledge_file_schema_validation_error(
    mcp_services, tmp_path, monkeypatch
):
...
@pytest.mark.asyncio
async def test_add_knowledge_directory_schema_validation_error(
    mcp_services, tmp_path, monkeypatch
):
    ...
    with patch(PIPELINE_PATCH_PATH) as mock_create_pipeline:
        ...
        async def process_side_effect(doc, flush_relationships=True):
            ...
            if call_count == 1:
                raise SchemaValidationError(
                    table_name="document_chunks", missing_fields=["required_field"]
                )
        ...
        with patch(PARSER_CHAIN_PATCH_PATH) as mock_create_chain:
            ...
            # Mock ignore handler
            with patch(
                "agentic_inquiry.mcp.tools.knowledge.get_ignore_handler"
            ) as mock_ignore:
                ...
                while loop.time() - start_time < timeout:
                    ...
                    for event in events:
                        if event.get("event_type") == "indexing_completed":
                            if (
                                event.get("data", {}).get("operation_id")
                                == result["operation_id"]
                            ):
                                completed = True
                                final_event = event
...
@pytest.mark.asyncio
async def test_add_knowledge_schema_error_includes_hints(
    mcp_services, tmp_path, monkeypatch
):
