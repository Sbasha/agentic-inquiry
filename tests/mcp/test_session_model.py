...
class TestSessionToDbRecord:
    ...
    def test_to_db_record_includes_ttl_hours(self):
        ...
        session = Session(session_id="test_id", project_id="test_project", ttl_hours=72)
    ...
    def test_to_db_record_is_expired_uses_property(self):
        ...
        new_session = Session(
            session_id="test_id", project_id="test_project", ttl_hours=48
        )
...
class TestSessionStatusField:
    ...
    def test_session_has_status_field(self):
        ...
        session = Session(session_id="test_id", project_id="test_project")
    ...
    def test_status_can_be_set(self):
        ...
        session_active = Session(
            session_id="test_id", project_id="test_project", status="active"
        )
        assert session_active.status == "active"

        # Test expired
        session_expired = Session(
            session_id="test_id2", project_id="test_project", status="expired"
        )
        assert session_expired.status == "expired"

        # Test closed
        session_closed = Session(
            session_id="test_id3", project_id="test_project", status="closed"
        )
...
class TestSessionRoundTrip:
    ...
    def test_round_trip_preserves_status(self):
        ...
        original = Session(
            session_id="test_id", project_id="test_project", status="expired"
        )
