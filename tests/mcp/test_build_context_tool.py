...
from agentic_inquiry.mcp.tools.context import (
    build_context,
...
def mcp_services():
    ...
    services["session_manager"].get_session = AsyncMock(
        return_value=MagicMock(project_id="test_project")
    )
...
async def test_build_context_invalid_session(mcp_services):
    ...
    with patch(
        "agentic_inquiry.mcp.utils.errors.MCPErrorHandler"
    ) as mock_error_handler:
        mock_error_handler.handle = AsyncMock(
            return_value={"error": "Session not found"}
        )
...
def test_generate_empty_context_suggestions_no_memories():
    ...
    suggestions = _generate_empty_context_suggestions(
        query="test query", has_memories=False
    )
...
def test_generate_empty_context_suggestions_with_memories():
    ...
    suggestions = _generate_empty_context_suggestions(
        query="test query", has_memories=True
    )
    ...
    memory_suggestions = [
        s for s in suggestions if "save" in s.lower() and "memory" in s.lower()
    ]
