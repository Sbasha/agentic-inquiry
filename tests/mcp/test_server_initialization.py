...
class TestMCPServerInitialization:
    ...
    async def test_initialize_with_project_id(self, mock_config, mcp_services):
        ...
        with (
            ...
            mock_fastmcp.assert_called_once_with(
                name="Test MCP Server", version="1.0.0", instructions="Test server"
            )
    ...
    @pytest.mark.asyncio
    async def test_initialize_with_direct_tools_enabled(
...
class TestServerLifecycle:
    ...
    async def test_shutdown_cancels_maintenance_task(self, mock_config, mcp_services):
        ...
        with (
            ...
            patch(
                "agentic_inquiry.mcp.server.create_mcp_services",
                return_value=services_with_task,
            ),
        ):
            server = MCPServer(config=mock_config, project_id="test_project")
            await server.initialize()
            ...
            assert task.cancelled(), (
                "shutdown must cancel (not let it finish naturally)"
            )
            assert len(server.services) == 0
