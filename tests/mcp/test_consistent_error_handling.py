...
async def test_memory_recall_error_format(mcp_services):
    ...
    result = await recall_memories(
        services=mcp_services, session_id="invalid-session", query="test query"
    )
...
async def test_analysis_understand_entity_error_format(mcp_services):
    ...
    result = await understand_entity(
        services=mcp_services, session_id="invalid-session", entity="TestEntity"
    )
...
async def test_analysis_find_patterns_error_format(mcp_services):
    ...
    result = await find_patterns(
        services=mcp_services, session_id="invalid-session", pattern_type="auto"
    )
...
async def test_error_suggestions_are_actionable():
    ...
    result = await understand_entity(
        services=mcp_services, session_id="invalid-session", entity="MyClass"
    )
    ...
    assert any(
        "create" in s.lower() and "session" in s.lower() for s in result["suggestions"]
    )
...
async def test_all_tools_use_same_error_format():
    ...
    tools_to_test = [
        ...
        (
            add_knowledge,
            {"session_id": "test", "content_type": "file", "source": "test.py"},
        ),
    ]
    ...
    for structure in error_structures[1:]:
        assert structure == first_structure, (
            "All tools must return errors in the same format"
        )
...
async def test_error_help_text_is_meaningful():
    ...
    assert any(
        keyword in help_text.lower() for keyword in ["create", "session", "use", "try"]
    )
...
async def test_error_message_includes_context_values():
    ...
    result = await recall_memories(
        services=mcp_services, session_id=session_id, query="test query"
    )
