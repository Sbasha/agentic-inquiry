...
def mcp_services():
    ...
    session_manager.get_project_statistics = AsyncMock(
        return_value=ProjectStatistics(
...
class TestGetServerInfo:
    ...
    @pytest.mark.asyncio
    async def test_project_discovery_from_database(
        self, mcp_services, sample_chunks, sample_project_stats
    ):
    ...
    @pytest.mark.asyncio
    async def test_statistics_gathering_for_projects(
        self, mcp_services, sample_chunks, sample_project_stats
    ):
    ...
    async def test_limit_parameter_default(self, mcp_services, sample_chunks):
        ...
        mcp_services[
            "session_manager"
        ].get_project_statistics.return_value = ProjectStatistics(
    ...
    @pytest.mark.asyncio
    async def test_sort_by_last_indexed(
        self, mcp_services, sample_chunks, sample_project_stats
    ):
        ...
        mcp_services[
            "session_manager"
        ].get_project_statistics.return_value = sample_project_stats
    ...
    async def test_statistics_failure_handling(self, mcp_services, sample_chunks):
        ...
        # Setup
        mcp_services["storage"].advanced_filter.return_value = sample_chunks
        mcp_services["session_manager"].get_project_statistics.side_effect = Exception(
            "Stats error"
        )
...
class TestGetProjectInfo:
    ...
    async def test_get_project_info_statistics_accuracy(self, mcp_services):
        ...
        mcp_services["storage"].count_records = AsyncMock(
            side_effect=lambda table_name, **kwargs: {
                "document_chunks": 150,
                "graph_entities": 75,
            }.get(table_name, 0)
        )
        ...
        mcp_services["storage"].count_relationships_by_type = AsyncMock(
            return_value={
                "calls": 100,
                "imports": 50,
                "defines": 50,
...
class TestRunMaintenance:
    ...
    def maintenance_services(self, mcp_services):
        ...
        mcp_services["storage"].run_maintenance = AsyncMock(
            return_value={
                "compaction": {
                    "document_chunks": {
                        "status": "success",
    ...
    async def test_run_maintenance_success(self, maintenance_services):
        ...
        result = await run_maintenance(
            maintenance_services, session_id="test-session-123", cleanup_hours=1.0
        )
    ...
    async def test_run_maintenance_invalid_session(self, maintenance_services):
        ...
        maintenance_services["session_manager"].validate_session = AsyncMock(
            return_value=False
        )

        result = await run_maintenance(
            maintenance_services, session_id="invalid-session", cleanup_hours=1.0
        )
    ...
    async def test_run_maintenance_custom_cleanup_hours(self, maintenance_services):
        ...
        result = await run_maintenance(
            maintenance_services, session_id="test-session-123", cleanup_hours=24.0
        )
