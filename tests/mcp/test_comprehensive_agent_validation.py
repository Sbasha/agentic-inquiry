...
async def test_services(mock_config, tmp_path, monkeypatch):
    ...
    services = await create_mcp_services(
        config=mock_config, project_id="test_validation_project"
    )
...
""")
    ...
    search_result = await search_knowledge(
        services=test_services, session_id=session_id, query="calculate sum", limit=10
    )
...
""")
    ...
    # Verify content is searchable
    search_result = await search_knowledge(
        services=test_services, session_id=session_id, query="UserManager", limit=10
    )
...
""")
    ...
    # Verify content is searchable
    search_result = await search_knowledge(
        services=test_services, session_id=session_id, query="installation", limit=10
    )
...
""")
    ...
    assert (
        final_event["data"]["items_processed"] >= 1
    )  # At least 1 file (schema issues may prevent all files from indexing)
    assert "files" in final_event["data"]

    # Verify all files are searchable
    # Search for Python content
    py_search = await search_knowledge(
        services=test_services, session_id=session_id, query="format_data", limit=10
    )
    ...
    md_search = await search_knowledge(
        services=test_services, session_id=session_id, query="API key", limit=10
    )
...
""")
    ...
    result = await search_knowledge(
        services=test_services, session_id=session_id, query="How can I help?", limit=10
    )
...
""")
    ...
    problematic_queries = [
        ...
        {
            "query": "Find text with \\ backslash",
            "expected": "Find text with backslash",
        },
    ]
    ...
    for query_dict in problematic_queries:
        ...
        result = await search_knowledge(
            services=test_services, session_id=session_id, query=query_str, limit=10
        )
        ...
        if "error" in result:
            ...
            if isinstance(error_value, str):
                assert "syntax" not in error_value.lower(), (
                    f"Query '{query_str}' caused syntax error: {error_value}"
                )
            elif isinstance(error_value, dict):
                # Check if it's a syntax error in the error dict
                error_msg = error_value.get("message", "")
                if isinstance(error_msg, str):
                    assert "syntax" not in error_msg.lower(), (
                        f"Query '{query_str}' caused syntax error: {error_msg}"
                    )
        else:
...
""")
    ...
    search_result = await search_knowledge(
        services=test_services, session_id=session_id, query="legacy function", limit=10
    )
...
""")
    ...
    search_result = await search_knowledge(
        services=test_services, session_id=session_id, query="application run", limit=10
    )
    assert len(search_result["results"]) > 0
