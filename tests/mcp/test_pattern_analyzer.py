...
async def test_find_patterns_successful_clustering(pattern_analyzer, mock_db_manager):
    ...
    for i in range(5):
        mock_results.append({
            "id": f"cluster1_{i}",
            "vector": [1.0 + np.random.normal(0, 0.1), 0.0 + np.random.normal(0, 0.1)],
    ...
    for i in range(5):
        mock_results.append({
            "id": f"cluster2_{i}",
            "vector": [0.0 + np.random.normal(0, 0.1), 1.0 + np.random.normal(0, 0.1)],
...
def test_infer_pattern_name_antipattern(pattern_analyzer):
    ...
    assert "God" in pattern_name or "Smell" in pattern_name or "antipattern" in pattern_name.lower()
...
@pytest.mark.asyncio
async def test_find_patterns_examples_per_cluster_limit(pattern_analyzer, mock_search_service):
