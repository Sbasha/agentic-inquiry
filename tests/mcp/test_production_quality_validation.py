...
class TestStatisticsAccuracy:
    ...
    @pytest.mark.asyncio
    async def test_statistics_reflect_indexed_content(
        ...
        mock_db_manager.count_relationships_by_type = AsyncMock(
            return_value={"calls": 200}
        )
    ...
    @pytest.mark.asyncio
    async def test_statistics_consistency_after_multiple_indexes(
        self, mock_db_manager, mock_session
    ):
        ...
        assert (
            result2["statistics"]["chunks_indexed"]
            > result1["statistics"]["chunks_indexed"]
        )
...
class TestKeywordExtraction:
    ...
    async def test_extract_top_keywords_from_indexed_content(
        ...
        keywords = await keyword_extractor.extract_top_keywords(
            mock_db_manager, "test-project", limit=10
        )
    ...
    async def test_keyword_extraction_handles_empty_content(
        ...
        # Execute
        keywords = await keyword_extractor.extract_top_keywords(
            mock_db_manager, "empty-project", limit=10
        )
    ...
    async def test_overview_content_detection(self):
        ...
        for path in overview_paths:
            assert any(
                ...
                for pattern in ["readme", "docs/", "architecture"]
            )

        for path in implementation_paths:
            assert not any(
                pattern in path.lower() for pattern in ["readme", "architecture"]
            )
            assert any(
                pattern in path.lower() for pattern in ["src/", "lib/", "tests/"]
            )
