...
async def test_services(tmp_path):
    ...
    # Create services
    services = await create_mcp_services(config=config, project_id="test_entity_ops")
...
async def test_session(test_services):
    ...
    session_info = await session_manager.create_session(
        project_id="test_entity_ops", description="Test session for entity operations"
    )
...
'''
    ...
    for file_path in [
        search_service_file,
        indexing_pipeline_file,
        context_builder_file,
    ]:
...
@pytest.mark.asyncio
async def test_understand_entity_after_schema_changes(
    ...
    result = await understand_entity(
        services=test_services, session_id=test_session, entity="SearchService"
    )

    # Verify no "file not found" errors
    assert (
        "error" not in result
        or "file not found" not in str(result.get("error", "")).lower()
...
async def test_understand_entity_method_after_schema_changes(
    ...
    result = await understand_entity(
        services=test_services, session_id=test_session, entity="search"
    )
    ...
    if "entity" in result:
        assert result["entity"]["entity_type"] in [
            "function",
            "method",
            "function",
            "method",
...
@pytest.mark.asyncio
async def test_analyze_impact_after_schema_changes(
    test_services, test_session, indexed_test_files
):
...
@pytest.mark.asyncio
async def test_entity_operations_database_integrity(
    test_services, test_session, indexed_test_files
):
    ...
    assert "document_chunks" in tables, (
        f"document_chunks table not found. Tables: {tables}"
    )
    assert "graph_entities" in tables, (
        f"graph_entities table not found. Tables: {tables}"
    )
...
@pytest.mark.asyncio
async def test_entity_operations_no_orphaned_references(
    test_services, test_session, indexed_test_files
):
    ...
    relationships = await db_manager.get_graph_relationships(
...
@pytest.mark.asyncio
async def test_entity_operations_file_paths_valid(
    test_services, test_session, indexed_test_files
):
    ...
    for entity in entities:
        ...
        assert str(file_path), (
            f"Entity {entity.name} has invalid file_path: {entity.file_path}"
        )


@pytest.mark.asyncio
async def test_multiple_entity_operations_success_rate(
    test_services, test_session, indexed_test_files
):
    ...
    for entity_name in entities_to_test:
        ...
        understand_result = await understand_entity(
            services=test_services, session_id=test_session, entity=entity_name
        )

        # Count as successful if entity was found and no file errors
        if (
            "entity" in understand_result
            and "file not found" not in str(understand_result).lower()
        ):
        ...
        if (
            "impact_radius" in impact_result
            and "file not found" not in str(impact_result).lower()
        ):
            impact_successful += 1
...
@pytest.mark.asyncio
async def test_entity_operations_after_directory_indexing(
    test_services, test_session, tmp_path, monkeypatch
):
...
""")
    ...
    assert completion["items_processed"] == 2, (
        f"Expected 2 files, got: {completion['items_processed']}"
    )

    # Test entity operations work
    understand_result = await understand_entity(
        services=test_services, session_id=test_session, entity="ClassA"
    )

    # Verify no file errors
    assert (
        "error" not in understand_result
        or "file not found" not in str(understand_result.get("error", "")).lower()
    )

    # Test impact analysis
    impact_result = await analyze_impact(
        services=test_services, session_id=test_session, entity="ClassA", max_depth=2
    )

    # Verify no file errors
    assert (
        "error" not in impact_result
        or "file not found" not in str(impact_result.get("error", "")).lower()
    )
