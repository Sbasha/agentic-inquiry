...
async def test_analyze_impact_returns_impact_radius(indexed_services, test_session):
    ...
    assert result["impact_radius"] > 0, "Impact radius should be > 0 for entity with relationships"
...
async def test_analyze_impact_returns_affected_entities(indexed_services, test_session):
    ...
    assert len(result["affected_entities"]) > 0, "Should have at least one affected entity"
...
@pytest.mark.asyncio
async def test_analyze_impact_returns_relationship_types(indexed_services, test_session):
