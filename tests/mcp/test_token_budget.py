...
class TestBudgetAllocation:
    ...
    def test_valid_allocation(self) -> None:
        ...
        allocation = BudgetAllocation(
            code_budget=7000, docs_budget=3000, total_budget=10000, focus="code"
        )
    ...
    def test_invalid_allocation_exceeds_total(self) -> None:
        ...
        with pytest.raises(ValueError, match="exceeds total_budget"):
            BudgetAllocation(
                code_budget=7000, docs_budget=4000, total_budget=10000, focus="code"
            )
...
class TestTokenBudgetManager:
    ...
    def test_initialization(
        self, budget_manager: TokenBudgetManager, config: Config
    ) -> None:
        """Test manager initialization."""
        assert budget_manager.config == config
        assert budget_manager._encoding is not None

    def test_allocate_budget_with_default(
    ...
    def test_allocate_budget_code_focus(
        self, budget_manager: TokenBudgetManager
    ) -> None:
    ...
    def test_allocate_budget_docs_focus(
        self, budget_manager: TokenBudgetManager
    ) -> None:
    ...
    def test_allocate_budget_balanced_focus(
        self, budget_manager: TokenBudgetManager
    ) -> None:
        """Test budget allocation with balanced focus."""
        allocation = budget_manager.allocate_budget(
            total_budget=10000, focus="balanced"
        )
    ...
    def test_allocate_budget_below_minimum(
        self, budget_manager: TokenBudgetManager
    ) -> None:
    ...
    def test_allocate_budget_above_maximum(
        self, budget_manager: TokenBudgetManager
    ) -> None:
    ...
    def test_allocate_budget_invalid_focus(
        self, budget_manager: TokenBudgetManager
    ) -> None:
    ...
    def test_allocate_budget_different_sizes(
        self, budget_manager: TokenBudgetManager
    ) -> None:
...
"""
    ...
    def test_fits_budget_exceeds_limit(
        self, budget_manager: TokenBudgetManager
    ) -> None:
    ...
    def test_fits_budget_with_buffer(self, budget_manager: TokenBudgetManager) -> None:
        ...
        fits_with_buffer = budget_manager.fits_budget(
            text, budget=100, buffer_ratio=0.1
        )

        # With 50% buffer, effective budget is 50
        fits_with_large_buffer = budget_manager.fits_budget(
            text, budget=100, buffer_ratio=0.5
        )
    ...
    def test_fits_budget_negative_budget(
        self, budget_manager: TokenBudgetManager
    ) -> None:
    ...
    def test_estimate_batch_tokens_empty_list(
        self, budget_manager: TokenBudgetManager
    ) -> None:
    ...
    def test_estimate_batch_tokens_single_text(
        self, budget_manager: TokenBudgetManager
    ) -> None:
    ...
    def test_estimate_batch_tokens_multiple_texts(
        self, budget_manager: TokenBudgetManager
    ) -> None:
    ...
    def test_estimate_batch_tokens_statistics(
        self, budget_manager: TokenBudgetManager
    ) -> None:
...
"""
        ...
        assert (
            total_tokens <= allocation.total_budget * 1.5
        )  # Allow 50% overflow for test
