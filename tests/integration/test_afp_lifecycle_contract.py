...
class Harness:
    ...
    def hook(
        ...
        *,
        ...
        data = (
            raw
            if raw is not None
            else json.dumps(payload or self.payload()).encode("utf-8")
        )
        completed = self.run(
            "integration",
            "hook",
    ...
    def integration_status(self) -> dict[str, Any]:
        completed = self.run(
            "integration", "status", "--json", "--project-root", str(self.project)
        )
    ...
    def records(self) -> Path:
        project_id = json.loads(
            (self.project / ".agentic-inquiry" / "integration.json").read_text(
                encoding="utf-8"
            )
        )["project_id"]
        return (
            self.home / ".agentic-inquiry" / "projects" / project_id / "records.sqlite3"
        )
...
def home(tmp_path: Path) -> Path:
    ...
    (path / ".agentic-inquiry").mkdir(
        mode=0o700
    )  # the ledger home must be private (AC10)
    return path
...
def _observations(
    text: str = "Auth uses JWT with refresh tokens issued by AuthService.",
) -> list[dict]:
...
def test_capabilities_reports_the_contract(harness: Harness) -> None:
    ...
    for client in CLIENTS:
        assert (
            report["clients"][client]["activation"]
            == "explicit per-project and client owner"
        )
...
def test_unsupported_event_is_reported_before_validation(
    harness: Harness, event: str
) -> None:
...
def test_malformed_hook_invocations_answer_unsupported_on_stdout(
    enabled: Harness,
) -> None:
    for argv, code in (
        ...
        completed = enabled.run(
            *argv, stdin=json.dumps(enabled.payload()).encode("utf-8")
        )
        assert completed.returncode == 1 and completed.stderr == b"", (
            argv,
            completed.stderr,
        )
        body = json.loads(completed.stdout)
        RESPONSE.validate(body)
        assert body["status"] == "unsupported" and body["errors"][0]["code"] == code, (
            argv
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"observations": [{"content": "x" * 8193}], "event_id": "e"},
            "invalid_payload",
        ),
        ...
        (
            {"observations": [{"summary": "no content"}], "event_id": "e"},
            "invalid_payload",
        ),
        (
            {
                "observations": [{"content": "x", "summary": "s" * 1025}],
                "event_id": "e",
            },
            "invalid_payload",
        ),
        (
            {"observations": [{"content": "x", "importance": 2}], "event_id": "e"},
            "invalid_payload",
        ),
        (
            {
        ...
        (
            {"observations": [{"content": "x"}], "event_id": "e" * 301},
            "invalid_payload",
        ),
        (
            {
                "observations": [{"content": "x", "metadata": {"event_key": "forged"}}],
                "event_id": "e",
            },
            "invalid_payload",
        ),
        (
            {
                "observations": [{"content": "x", "metadata": {"Bad-Key": "v"}}],
                "event_id": "e",
            },
            "invalid_payload",
        ),
        (
            {
                "observations": [
                    {"content": "x", "metadata": {f"k{i}": "v" for i in range(17)}}
                ],
                "event_id": "e",
            },
            "invalid_payload",
        ...
        (
            {
                "observations": [{"content": "x", "metadata": {"k": [{"nested": 1}]}}],
                "event_id": "e",
            },
            "invalid_payload",
        ),
        (
            {
                "observations": [{"content": "x", "metadata": {"k": "v" * 257}}],
                "event_id": "e",
            },
            "invalid_payload",
        ),
...
def test_unadmitted_fields_are_refused_even_when_unregistered(harness: Harness) -> None:
    completed, body = harness.hook(
        "SessionStart", harness.payload(transcript_path="/t")
    )
...
def test_content_bounds_are_not_checked_for_an_unregistered_project(
    harness: Harness,
) -> None:
    completed, body = harness.hook(
        "UserPromptSubmit", harness.payload(query="q" * 16385)
    )
    assert completed.returncode == 0 and body["status"] == "inert"


def test_non_canonical_project_root_is_refused(
    enabled: Harness, tmp_path: Path
) -> None:
    alias = tmp_path / "alias"
    alias.symlink_to(enabled.project, target_is_directory=True)
    completed, body = enabled.hook(
        "SessionStart", enabled.payload(project_root=str(alias))
    )
    assert completed.returncode == 2
...
def test_artifact_escape_is_refused_and_writes_nothing(
    enabled: Harness, tmp_path: Path
) -> None:
    ...
    cases = [
        ("../outside/secret.txt", None),
        (str(enabled.project / "linked" / "secret.txt"), "linked/secret.txt"),
    ]
    for index, (artifact, expected_path) in enumerate(cases):
        completed, body = enabled.hook(
            "PostToolUse",
            enabled.payload(artifacts=[artifact], event_id=f"tool-escape-{index}"),
        )
        assert completed.returncode == 2, body
        assert body["errors"][0]["code"] == "artifact_escape"
        assert body["errors"][0].get("path") == expected_path, body["errors"][0]
        assert str(enabled.project) not in json.dumps(body) and str(
            tmp_path
...
def test_ignored_artifacts_are_dropped_with_an_advisory_error(enabled: Harness) -> None:
    ...
    (enabled.project / ".git" / "HEAD").write_text(
        "ref: refs/heads/main\n", encoding="utf-8"
    )
    os.symlink(
        ".git", enabled.project / "docs"
    )  # in-project symlink: resolved form is matched
    (enabled.project / "CredentialsService.java").write_text(
    ...
    ignored = sorted(
        e["path"] for e in body["errors"] if e["code"] == "artifact_ignored"
    )
    assert ignored == [
        ".env",
        ".git/HEAD",
        ...
        "big.log",
        "config/.env",
        "packages/api/.ssh/id_rsa",
        "secrets/credentials.json",
        "server.pem",
    ]
    (enabled.project / "adir").mkdir()
    os.mkfifo(
        enabled.project / "pipe"
    )  # never opened for reading, so the hook never blocks
    started = time.perf_counter()
    completed, body = enabled.hook(
        "PostToolUse",
        enabled.payload(
            artifacts=[
                str(enabled.project / "adir"),
                str(enabled.project / "gone.md"),
                str(enabled.project / "pipe"),
            ]
        ),  # no event_id
    )
    ...
    assert completed.returncode == 1 and body["status"] == "partial", (
        body
    )  # no mutation; the earlier row is still pending
    assert sorted(e["path"] for e in body["errors"]) == ["adir", "gone.md", "pipe"]
    assert (
        all(e["code"] == "artifact_ignored" for e in body["errors"])
        and body["receipts"] == []
    )
    if os.geteuid() != 0:
        ...
        try:
            _, body = enabled.hook(
                "PostToolUse",
                enabled.payload(artifacts=[str(sealed)], event_id="tool-sealed"),
            )
            assert [e["code"] for e in body["errors"]] == ["artifact_ignored"] and body[
                "status"
            ] != "error"
        finally:
            sealed.chmod(0o600)
    completed, body = enabled.hook(
        "PostToolUse",
        enabled.payload(
            artifacts=[str(enabled.project / ".env")], event_id="tool-only-ignored"
        ),
    )
    assert (
        completed.returncode == 1 and body["status"] == "partial"
    )  # earlier row still pending
    assert body["receipts"] == [] and [e["code"] for e in body["errors"]] == [
        "artifact_ignored"
    ]
...
def test_error_list_is_capped_in_gate_order(enabled: Harness) -> None:
    ...
    completed, body = enabled.hook(
        "PostToolUse",
        enabled.payload(
            artifacts=[*ignored, str(enabled.project / "README.md")],
            event_id="tool-cap",
        ),
    )
    assert completed.returncode == 1 and body["status"] == "partial", body
    ...
    assert [e["path"] for e in body["errors"]] == [
        f"keys/host-{i:02d}.pem" for i in range(16)
    ]
...
def test_missing_ledger_is_inert_with_a_diagnostic(enabled: Harness) -> None:
    ...
    completed, body = enabled.hook("SessionStart")
    assert (
        body["errors"][0]["notify"] is True
    )  # no notification table exists to remember it
    assert not records.exists()
...
@pytest.mark.parametrize(
    ("field", "code"),
    [("session_id", "session_id_required"), ("event_id", "invalid_payload")],
)
def test_identity_fields_must_be_printable_ascii(
    enabled: Harness, field: str, code: str
) -> None:
    ...
    assert completed.returncode == 2
    assert body["errors"][0]["code"] == code
    assert enabled.integration_status()["events"] == {
        "pending": 0,
        "committed": 0,
        "failed": 0,
        "exhausted": 0,
...
def test_enable_writes_identity_marker_ledger_and_gitignore_hint(
    onboarded: Harness,
) -> None:
    completed = onboarded.enable()
    assert completed.returncode == 0, completed.stdout + completed.stderr
    ...
    for directory in (
        onboarded.records().parent,
        onboarded.records().parent.parent,
        onboarded.home / ".agentic-inquiry",
    ):
...
def test_enable_refuses_a_symlinked_marker(onboarded: Harness, tmp_path: Path) -> None:
    victim = (
        onboarded.project / ".git" / "config"
    )  # an in-project target: containment alone would pass it
    ...
    marker.unlink()
    (onboarded.project / ".agentic-inquiry" / "integration.json.tmp").symlink_to(
        Path("..") / ".git" / "config"
    )
    completed = onboarded.enable(
        "--json"
    )  # a predictable temporary name is never opened for writing
    assert completed.returncode == 0, completed.stdout + completed.stderr
    assert (
    ...
    completed = onboarded.run(
        "integration", "status", "--project-root", str(onboarded.project), "--json"
    )
    assert completed.returncode == 0, completed.stdout + completed.stderr
...
def test_ledger_home_with_a_write_bit_is_refused_with_a_remedy(
    enabled: Harness,
) -> None:
    projects = enabled.records().parent.parent
    projects.chmod(0o770)  # another account could have written here: never healed
    completed = enabled.run(
        "integration", "status", "--project-root", str(enabled.project), "--json"
    )
    assert completed.returncode == 1, completed.stdout + completed.stderr
    ...
    assert completed.returncode == 1 and body["status"] == "partial"
    assert (
        body["errors"][0]["code"] == "ledger_unavailable"
        and body["errors"][0]["notify"] is True
    )
    assert str(projects) not in json.dumps(body)  # a hook never names an absolute path
    _, again = enabled.hook("SessionStart")
    assert (
        again["errors"][0]["notify"] is False
    )  # the database itself is fine, so once per session
    projects.chmod(0o700)
    enabled.records().chmod(
        0o644
    )  # a readable ledger is refused by a hook and repaired by the CLI
    _, loose = enabled.hook("SessionStart")
    assert (
        loose["status"] == "partial"
        and loose["errors"][0]["code"] == "ledger_unavailable"
    )
    _, loose_again = enabled.hook("SessionStart")
    assert (
        loose["errors"][0]["notify"] is True
        and loose_again["errors"][0]["notify"] is True
    )  # never written into a condemned file
    assert (
        enabled.run(
            "integration", "status", "--project-root", str(enabled.project), "--json"
        ).returncode
    ...
    refused = json.loads(completed.stdout)["errors"][0]
    assert (
        refused["code"] == "project_files_invalid"
        and "records.sqlite3" in refused["message"]
    )
...
def test_unregistered_clone_on_a_hostile_ledger_home_is_not_inert(
    onboarded: Harness,
) -> None:
    ...
    assert completed.returncode == 1 and body["status"] == "partial", (
        body
    )  # never plain inert on a hostile home
    assert body["errors"][0]["code"] == "ledger_unavailable"


def test_enable_refuses_a_ledger_home_that_is_not_a_directory(
    onboarded: Harness,
) -> None:
    (onboarded.home / ".agentic-inquiry" / "projects").write_text(
...
def test_purge_refuses_while_a_committer_holds_the_lock(enabled: Harness) -> None:
    enabled.hook(
        "Stop",
        enabled.payload(
            observations=_observations("Held: rotate the key."), event_id="stop-held"
        ),
    )
    ...
    try:
        ...
        assert (
            refused["ok"] is False
            and refused["errors"][0]["code"] == "environment_busy"
        )
        assert (
            "reconcile" in refused["errors"][0]["message"]
            and "--wait" in refused["errors"][0]["message"]
        )
        assert enabled.integration_status()["events"] == {
            "pending": 1,
            "committed": 0,
        ...
        reconcile = enabled.run(
            "integration", "reconcile", "--project-root", str(enabled.project), "--json"
        )
        assert (
            reconcile.returncode == 0
        )  # a shared holder does not block another committer
    ...
    assert completed.returncode == 0, completed.stdout + completed.stderr
    assert (
        json.loads(completed.stdout)["events"] == 1
        and enabled.integration_status()["events"]["purged"] == 0
    )
...
def test_enable_refuses_while_the_standalone_plugin_is_enabled(
    onboarded: Harness,
) -> None:
    settings = onboarded.project / ".claude" / "settings.json"
    settings.parent.mkdir()
    settings.write_text(
        json.dumps({"enabledPlugins": {"ai@agentic-inquiry": True}}), encoding="utf-8"
    )
    completed = onboarded.enable("--json")
    assert completed.returncode == 1
    assert (
        json.loads(completed.stdout)["errors"][0]["code"] == "standalone_plugin_enabled"
    )
    assert not (onboarded.project / ".agentic-inquiry" / "integration.json").exists()
...
def test_disable_freezes_queued_rows_until_enable_or_purge(enabled: Harness) -> None:
    enabled.hook(
        "Stop", enabled.payload(observations=_observations(), event_id="stop-frozen")
    )
...
def test_owner_is_immutable_while_rows_exist(enabled: Harness) -> None:
    enabled.hook(
        "Stop", enabled.payload(observations=_observations(), event_id="stop-owner")
    )
    assert enabled.disable().returncode == 0
    refused = enabled.enable("--json", owner="standalone")
    assert (
        refused.returncode == 1
        and json.loads(refused.stdout)["errors"][0]["code"] == "owner_conflict"
    )
...
def test_owner_changes_after_disable_and_purge(enabled: Harness) -> None:
    enabled.hook(
        "Stop",
        enabled.payload(observations=_observations(), event_id="stop-owner-change"),
    )
    assert enabled.disable().returncode == 0
    purge = enabled.run(
        "integration",
    ...
    )  # tombstones are not event rows
    binding = enabled.integration_status()["bindings"][0]
    assert binding["owner"] == "standalone" and binding["enabled"] is True
    completed, body = enabled.hook(
        "Stop", enabled.payload(observations=_observations(), event_id="stop-new-owner")
    )
    assert (
        completed.returncode == 2 and body["errors"][0]["code"] == "owner_conflict"
    )  # the afp hook is now the stranger
...
def test_owner_exclusivity_and_disable(enabled: Harness) -> None:
    ...
    assert enabled.disable().returncode == 0
    marker = json.loads(
        (enabled.project / ".agentic-inquiry" / "integration.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_enable_refuses_an_unreadable_settings_file(
...
def test_project_identity_is_bound_to_one_root(
    ...
    assert completed.returncode == 1
    assert (
        json.loads(completed.stdout)["errors"][0]["code"] == "project_identity_invalid"
    )
    ...
    completed, body = enabled.hook(
        "SessionStart", enabled.payload(project_root=str(other))
    )
    assert completed.returncode == 2
    assert body["errors"][0]["code"] == "project_identity_invalid"
    assert str(tmp_path) not in json.dumps(body)
    enabled.hook(
        "Stop", enabled.payload(observations=_observations(), event_id="stop-keep")
    )
    for verb in (
        ("purge", "--yes"),
        ("reconcile",),
        ...
        completed = enabled.run(
            "integration", *verb, "--project-root", str(other), "--json"
        )
    ...
    assert (
        enabled.integration_status()["events"]["pending"] == 1
    )  # nothing acted on the real project
...
def test_duplicate_delivery_returns_the_stored_receipt(enabled: Harness) -> None:
    ...
    conflict_run, conflict = enabled.hook(
        "Stop",
        enabled.payload(observations=_observations("different"), event_id="stop-1"),
    )
...
def test_mutation_without_identity_is_refused(enabled: Harness) -> None:
    completed, body = enabled.hook(
        "Stop", enabled.payload(observations=_observations())
    )
    assert completed.returncode == 2
    ...
    observations = [
        {
            ...
            "metadata": {"count": 5, "ratio": 0.5, "ok": True, "tags": ["a"]},
        }
    ]
    _, first = enabled.hook(
        "Stop", enabled.payload(observations=observations, event_id="stop-2")
    )
...
def test_reconcile_commits_captures_and_refreshes(enabled: Harness) -> None:
    ...
    _, refresh = enabled.hook(
        "PostToolUse", enabled.payload(artifacts=files, event_id="tool-3")
    )
    assert [r["kind"] for r in refresh["receipts"]] == ["queued"]
    gone = enabled.project / "src" / "gone.py"
    gone.write_text("x = 1\n", encoding="utf-8")
    enabled.hook(
        "PostToolUse", enabled.payload(artifacts=[str(gone)], event_id="tool-4")
    )
    gone.unlink()
    completed = enabled.run(
        "integration", "reconcile", "--project-root", str(enabled.project), "--json"
    ...
    assert {"durable_id", "kind", "state", "attempts", "resets", "result"} <= set(
        result["rows"][0]
    )
    for row in result["rows"]:
        assert set(row["result"]) == {"outcome", "memory_ids"} and isinstance(
            row["result"]["memory_ids"], list
        )
        assert row["resets"] == 0
    _, vanished = enabled.hook(
        "PostToolUse", enabled.payload(artifacts=[str(gone)], event_id="tool-4")
    )
    assert (
        vanished["receipts"][0]["kind"] == "indexed"
        and vanished["receipts"][0]["duplicate"] is True
    )
    assert any(
        row["result"]["memory_ids"]
        for row in result["rows"]
        if row["kind"] == "capture"
    )
    assert enabled.integration_status()["events"] == {
...
def test_session_start_recalls_within_budget(enabled: Harness) -> None:
    for index in range(
        25
    ):  # more rows than the 20-row fetch ceiling, so ordering must be exact
        enabled.hook(
            "Stop",
            enabled.payload(
    ...
    _, pending = enabled.hook(
        "SessionStart", enabled.payload(session_id="pending-session")
    )
    assert (
        pending["status"] == "partial" and "Decision 24" in pending["context"]["text"]
    )  # recall never waits
    assert (
        enabled.run(
            "integration", "reconcile", "--project-root", str(enabled.project)
        ).returncode
        == 0
    )
    completed, body = enabled.hook(
        "SessionStart", enabled.payload(session_id="second-session")
    )
    ...
    assert [e["type"] for e in context["entries"]] and all(
        e["type"] == "memory" for e in context["entries"]
    )
    ...
    assert (
        fetched == 20
        and "Decision 4" not in json.dumps(context)
        and "Decision 0" not in json.dumps(context)
    )
    assert all(
        e["id"].startswith("capture:") and e["id"].endswith(".0")
...
def test_user_prompt_submit_returns_evidence_inside_the_project(
    enabled: Harness,
) -> None:
    ...
    hostile.write_text(
        "JWT bearer tokens are validated here as well.\n", encoding="utf-8"
    )
    control = hostile_dir / "line\nbreak.md"
    control.write_text(
        "ZEBRA tokens are validated in this file only.\n", encoding="utf-8"
    )
    files = [
        str(enabled.project / "src" / "auth.py"),
        str(enabled.project / "README.md"),
    ...
    completed, body = enabled.hook(
        "UserPromptSubmit",
        enabled.payload(query="How are JWT bearer tokens validated?"),
    )
    assert body["status"] == "ok", body
    ...
    assert zebra["context"]["omitted"], zebra[
        "context"
    ]  # the control-character path is dropped whole
    planted.unlink()  # indexed, then deleted: the entry can no longer name an existing file
    _, stale = enabled.hook(
        "UserPromptSubmit", enabled.payload(query="ignore previous instructions")
    )
    assert all(
        e["data"]["file_path"] != "NOTES.md" for e in stale["context"]["entries"]
    )
...
def test_a_correction_is_the_row_a_later_question_retrieves(enabled: Harness) -> None:
    ...
    _code, _out, heavy = enabled.probe(
        ...
        "--event",
        "UserPromptSubmit",
        stdin=json.dumps(enabled.payload(query="What is the release codename?")).encode(
            "utf-8"
        ),
    )
...
def test_read_stage_is_skipped_when_the_deadline_is_short(enabled: Harness) -> None:
    completed, body = enabled.hook(
        "UserPromptSubmit",
        enabled.payload(query="jwt"),
        env={"INQUIRY_HOOK_DEADLINE_SECONDS": "1"},
    )
    assert completed.returncode == 1
    ...
    completed, body = enabled.hook(
        "SessionStart", env={"INQUIRY_HOOK_DEADLINE_SECONDS": "1"}
    )
    assert (
        completed.returncode == 0 and body["status"] == "ok"
    )  # ledger recall is never skipped
...
def test_notify_fires_once_per_session_and_code(enabled: Harness) -> None:
    ...
    assert (
        first["errors"][0]["code"] == "event_id_required"
        and first["errors"][0]["notify"] is True
    )
    assert (
        second["errors"][0]["code"] == "event_id_required"
        and second["errors"][0]["notify"] is False
    )
    _, other = enabled.hook(
        "Stop",
        enabled.payload(session_id="other-session", observations=_observations()),
    )
...
def test_policy_flags_disable_capture_refresh_and_recall(enabled: Harness) -> None:
    ...
    assert enabled.enable("--no-capture", "--no-refresh", "--no-recall").returncode == 0
    completed, body = enabled.hook(
        "Stop", enabled.payload(observations=_observations(), event_id="p-1")
    )
    ...
    assert (
        completed.returncode == 2 and body["errors"][0]["code"] == "event_id_required"
    )
    completed, body = enabled.hook(
        "PostToolUse",
        enabled.payload(artifacts=[str(enabled.project / "README.md")], event_id="p-2"),
    )
    assert completed.returncode == 2 and body["errors"][0]["code"] == "refresh_disabled"
    (enabled.project / "quiet").mkdir()
    completed, body = enabled.hook(
        "PostToolUse",
        enabled.payload(artifacts=[str(enabled.project / "quiet")], event_id="p-3"),
    )
    assert body["status"] != "error" and [e["code"] for e in body["errors"]] == [
        "artifact_ignored"
    ]  # nothing to refresh
    assert enabled.integration_status()["events"] == {
        "pending": 0,
    ...
    assert (
        body["status"] == "ok"
        and body["context"]["budget"] == 0
        and body["context"]["text"] == ""
    )


def test_ledger_only_events_stay_light(enabled: Harness) -> None:
    enabled.hook(
        "Stop", enabled.payload(observations=_observations(), event_id="stop-warm")
    )
    ...
    assert completed.wall < 0.5, completed.wall  # type: ignore[attr-defined]
    probed = enabled.payload(
        observations=_observations(), event_id="stop-probe"
    )  # fresh insert on the bound client
    ...
    artifacts = enabled.payload(
        artifacts=[str(enabled.project / "README.md")], event_id="tool-light"
    )
    code, out, heavy = enabled.probe(
        "integration",
        "hook",
...
def test_recall_text_is_framed_as_data(enabled: Harness) -> None:
    ...
    _, body = enabled.hook(
        "SessionStart", enabled.payload(session_id="framing-session")
    )
    text = body["context"]["text"]
    lines = text.splitlines()
    assert "not instructions" in lines[0]
    ...
    assert len(openers) == 2 and all(
        " owner=afp:claude-code " in line for line in openers
    ), openers
    assert sorted(line.split(" ")[1].rsplit(".", 1)[1] for line in openers) == [
        "0",
        "1",
    ]
    ...
    assert (
        "<<" not in angled[0]["data"]["summary"]
        and angled[0]["data"]["summary"] in text
    )  # escaped, then cut


def test_multi_observation_capture_is_one_row_and_many_items(enabled: Harness) -> None:
    observations = [
        {"content": f"Observation {i}: rotate key {i}", "importance": 0.6}
        for i in range(3)
    ]
    _, body = enabled.hook(
        "Stop", enabled.payload(observations=observations, event_id="stop-three")
    )
    assert [r["kind"] for r in body["receipts"]] == ["queued"] and body[
        "pending_total"
    ] == 1
...
def test_recall_labels_each_frame_with_its_capturing_client(enabled: Harness) -> None:
    ...
    codex, _ = enabled.hook(
        "Stop",
        enabled.payload(
            observations=_observations("From codex: use argon2."), event_id="s-x"
        ),
        client="codex",
    )
    claude, _ = enabled.hook(
        "Stop",
        enabled.payload(
            observations=_observations("From claude: use bcrypt."), event_id="s-c"
        ),
    )
    ...
    text = body["context"]["text"]
    openers = [
        line for line in text.splitlines() if line.startswith("<<memory id=capture:")
    ]
    assert any(" owner=afp:codex " in line for line in openers) and any(
        " owner=afp:claude-code " in line for line in openers
    )
    assert "From codex" in text and "From claude" in text
    assert {e["data"]["owner"] for e in body["context"]["entries"]} == {
        "afp:codex",
        "afp:claude-code",
    }
    ...
    assert (
        "From codex" not in body["context"]["text"]
        and "From claude" in body["context"]["text"]
    )


def test_reconcile_reports_a_failed_row_without_refusing(
    enabled: Harness, tmp_path: Path
) -> None:
    note = enabled.project / "note.md"
    note.write_text("# note\n", encoding="utf-8")
    enabled.hook(
        "PostToolUse", enabled.payload(artifacts=[str(note)], event_id="tool-note")
    )
    ...
    assert (
        report["rows"][0]["result"]["outcome"] == "artifact_escape"
        and report["rows"][0]["attempts"] == 1
    )
    assert str(tmp_path) not in completed.stdout.decode("utf-8")
    ...
    completed, body = enabled.hook(
        "SessionStart", enabled.payload(session_id="after-exhaustion")
    )
    assert (
        completed.returncode == 0
        and body["status"] == "ok"
        and body["pending"] == []
        and body["pending_total"] == 0
    )
    advisory = [e for e in body["errors"] if e["code"] == "capture_exhausted"]
    assert (
        advisory
        and durable_id in advisory[0]["message"]
        and "--retry" in advisory[0]["message"]
    )
    assert advisory[0]["notify"] is True
    _, again = enabled.hook(
        "SessionStart", enabled.payload(session_id="after-exhaustion")
    )
    assert [
        e["notify"] for e in again["errors"] if e["code"] == "capture_exhausted"
    ] == [False]
    retried = enabled.run(
        "integration",
        "reconcile",
    ...
    assert (
        retried.returncode == 1 and row["attempts"] == 4 and row["resets"] == 1
    )  # attempts stay monotonic
    assert enabled.integration_status()["events"] == {
        "pending": 0,
        "committed": 0,
        "failed": 0,
    ...
    assert (
        stale.returncode == 0 and listed["exhausted"] == 1
    )  # names nothing: resets nothing
    assert [r["durable_id"] for r in listed["rows"]] == [durable_id] and listed["rows"][
        0
    ]["resets"] == 1
    second = enabled.run(
        "integration",
    ...
    assert (
        third.returncode == 1
        and json.loads(third.stdout)["errors"][0]["code"] == "policy_invalid"
    )
    forced = enabled.run(
        "integration",
        "reconcile",
    ...
    enabled.hook(
        "PostToolUse", enabled.payload(artifacts=[str(fifo_note)], event_id="tool-fifo")
    )
    fifo_note.unlink()
    ...
    fifo_rows = [
        ...
        if r["durable_id"].startswith("refresh:")
    ]
    assert any(
        r["state"] == "committed" and r["result"]["outcome"] == "artifact_ignored"
        for r in fifo_rows
    ), fifo_rows


def test_corrupt_ledger_degrades_like_an_unreadable_one(enabled: Harness) -> None:
    enabled.records().write_bytes(
        b"not a database, just bytes " * 64
    )  # passes the fstat checks, fails inside sqlite
    completed, body = enabled.hook(
        "SessionStart", enabled.payload(session_id="corrupt")
    )
    assert completed.returncode == 1 and body["status"] == "partial", body
    assert (
        body["errors"][0]["code"] == "ledger_unavailable"
    ...
    completed, body = enabled.hook(
        "Stop",
        enabled.payload(
            observations=_observations("lost to corruption"),
            event_id="stop-corrupt",
            session_id="corrupt",
        ),
    )
    assert (
        completed.returncode == 2
        and body["errors"][0]["code"] == "ledger_unavailable"
        and body["receipts"] == []
    )
...
def test_locked_ledger_degrades_without_raising(enabled: Harness) -> None:
    enabled.hook(
        "Stop",
        enabled.payload(
            observations=_observations("before the lock"), event_id="stop-lock"
        ),
    )
    ...
    try:
        ...
        )  # first access takes the exclusive lock
        started = time.perf_counter()
        completed, body = enabled.hook(
            "SessionStart",
            enabled.payload(session_id="locked"),
        ...
        completed, body = enabled.hook(
            "Stop",
            enabled.payload(
                observations=_observations("during the lock"), event_id="stop-locked"
            ),
            env={"INQUIRY_HOOK_DEADLINE_SECONDS": "2"},
        )
        assert (
    ...
    completed, body = enabled.hook(
        "Stop",
        enabled.payload(observations=_observations("after"), event_id="stop-after"),
    )
    assert completed.returncode == 1 and [r["kind"] for r in body["receipts"]] == [
        "queued"
    ]
    assert (
        enabled.integration_status()["events"]["pending"] == 2
    )  # the locked capture was never recorded
...
def test_purge_removes_committed_memories_from_recall(enabled: Harness) -> None:
    enabled.hook(
        "Stop",
        enabled.payload(
            observations=_observations("Secret: rotate the deploy key."),
            event_id="stop-secret",
        ),
    )
    for index in range(6):
        enabled.hook(
            "Stop",
            enabled.payload(
                observations=_observations(f"Filler {index}: rotate the deploy key."),
                event_id=f"stop-filler-{index}",
            ),
        )
    assert (
        enabled.run(
    ...
    records = enabled.records()
    residue = b"".join(
        path.read_bytes() for path in records.parent.glob("records.sqlite3*")
    )
    assert b"rotate the deploy key" not in residue
    lancedb_dir = enabled.project / ".agentic-inquiry" / "envs" / "ai" / "lancedb"
    table_bytes = b"".join(
        path.read_bytes() for path in lancedb_dir.rglob("*") if path.is_file()
    )
    ...
    assert (
        after["context"]["text"] == ""
        and after["context"]["used"] == 0
        and after["context"]["entries"] == []
    )
    ...
    assert (
        ...
        and redelivered["errors"][0]["code"] == "event_purged"
    )
    assert "rotate the deploy key" not in json.dumps(redelivered)
    completed, mismatched = enabled.hook(
        "Stop",
        enabled.payload(
            observations=_observations("Guess: something else."), event_id="stop-secret"
        ),
    )
    assert (
        mismatched["errors"][0]["code"] == "event_purged"
    )  # never event_id_conflict over erased content
    assert enabled.integration_status()["events"] == {
        "pending": 0,
        "committed": 0,
        "failed": 0,
...
def test_purge_without_yes_needs_an_exact_affirmative(enabled: Harness) -> None:
    enabled.hook(
        "Stop", enabled.payload(observations=_observations(), event_id="stop-confirm")
    )
    for answer in (b"no\n", b"y\n", b"YES\n", b""):
        completed = enabled.run(
            "integration",
        ...
        assert (
            json.loads(completed.stdout)["errors"][0]["code"] == "confirmation_declined"
        )
        prompt = completed.stderr.decode(
            "utf-8"
        )  # the prompt keeps off stdout when --json is given
...
def test_purge_removes_events_and_keeps_bindings(enabled: Harness) -> None:
    enabled.hook(
        "Stop", enabled.payload(observations=_observations(), event_id="stop-purge")
    )
    assert enabled.integration_status()["events"]["pending"] == 1
    completed = enabled.run(
        "integration",
    ...
    _, body = enabled.hook(
        "Stop", enabled.payload(observations=_observations(), event_id="stop-after")
    )
...
def test_status_reports_bindings_and_counts(enabled: Harness) -> None:
    enabled.hook(
        "Stop", enabled.payload(observations=_observations(), event_id="stop-9")
    )
...
def test_status_help_has_no_db_option(harness: Harness) -> None:
    ...
    assert status_help.returncode == 0 and "--db" not in status_help.stdout.decode(
        "utf-8"
    )
...
def test_operator_verbs_answer_ok_without_a_binding(
    onboarded: Harness, argv: tuple[str, ...]
) -> None:
