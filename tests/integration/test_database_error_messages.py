    ...
    async def test_database_error_includes_operation_type(self):
        ...
        **Validates: Requirements 9.4**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(db_manager, '_get_or_create_table', return_value=(mock_table, False)):
                # Try to add rows which should fail
                with pytest.raises(StorageError) as exc_info:
                    await db_manager._add_rows("test_table", [{"id": "1", "data": "test"}])
    ...
    async def test_table_not_found_error_suggestion(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(db_manager, '_get_or_create_table', return_value=(mock_table, False)):
                ...
                assert any(word in error_msg.lower() for word in ["recreat", "creat", "exist"])
    ...
    async def test_schema_mismatch_error_suggestion(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(db_manager, '_get_or_create_table', return_value=(mock_table, False)):
                ...
                assert any(word in error_msg.lower() for word in ["field", "type", "match"])
    ...
    async def test_permission_error_suggestion(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(db_manager, '_get_or_create_table', return_value=(mock_table, False)):
                ...
                assert any(word in error_msg.lower() for word in ["access", "write", "directory"])
    ...
    async def test_generic_database_error_suggestion(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(db_manager, '_get_or_create_table', return_value=(mock_table, False)):
                ...
                assert "Suggestion:" in error_msg or "suggestion" in error_msg.lower()
                assert any(word in error_msg.lower() for word in ["database", "connection", "storage"])
