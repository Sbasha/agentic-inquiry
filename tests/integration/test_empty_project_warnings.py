...
async def empty_project_services():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        storage = await StorageFacade.from_config(
            config, project_id="empty_test_project"
        )
...
async def incomplete_project_services():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        db_manager = LanceDBManager.from_config(config)
        storage = await StorageFacade.from_config(
            config, project_id="incomplete_test_project"
        )

        async with EventSystem(
            config, project_id="incomplete_test_project"
        ) as event_system:
            embedding_service = EmbeddingService(config)
            search_service = SearchService(
            ...
            await db_manager.add_document_chunks(
                chunks=chunks_data, project_id="incomplete_test_project"
            )
...
async def test_empty_project_warning_in_search(empty_project_services):
    ...
    **Validates: Requirements 4.3, 5.1**
    ...
    result = await search_knowledge(
        services=services, session_id=session_id, query="test query"
    )
...
async def test_incomplete_data_warning_in_search(incomplete_project_services):
    ...
    **Validates: Requirements 4.4, 5.2**
    ...
    warning_text = " ".join(result["warnings"])
    assert (
        "only" in warning_text.lower()
        or "consider" in warning_text.lower()
        or "more" in warning_text.lower()
    )
...
async def test_empty_project_guidance_in_project_info(empty_project_services):
    ...
    **Validates: Requirements 5.4, 5.5, 12.3**
    ...
    result = await get_project_info(services=services, session_id=session_id)
