    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config = Config(
            storage=StorageConfig(
                root=str(storage_root), default_project_id="test_project"
            )
        )
        yield config
...
@pytest_asyncio.fixture
async def indexing_pipeline_fixture(
    mock_db_manager, test_config_with_db, mock_embedding_registry, temp_project_root
):
...
class TestIndexingPipelineIntegration:
    async def test_complete_indexing_flow(
        self, indexing_pipeline_fixture, temp_project_root, mock_db_manager
    ):
...
class TestMultiProjectIntegration:
    async def test_multi_project_isolation(
        self, temp_project_root, test_config_with_db, mock_embedding_registry
    ):
        from agentic_inquiry.parsers.executor import get_parser_instance, execute_parser
        ...
        async with LanceDBManager.from_config(test_config_with_db) as mock_db_manager:
            ...
            chunks1 = await mock_db_manager.advanced_filter(
                table_name="document_chunks",
                filters={"project_id": "project1"},
                limit=100,
                project_id=None,
            )
            chunks2 = await mock_db_manager.advanced_filter(
                table_name="document_chunks",
                filters={"project_id": "project2"},
                limit=100,
                project_id=None,
            )
...
class TestConfigurationIntegration:
    def test_config_with_default_project_id(self, tmp_path):
        ...
        config = Config(
            storage=StorageConfig(root=str(tmp_path), default_project_id="my_project")
        )
    ...
    def test_config_without_default_project_id(self, tmp_path):
        ...
        config = Config(
            storage=StorageConfig(root=str(tmp_path), default_project_id=None)
        )
    ...
    def test_config_storage_root_path(self, tmp_path):
        ...
        config = Config(
            storage=StorageConfig(root=str(tmp_path), default_project_id="test_proj")
        )
