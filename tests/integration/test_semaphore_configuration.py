...
class TestSemaphoreConfiguration:
    ...
    async def test_indexing_pipeline_respects_configured_limit(self, mock_db_manager, mock_config):
    ...
    async def test_indexing_pipeline_uses_default_when_not_specified(self, mock_db_manager, mock_config):
    ...
    async def test_max_concurrent_parameter_deprecated(self, mock_db_manager, mock_config):
        """Test that max_concurrent parameter shows deprecation warning."""
        # Verify backward compatibility with deprecation warning
        with pytest.warns(DeprecationWarning, match="max_concurrent parameter is deprecated"):
            pipeline = IndexingPipeline(
                db_manager=mock_db_manager,
                config=mock_config,
                project_id="test_project",
    ...
    async def test_config_takes_precedence_over_deprecated_parameter(self, mock_db_manager, mock_config):
