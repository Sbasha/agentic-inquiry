...
async def test_relationship_resolver_database_query():
    ...
    resolver = RelationshipResolver(
        symbol_registry=symbol_registry, db_manager=mock_db_manager, project_root="."
    )

    # Test resolving from database
    result = await resolver._resolve_from_database(
        target_name="ClassB", source_file="src/module_a.py"
    )
    ...
    result = await resolver._resolve_from_database(
        target_name="helper_function", source_file="src/module_c.py"
    )
...
async def test_relationship_resolver_database_query_no_match():
    ...
    result = await resolver._resolve_from_database(
        target_name="NonExistentClass", source_file="src/module_a.py"
    )
...
""")
    ...
    monkeypatch.setenv(
        "INQUIRY_STORAG_ROOT", "./typo_data"
    )  # typo: STORAG instead of STORAGE
    monkeypatch.setenv(
        "INQUIRY_DATABSE_PATH", "./db"
    )  # typo: DATABSE instead of DATABASE (not valid anyway)
...
""")
    ...
    monkeypatch.setenv(
        "INQUIRY_DATABASE_PATH", "./db"
    )  # DATABASE is not a valid section
    ...
    assert any(
        "storage" in msg and "cache" in msg and "search" in msg
...
""")
    ...
    assert len(typo_warnings) > 0, (
        f"Expected warning about INQUIRY_STORAG_ROOT, got: {warning_messages}"
    )

    # Warning should list valid sections as suggestions
    assert any("storage" in msg for msg in typo_warnings), (
        "Warning should list 'storage' as valid section"
    )
