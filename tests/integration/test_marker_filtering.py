...
def test_marker_combination_filtering():
    ...
    **Validates: Requirements 9.4**
    ...
    result = subprocess.run(
        ["uv", "run", "pytest", "-m", "integration and not slow", "--collect-only", "-q"],
        capture_output=True,
        text=True,
        timeout=30
    )
    ...
    assert "test" in output.lower() or "selected" in output or "deselected" in output or \
           "no tests" in output.lower(), \
...
def test_invalid_marker_produces_warning():
    ...
    **Validates: Requirements 9.4**
    ...
    assert "warning" in output.lower() or "no tests" in output.lower() or \
           "0 selected" in output or "empty" in output.lower(), \
