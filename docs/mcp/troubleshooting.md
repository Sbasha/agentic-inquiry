...
- [Cloud SQL Storage Issues](#cloud-sql-storage-issues)
...
## Cloud SQL Storage Issues

**Applies to:** All usage modes when using Cloud SQL/PostgreSQL backend

### Error Codes Reference

When using Cloud SQL storage, you may encounter these error codes:

| Error Code | Error Name | Symptoms | Likely Cause | Resolution |
|------------|------------|----------|--------------|------------|
| **AUTH_001** | AUTH_FAILED | Error: "Authentication failed"<br>Server won't connect<br>Credentials rejected | Invalid credentials<br>Wrong service account<br>Missing IAM permissions | 1. Verify credentials in configuration<br>2. Check service account has cloudsql.client role<br>3. Confirm project ID is correct<br>4. See [Cloud SQL Backend Guide](../backends/cloudsql.md) |
| **CONN_001** | CONNECTION_FAILED | Error: "Connection failed"<br>Cannot reach database<br>Timeout errors | Network connectivity issues<br>Firewall blocking connection<br>Instance not running<br>Wrong instance connection name | 1. Verify instance is running<br>2. Check connection string format<br>3. Test network connectivity<br>4. Review firewall rules<br>5. See [Cloud SQL Backend Guide](../backends/cloudsql.md) |
| **CFG_001** | CONFIG_INVALID | Error: "Configuration invalid"<br>Server fails to start<br>Missing required fields | Missing required configuration<br>Invalid YAML syntax<br>Wrong field names | 1. Validate YAML syntax<br>2. Check all required fields present<br>3. Verify field names match schema<br>4. See [Cloud SQL Backend Guide](../backends/cloudsql.md) |
| **START_001** | STARTUP_FAILED | Error: "Startup failed"<br>Server crashes on start<br>Cannot initialize storage | Database not initialized<br>Missing tables<br>Permission issues | 1. Run database initialization script<br>2. Check database user permissions<br>3. Verify schema exists<br>4. See [Cloud SQL Backend Guide](../backends/cloudsql.md) |
| **QUERY_001** | QUERY_FAILED | Error: "Query failed"<br>Search returns errors<br>Database operations fail | Query syntax error<br>Missing indexes<br>Database connection lost | 1. Check query syntax<br>2. Verify indexes are created<br>3. Test database connection<br>4. Review query logs<br>5. See [Cloud SQL Backend Guide](../backends/cloudsql.md) |
...
1. **Missing IAM Role**
   - **Solution**: Add `roles/cloudsql.client` role
   ```bash
...
2. **Wrong Credentials Path**
   - **Solution**: Verify path in configuration
   ```yaml
   storage:
     backends:
       cloudsql:
...
**See Also:** [Cloud SQL Backend Guide](../backends/cloudsql.md)

#### Problem: Connection Timeout or Refused

**Symptoms:**
- Error: "CONN_001: Connection failed"
- Timeout after 30 seconds
- Cannot reach Cloud SQL instance
...
# Test direct connection (if using public IP)
...
1. **Wrong Connection String**
   - **Solution**: Verify format `project:region:instance`
   ```yaml
   storage:
     backends:
       cloudsql:
...
2. **Firewall Blocking Connection**
   - **Solution**: Add authorized network
   ```bash
...
**Symptoms:**
- Error: "CFG_001: Configuration invalid"
...
1. **Missing Required Field**
   ...
   - See configuration template in [Cloud SQL Backend Guide](../backends/cloudsql.md)

2. **Wrong Field Names**
   - **Solution**: Use correct field names
   - Use `connection_name` not `instance_name`
   - Use `credentials_path` not `credentials_file`

3. **Invalid YAML Syntax**
   - **Solution**: Fix indentation (use spaces, not tabs)
   - Validate with YAML linter

**See Also:** [Cloud SQL Backend Guide](../backends/cloudsql.md)

### Quick Diagnostics for Cloud SQL

```python
# Test Cloud SQL connection from Python
import asyncpg
...
**For detailed Cloud SQL setup and troubleshooting, see:** [Cloud SQL Backend Guide](../backends/cloudsql.md)
