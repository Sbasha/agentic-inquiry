...
class EventStore:
    ...
    async def _init_database(self) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Create indexes
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_operation_id ON events(operation_id, timestamp)"
            )
            await db.execute(
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_project_timestamp ON events(project_id, timestamp)"
            )
        ...
        for attempt in range(policy.max_attempts):
            try:
                ...
                try:
                    ...
                    logger.critical(
                        "Failed to recover from database error: %s", recovery_error
                    )
                raise
    ...
    async def cleanup_old_events(
        ...
        try:
            ...
            logger.info(
                "Deleted %s events older than %s days for project %s",
                count_to_delete,
                retention_days,
                project_id,
            )
