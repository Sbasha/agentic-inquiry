...
SAFE_PICKLE_MODULES = frozenset(
    {
        "agentic_inquiry.parsers.models",
        "agentic_inquiry.models.document",
        "agentic_inquiry.models.graph_entity",
        "agentic_inquiry.indexing.models",
        "builtins",
        "datetime",
        "pathlib",
        "collections",
...
class DocumentCache:
    ...
    def __init__(
        ...
        if self._disk_enabled:
            try:
                ...
                self._cache_dir: Optional[Path] = (
                    self.config.storage.get_document_cache_path()
                )
                ...
                logger.debug(
                    "Initialized DocumentCache with disk persistence at %s",
                    self._cache_dir,
                )
        ...
        logger.debug(
            "Initialized DocumentCache with max_size=%s, disk_enabled=%s, project_id=%s",
            max_size,
            self._disk_enabled,
            self._project_id,
        )
    ...
    async def from_config(
        cls, config: Optional[Config] = None, project_id: Optional[str] = None
    ) -> "DocumentCache":
            ...
            for key, entry_file in index.items():
                ...
                if entry_path.exists():
                    try:
                        ...
                        logger.warning(
                            "Failed to load cache entry %s: %s. Entry will be removed.",
                            entry_file,
                            e,
                        )
    ...
    async def _cleanup_corrupted_entries(
        self, corrupted_entries: list[tuple[str, str]]
    ) -> None:
        ...
        try:
            ...
            for key, entry_file in corrupted_entries:
                ...
                try:
                    ...
                    logger.debug(
                        "Failed to remove corrupted entry file %s: %s", entry_file, e
                    )
            ...
            logger.debug(
                "Cleaned up %s corrupted cache entries", len(corrupted_entries)
            )
    ...
    async def _save_to_disk(self, key: str, value: Tuple[Any, float, str]) -> None:
        ...
        try:
            ...
            loop = asyncio.get_running_loop()
            pickled_data = await loop.run_in_executor(None, pickle.dumps, value)
    ...
    async def put(self, path: str, document: Any) -> None:
        ...
        try:
            ...
            cache_value = (document, datetime.now().timestamp(), path)
    ...
    async def invalidate(self, path: str) -> None:
        ...
        """
        keys_to_remove = [k for k, (_, _, p) in self._cache.items() if p == path]
        ...
        if keys_to_remove:
            logger.debug(
                "Invalidated %s cache entries for %s", len(keys_to_remove), path
            )
