...
class BackendConfig(BaseModel):
    ...
    # their own spellings (see agentic_inquiry.storage.similarity).
    #
    ...
    def validate_cloudsql_config(self) -> Self:
        ...
        if missing:
            raise ValueError(
                ...
                f"region='us-central1', instance='my-instance', database='ai', user='ai-user')"
            )
    ...
    def validate_rds_config(self) -> Self:
        ...
        if missing:
            raise ValueError(
                ...
                f"database='ai', user='ai_user', password='secret')"
            )
...
DEFAULT_BACKENDS: dict[str, BackendConfig] = {
    ...
    "sqlite": BackendConfig(
        type="sqlite",
        database_path="./.agentic-inquiry/agentic-inquiry.db",
    ),
}
