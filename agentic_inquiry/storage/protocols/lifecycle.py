...
each backend owns its own setup/teardown, with ai core providing only context.
...
Resource ownership model:
    ai Core Provides          Backend Creates
    ─────────────────          ───────────────
...
class BackendLifecycle(Protocol):
    ...
    async def initialize(self) -> None:
        ...
        Resource creation is the backend's responsibility, not ai core.
