...
Defined in `agentic_inquiry/storage/protocols/`. A backend registers for one or more roles:
...
5. Add an entry to `agentic-inquiry.yaml.example` so users see the shape.
