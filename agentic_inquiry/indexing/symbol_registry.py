...
class SymbolRegistry:
    ...
    async def register(
        ...
        logger.debug(
            "Registered symbol: %s (%s) in %s [module: %s]",
            name,
            entity_type,
            file_path,
            module_path,
    ...
    def resolve_with_cache(
        self, target_name: str, target_type: str, source_file: str
    ...
    def _resolve_impl(
        ...
        if len(candidates) == 1:
            ...
            logger.debug(
                "Resolved %s via exact match: %s", target_name, candidate.file_path
            )
        ...
        for candidate in candidates:
            ...
            co_occurrence = self.get_co_occurrence_score(
            ...
            naming_score = self.score_file_naming_pattern(
            ...
            directory_score = self.score_directory_structure(
                target_name, candidate.file_path
            )
        ...
        if confidence >= 0.3:
            logger.debug(
                "Resolved %s to %s (score: %.1f, confidence: %.2f)",
                target_name,
        ...
        logger.debug(
            "Best match for %s has low confidence (%.2f), returning None",
            target_name,
            confidence,
        )
        return None
        ...
        for symbol in symbols:
            ...
            if symbol in self._by_name:
                self._by_name[symbol] = [
                    metadata
                    for metadata in self._by_name[symbol]
            ...
            for key in list(self._by_name_type.keys()):
                if key[0] == symbol:
                    self._by_name_type[key] = [
                        metadata
                        for metadata in self._by_name_type[key]
                        if metadata.file_path != file_path
                    ]
    ...
    def _file_to_module_path(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(
                "Failed to convert file path to module path: %s - %s", file_path, e
            )
            return None
    ...
    def score_file_naming_pattern(self, symbol_name: str, file_path: str) -> float:
        ...
        try:
            ...
            logger.debug(
                "Failed to score file naming pattern: %s, %s - %s",
                symbol_name,
                file_path,
                e,
            )
    ...
    def score_directory_structure(self, symbol_name: str, file_path: str) -> float:
        ...
        try:
            ...
            logger.debug(
                "Failed to score directory structure: %s, %s - %s",
                symbol_name,
                file_path,
                e,
            )
    ...
    async def track_import(
        self, symbol_name: str, source_file: str, target_file: str
    ) -> None:
        ...
        self._import_frequency[symbol_name] = (
            self._import_frequency.get(symbol_name, 0) + 1
        )
        ...
        logger.debug(
            "Tracked import: %s from %s to %s (frequency: %s, co-occurrence: %s)",
            symbol_name,
            source_file,
            target_file,
            self._import_frequency[symbol_name],
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        most_imported = sorted(
            self._import_frequency.items(), key=lambda x: x[1], reverse=True
        )[:10]
