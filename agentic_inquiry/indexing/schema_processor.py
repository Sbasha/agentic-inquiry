...
class SchemaProcessor:
    ...
    def __init__(self, db_manager: Any, schema: Optional[LogicalSchema] = None):
    ...
    def _validate_chunk(self, chunk: ParserChunk) -> None:
        ...
        if not chunk.content and not chunk.fts_text:
            raise ValidationError("Chunk must have either content or fts_text")
    ...
    def _resolve_source_modified_at(
        self, parsed_document: ParsedDocument, chunk: ParserChunk
    ) -> Optional[datetime]:
    ...
    def _transform_metadata(self, chunk: ParserChunk) -> Dict[str, str]:
        ...
        if chunk.metadata:
            try:
                ...
                logger.warning(
                    "Failed to serialize chunk metadata: %s. Using empty dict.", e
                )
                metadata_dict["data"] = "{}"
        ...
        if chunk.symbol_metadata:
            try:
                ...
                logger.warning(
                    "Failed to serialize symbol_metadata: %s. Using empty dict.", e
                )
        ...
        if chunk.symbol_rankings:
            try:
                ...
                logger.warning(
                    "Failed to serialize symbol_rankings: %s. Using empty dict.", e
                )
    ...
    def _transform_ranking_signals(self, chunk: ParserChunk) -> Dict[str, str]:
        ...
        if chunk.ranking_signals:
            try:
                ...
                logger.warning(
                    "Failed to serialize ranking_signals: %s. Using empty dict.", e
                )
    ...
    def _sanitize_metadata(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "ranking_signals" in record and not isinstance(
            record["ranking_signals"], dict
        ):
    ...
    async def process_chunks(
        ...
        logger.info(
            "Processing %d chunks for document %s", len(chunks), parsed_document.doc_id
        )
        ...
        async with Transaction(self.db_manager) as txn:
            ...
            total_chunks = (
                ...
                if original_total_chunks is not None
                else len(chunks)
            )
            ...
            async def write_chunks(records: List[Dict[str, Any]]) -> None:
                ...
                for record in records:
                    ...
                    if isinstance(record.get("indexed_at"), str):
                        record["indexed_at"] = datetime.fromisoformat(
                            record["indexed_at"]
                        )
                    if isinstance(record.get("source_modified_at"), str):
                        record["source_modified_at"] = datetime.fromisoformat(
