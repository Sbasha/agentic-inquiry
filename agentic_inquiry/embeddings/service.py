...
class EmbeddingService:
    ...
    def _create_embedder(self) -> Embedder:
        ...
        if provider == "hashing":
            ...
            logger.info("Created HashingEmbedder (%d dims)", embedder.ndims())
            return embedder
    ...
    async def embed_async(self, text: str) -> np.ndarray:
        ...
        embeddings = await loop.run_in_executor(
            get_embedding_executor(), embedder.generate, [text]
        )
    ...
    async def embed_batch_async(self, texts: List[str]) -> List[np.ndarray]:
        ...
        loop = asyncio.get_running_loop()
        embeddings = await loop.run_in_executor(
            get_embedding_executor(), embedder.generate, texts
        )
    ...
    def start_background_warmup(self) -> None:
        ...
        self._warmup_thread = threading.Thread(
            target=_warmup, name="embedding-warmup", daemon=True
        )
    ...
    async def wait_for_warmup(self, timeout: float = 30.0) -> bool:
        ...
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(None, self._warmup_complete.wait, timeout)
    ...
    async def generate_embeddings_batch(
        ...
        if len(texts) > 100:
            logger.info(
                "Generating embeddings for %d texts (purpose: %s)", len(texts), purpose
            )
