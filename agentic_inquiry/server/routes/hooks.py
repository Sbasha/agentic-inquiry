...
logger = logging.getLogger("ai.server.routes.hooks")
...
async def hooks_post_write(request: Request, body: PostWriteRequest) -> dict:
    ...
    if importance == "config":
        prompt = (
            "[ai Memory] Configuration file modified. "
            "If this changes behavior, store why via /ai:memory save."
        )
    elif importance == "entry_point":
        prompt = (
            "[ai Memory] Entry point modified. "
