...
from agentic_inquiry.metrics import get_metrics_tracker, MetricsTracker
from agentic_inquiry.models.health import (
    ComponentHealth,
...
class HealthTracker:
    ...
    def get_component_health(self, component_id: str) -> ComponentHealth:
        ...
        critical_thresh = self._thresholds.get(
            f"{component_id}.latency_p95_critical", 2.0
        )
        degraded_thresh = self._thresholds.get(
            f"{component_id}.latency_p95_degraded", 1.0
        )

        if p95 >= critical_thresh:
            status = HealthStatus.CRITICAL
            issues.append(
                f"Latency P95 ({p95:.2f}s) exceeds critical threshold ({critical_thresh}s)"
            )
        elif p95 >= degraded_thresh:
            status = HealthStatus.DEGRADED
            issues.append(
                f"Latency P95 ({p95:.2f}s) exceeds degraded threshold ({degraded_thresh}s)"
            )
    ...
    def get_overall_health(self) -> CognitiveHealthState:
        ...
        return CognitiveHealthState(
            overall_status=worst_status, components=component_healths, insights=insights
        )
