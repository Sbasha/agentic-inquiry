...
def detect_ambiguity(
    ...
    if top_score < min_confidence:
        return {"ambiguous": True, "reason": "low_confidence", "confidence": top_score}
    ...
    if len(similar_results) >= 2:
        ...
        unique_files = {
            get_data(r).get("file_path")
            for r in similar_results
            if get_data(r).get("file_path")
        }
        unique_names = {
            get_data(r).get("name") for r in similar_results if get_data(r).get("name")
        }
