...
class MCPErrorHandler:
    ...
    @staticmethod
    async def handle(
        error: Exception, context: Dict[str, Any], services: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if isinstance(error, SessionNotFoundError):
            return await MCPErrorHandler._format_error(
                "SESSION_NOT_FOUND", context, services
            )
            ...
            return await MCPErrorHandler._format_error(
                "STORAGE_ERROR", {"details": str(error)}, services
            )

        elif isinstance(error, ConfigurationError):
            return await MCPErrorHandler._format_error(
                "CONFIGURATION_ERROR", {"details": str(error)}, services
            )
        ...
        if "session" in error_msg and (
            "not found" in error_msg or "expired" in error_msg
        ):
            return await MCPErrorHandler._format_error(
                "SESSION_NOT_FOUND", context, services
            )
            ...
            suggestions = await MCPErrorHandler._suggest_search_alternatives(
                context.get("query", ""), services
            )
            return await MCPErrorHandler._format_error(
                "NO_RESULTS", context, services, additional_suggestions=suggestions
            )

        elif "token" in error_msg and "budget" in error_msg:
            return await MCPErrorHandler._format_error(
                "TOKEN_BUDGET_EXCEEDED", context, services
            )
    ...
    async def _suggest_search_alternatives(
        query: str, services: Dict[str, Any]
    ) -> List[str]:
        ...
        if len(query) > 100:
            suggestions.append("Try: Using a shorter, more focused search query")
    ...
    async def _suggest_similar_entities(
        entity: str, services: Dict[str, Any]
    ) -> List[str]:
        """Generate suggestions for entity not found.
    ...
    def create_validation_error(
        ...
        if expected_values:
            suggestions.insert(
                0, f"Try: Using one of these valid values: {', '.join(expected_values)}"
            )
