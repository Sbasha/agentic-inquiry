    ...
    for term in terms:
        ...
        for variant in typo_variants:
            try:
                ...
                logger.debug(
                    "Spelling suggestion search failed for variant '%s': %s", variant, e
                )
                continue
...
async def _generate_topic_suggestions(
    ...
    try:
        ...
        logger.debug("Topic suggestion generation failed for query '%s': %s", query, e)
    ...
    if len(terms) > 1:
        ...
        for term in terms:
            ...
            try:
                ...
                logger.debug(
                    "Threshold suggestion search failed for term '%s': %s", term, e
                )
                continue
