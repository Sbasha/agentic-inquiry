...
class PatternAnalyzer:
    ...
    async def find_patterns(
        self, project_id: str, pattern_type: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    async def _find_architectural_patterns(
        ...
        try:
            ...
            for i in range(n_clusters):
                ...
                distances = [
                    np.linalg.norm(X[idx] - centroid) for idx in cluster_indices
                ]
    ...
    async def _find_patterns_by_query(
        self, project_id: str, query_info: Dict[str, str], limit: int
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            batch_embeddings = await self.embedding_service.embed_batch_async(
                texts_to_embed
            )
            ...
            for i in range(n_clusters):
                ...
                representatives = [
                    cluster_results[idx] for idx in np.argsort(distances)[:3]
                ]
                ...
                patterns.append(
                    {
                        "name": name,
                        "type": query_pattern_type,
                        "count": len(cluster_results),
                        "examples": examples,
                        "prevalence": len(cluster_results) / len(valid_results),
                    }
                )
            return patterns
        except Exception as e:
            logger.debug(
                "Query-based clustering failed for %s: %s", query_pattern_type, e
            )
            return []
    ...
    def _get_pattern_queries(self, pattern_type: Optional[str]) -> List[Dict[str, str]]:
        ...
        all_queries = {
            "architectural": [
                {
                    "query": "MVC model view controller architecture",
                    "type": "architectural",
                },
                {
    ...
    def _infer_pattern_name(self, examples: list, pattern_type: str) -> str:
