...
class SessionManager:
    ...
    async def create_session(
        ...
        logger.info(
            "Created session %s for project %s with status %s",
            session_id,
            project_id,
            status,
        )
    ...
    async def regenerate_session(
        ...
        session_age_hours = (
            datetime.now() - old_session.created_at
        ).total_seconds() / 3600
    ...
    async def change_project(
        self, session_id: str, new_project_id: str
    ) -> Dict[str, Any]:
    ...
    async def _audit_log(self, event_type: str, **kwargs: Any) -> None:
    ...
    async def get_session(
        self, session_id: str, include_history: bool = False
    ) -> Optional[Session]:
    ...
    async def _count_memories_safe(self, project_id: str) -> int:
        ...
        try:
            ...
            count = await self.db_manager.count_records(
                table_name="memory_episodic", filters={"project_id": project_id}
            )
            return count
    ...
    async def _gather_project_statistics(self, project_id: str) -> ProjectStatistics:
            ...
            try:
                ...
                try:
                    stats.total_chunks = await self.db_manager.count_records(
                        table_name="document_chunks", filters={"project_id": project_id}
                    )
                except Exception:
                ...
                try:
                    total_entities = await self.db_manager.count_records(
                        table_name="graph_entities", filters={"project_id": project_id}
                    )
                except Exception:
                    total_entities = 0
    ...
    async def list_sessions(
        ...
        try:
            sessions = await self.storage.list_sessions(
                project_id=project_id, include_expired=include_expired, limit=limit
            )
    ...
    async def add_event(
        ...
        try:
            ...
            logger.error(
                "Failed to persist session %s after adding event: %s", session_id, e
            )
...
class SessionCleanupManager:
    ...
    async def cleanup_expired_sessions(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        logger.info(
            "Cleaned %s expired sessions, archived %s", cleaned_count, archived_count
        )
        ...
        try:
            ...
            for archive_file in archive_dir.glob("*.json"):
                try:
                    ...
                    errors.append({"file": str(archive_file), "error": str(e)})
