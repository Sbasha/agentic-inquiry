...
class ProjectStatistics(BaseModel):
    ...
    index_health: str = Field(
        ..., description="Index health status: healthy, stale, or empty"
    )
    languages: Dict[str, int] = Field(
        default_factory=dict, description="File count by language"
    )
    top_directories: List[Dict[str, Any]] = Field(
        default_factory=list, description="Most active directories"
    )
...
class Session(BaseModel):
    ...
    status: str = Field(
        default="active", description="Session status (active, expired, closed)"
    )
    context_state: Dict[str, Any] = Field(
        default_factory=dict, description="Multi-turn refinement state"
    )
    history: List[Dict[str, Any]] = Field(
        default_factory=list, description="Tool call history"
    )
    events: List[Dict[str, Any]] = Field(
        default_factory=list, description="Session events for async operations"
    )
    ...
    def is_expired(self) -> bool:
        ...
        ).total_seconds() / 3600 > self.ttl_hours
    ...
    def to_db_record(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "context_state_json": json.dumps(self.context_state)
