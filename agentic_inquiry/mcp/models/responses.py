...
class AsyncIndexingResponse(BaseModel):
    ...
    operation_id: str = Field(..., description="Unique operation identifier for tracking")
    message: str = Field(..., description="Human-readable status message")
    monitor_instructions: str = Field(..., description="Instructions on how to monitor progress")
...
class SessionInfoResponse(BaseModel):
    ...
    state: str = Field(default="active", description="Session lifecycle state: active, expired, archived")
...
class GetSessionResponse(BaseModel):
    ...
    statistics: Dict[str, Any] = Field(..., description="Project statistics")
    state: str = Field(..., description="Session lifecycle state: active, expired, archived")
    log_file: str = Field(..., description="Path to session log file")
    history: Optional[List[Dict[str, Any]]] = Field(None, description="Tool call history")
...
class ResumeSessionResponse(BaseModel):
    ...
    state: Dict[str, Any] = Field(..., description="Restored context state for multi-turn refinement")
...
class SearchKnowledgeResponse(BaseModel):
    ...
    search_strategy: str = Field(..., description="Search strategy used (hybrid, vector, fts)")
...
class ContextResponse(BaseModel):
    ...
    entities_included: List[str] = Field(..., description="Entities included in context")
...
class UnderstandEntityResponse(BaseModel):
    ...
    dependencies: Optional[List[Dict[str, Any]]] = Field(None, description="Dependencies")
...
class ImpactAnalysisResponse(BaseModel):
    ...
    areas_to_review: List[str] = Field(default_factory=list, description="Areas requiring review")
    test_coverage: Dict[str, Any] = Field(default_factory=dict, description="Test coverage information")
...
class EventInfo(BaseModel):
    ...
    details: Dict[str, Any] = Field(default_factory=dict, description="Event details/metadata")
...
class GetEventsResponse(BaseModel):
    ...
    summary: Dict[str, Any] = Field(default_factory=dict, description="Event summary statistics")
    time_range: Dict[str, Any] = Field(default_factory=dict, description="Time range of events")
...
class ProjectInfoResponse(BaseModel):
    ...
    last_indexed: Optional[str] = Field(None, description="Last indexing time (ISO 8601)")
...
class ResponseMeta(BaseModel):
    ...
    embedding_time_ms: Optional[float] = Field(None, description="Time spent generating embeddings")
    query_time_ms: Optional[float] = Field(None, description="Time spent executing queries")
    entity_resolution_time_ms: Optional[float] = Field(None, description="Time spent resolving entities")
    graph_traversal_time_ms: Optional[float] = Field(None, description="Time spent traversing graph")
    context_build_time_ms: Optional[float] = Field(None, description="Time spent building context")
    memory_recall_time_ms: Optional[float] = Field(None, description="Time spent recalling memories")
