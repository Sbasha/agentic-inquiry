...
def _generate_empty_context_suggestions(query: str, has_memories: bool) -> List[str]:
    ...
    suggestions.append(f"Try a broader search query. Current query: '{query}'")
...
async def build_context(
    ...
    try:
        ...
        is_empty = not code_items and not doc_items and not memory_items
            ...
            if session:
                try:
                    ...
                    if capabilities.uses_server_side_embedding:
                        ...
                        query_vector_array = (
                            await search_service.embedding_service.embed_async(query)
                        )
                    ...
                    if search_results:
                        logger.info(
                            "Fallback search returned %d results", len(search_results)
                        )
                        ...
                        for result in search_results:
                            ...
                            result_distance = getattr(
                                result, "distance", data.get("distance", 1.0)
                            )
                    ...
                    logger.error(
                        "Fallback search failed: %s", fallback_error, exc_info=True
                    )
        ...
        is_still_empty = not code_items and not doc_items

        # Add suggestions for empty context
        if is_still_empty:
            suggestions.extend(
                _generate_empty_context_suggestions(
                    query=query, has_memories=bool(memory_items)
                )
            )
