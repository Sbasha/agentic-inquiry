...
class FileTracker:
    ...
    async def _init_database(self) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as conn:
            ...
            if table_exists:
                ...
                """)
                logger.debug(
                    "Created file_states table with project_id at %s", self.db_path
                )
    ...
    async def _migrate_schema(self, cursor: aiosqlite.Cursor) -> None:
        ...
        logger.info(
            "Migrated %s records to new schema with project_id=%s",
            cursor.rowcount,
            self.project_id,
        )
    ...
    async def update_hash(
        ...
        logger.debug(
            "Updated hash for %s in project %s: %s...",
            file_path,
            self.project_id,
            content_hash[:8],
        )
    ...
    async def remove_file(self, file_path: str) -> bool:
        ...
        if removed:
            logger.debug(
                "Removed %s from tracking in project %s", file_path, self.project_id
            )
        return removed
    ...
    async def clear(self) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as conn:
            cursor = await conn.cursor()
            await cursor.execute(
                "DELETE FROM file_states WHERE project_id = ?", (self.project_id,)
            )
            await conn.commit()
