...
# get edited away; templates move. Without a linter, the upstream
# `install-skill.py` (in the agent-ready-repo template, not vendored
# here) silently produces broken installs.
...
# adopter's own copy of these, and the upstream `install-skill.py` (in
# the agent-ready-repo template) emits whole-file deps as fragments
# under `*.fragments/<skill>.md` for the adopter to merge by hand. A
# whole-file dep is almost always a mistake — the skill rarely needs
# all of our prose. Authors should anchor-cite the specific section
# they need, or omit the dep and read at runtime (the reviewer pattern).
# Mirrors the upstream installer's FRAGMENT_FILES set; keep in sync.
