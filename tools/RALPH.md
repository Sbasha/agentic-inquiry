...
Or via env vars on the command line.
...
> **`.ralphrc` is shell input.** `tools/ralph.sh` `source`s the file, so
> anything in it executes as the Ralph operator (including arbitrary
> commands beyond the `*_CMD` assignments). Audit before each unattended
> run, and for shared repos prefer keeping `.ralphrc` out of tree — e.g.
> at `~/.config/ralph/<repo>.rc`, loaded via an explicit `source` you
> control — rather than committing it.
