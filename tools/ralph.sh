...
# Capture the bash binary path before .ralphrc has a chance to mutate PATH.
BASH_BIN="$(command -v bash)"

# Gates — override via env or .ralphrc.
LINT_CMD="${LINT_CMD:-}"
TEST_CMD="${TEST_CMD:-}"
TYPECHECK_CMD="${TYPECHECK_CMD:-}"

# ── Load project overrides ───────────────────────────────────────────────
# WARNING: .ralphrc is sourced as a shell script — anything in it runs as
# the Ralph operator. Audit before each unattended run, and prefer an
# out-of-tree path (e.g. ~/.config/ralph/<repo>.rc) for shared repos.
if [[ -f .ralphrc ]]; then
...
run_gates() {
  ...
  for pair in "lint:$LINT_CMD" "typecheck:$TYPECHECK_CMD" "test:$TEST_CMD"; do
    ...
    # Run in a child shell (via the bash captured at startup) so a gate
    # cannot mutate Ralph's own shell state — traps, set -e posture, vars.
    # This does NOT sanitize the command: $cmd is trusted shell input from
    # env / .ralphrc, and .ralphrc itself is sourced. The trust boundary
    # is the operator's audit of those, not this exec call.
    if ! "$BASH_BIN" -c "$cmd" >>"$LOG_FILE" 2>&1; then
...
while (( i < MAX_ITERATIONS )); do
  ...
  if run_gates; then
    # Gates green — check completion phrase. The contract is that the agent
    # ENDS its output with $COMPLETION_PHRASE, so anchor on the last non-blank
    # line rather than a substring search (an earlier mention in quoted text
    # or an explanation could otherwise trigger a premature clean exit).
    last_line=$(awk 'NF{line=$0} END{print line}' "$LAST_OUTPUT_FILE")
    if [[ "$last_line" == *"$COMPLETION_PHRASE"* ]]; then
      log "✓ Completion phrase on final line and gates green. Exiting clean."
      exit 0
    else
      log "Gates green but completion phrase not on final line. Continuing — Claude says it's not done."
      prev_failure=""
    fi
  else
