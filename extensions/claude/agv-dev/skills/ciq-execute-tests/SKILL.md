...
```bash
cat .agv/env-registry.json 2>/dev/null
```
...
|-------------|---------|--------|--------|
| agv-prod | alloydb | ACTIVE (proxy running, port reachable) | .agv/envs/agv-prod/config.yaml |
...
2. **Spawn subagent** using Agent tool with `subagent_type: "agv-dev:functional-tester"`:
   ```
   You are a functional tester. Follow the agv-functional-tester skill protocol.
   ...
   Environment notes:
   - Use agv_CONFIG={env_config_path} when starting the MCP server
