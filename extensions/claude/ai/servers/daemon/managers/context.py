...
logger = logging.getLogger("ai.daemon.context")
...
class ContextManager:
    ...
    async def initialize(self) -> None:
        """Initialize ai search and context services."""
        ...
        try:
            ...
            logger.info("Context manager initialized with ai search")
        except Exception:
            logger.warning(
                "ai search not available - context assembly will use memories only"
            )
                ...
                result.append(
                    {
                        ...
                        "tag": f"[ai Memory: {m.get('created', 'unknown')}{stale_tag}]",
