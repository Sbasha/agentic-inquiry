---
name: cleanup
description: Repository cleanup recommendations - stale files, dead code, agent artifacts, and hygiene. Use when maintaining repo health or before releases.
context: fork
agent: Explore
...
for f in agentic_inquiry/**/*.py; do
  module=$(echo $f | sed 's/\.py$//' | tr '/' '.')
  grep -rq "$module" agentic_inquiry/ tests/ || echo "POSSIBLY UNUSED: $f"
done
...
grep -rh "^[a-z_]*:" config/default.yaml | while read key; do
  grep -rq "${key%%:*}" agentic_inquiry/ || echo "UNUSED: $key"
done
```
...
du -sh .agentic-inquiry/ .serena/ .hypothesis/ 2>/dev/null

# Find old logs
find .agentic-inquiry/logs -name "*.log" -mtime +30 2>/dev/null
```
