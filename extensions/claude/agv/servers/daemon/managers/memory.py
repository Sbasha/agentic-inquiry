...
logger = logging.getLogger("agv.daemon.memory")
...
class MemoryManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "agv memory system not available - using basic storage"
            )
            self._initialized = True
    ...
    async def store(
        ...
        if self._memory_system and self._memory_context:
            try:
                ...
                logger.warning("agv memory store failed, using basic storage")
    ...
    async def recall(
        ...
        if self._memory_system and self._memory_context:
            try:
                ...
                logger.debug("agv memory recall failed", exc_info=True)
    ...
    async def pre_compact_snapshot(self, session_id: str) -> dict:
        ...
        if self._session_changes:
            ...
            prompt_parts.append(
                f"[agv] Auto-saved {len(files)} file changes."
            )
