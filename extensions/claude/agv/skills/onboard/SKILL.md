---
name: onboard
description: Intelligently onboard a codebase - parallel index, explore, and validate. Use when setting up agv for a new codebase or helping someone get started.
...
- Existing agv config: !`cat .agv/env-registry.json 2>/dev/null || echo "No agv environment configured"`
...
Agent tool:
  ...
  prompt: Index the codebase using: agv index {path}
```
...
4. Detect gaps where agv indexing may fall short
5. **Save memories throughout exploration** (not just at the end):

```bash
agv memory save "Architecture: {finding}"
agv memory save "Pattern: {pattern}"
agv memory save "Key entity: {entity} - {purpose}"
```
...
```bash
agv onboard complete --run-id {ONBOARD_RUN_ID}
```
