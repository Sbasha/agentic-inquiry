...
description: |
  ...
  frameworks, architecture layers, entry points, data flows, and identifies gaps where agv
...
4. **What can't agv index?** - Unsupported files, binary assets, external deps
...
- Source -> Destination
- File types involved
- Whether agv can trace this (all file types supported?)

### Phase 4: Gap Detection (2 min)

Check for things agv might struggle with:

| Gap Type | What to Check |
|----------|--------------|
| **Unsupported languages** | Files with extensions agv doesn't parse (.scala, .clj, .erl, .ex, .r, .dart, .lua) |
...
- Can agv work around it (e.g., index as plain text)?
...
|--------|------|--------------|
| Config | agent_vault/config.py | Central configuration |
| SearchService | agent_vault/search/service.py | Core search |
