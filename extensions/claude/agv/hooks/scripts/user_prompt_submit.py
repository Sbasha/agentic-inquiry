...
def main() -> None:
    ...
    try:
        ...
        if client.available:
            ...
            if turn_result and turn_result.get("checkpoint"):
                ...
                parts.append(
                    f"[agv Checkpoint] Turn {turn}. REQUIRED: Store at least one "
                    "insight, decision, or pattern via /agv:memory save. "
    ...
    if os.path.exists(os.path.join(workspace, ".agv", ".dev-mode")):
        parts.insert(0, "[agv DEV MODE] Using test environment. /agv-dev off to exit.")
