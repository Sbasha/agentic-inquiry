---
name: command-helper
description: List and describe available ai commands dynamically
tools:
  - read_file
  - glob
  - grep_search
...
# ai Command Helper Agent

Dynamically discover and describe available Agentic Inquiry commands by scanning the plugin cache or local extensions.

## Purpose

Instead of maintaining a hardcoded list of commands, this agent discovers commands at runtime by:
1. Scanning the plugin cache at `~/.claude/plugins/cache/agentic-inquiry/ai/*/commands/`
2. Falling back to local extensions at `extensions/gemini/ai/commands/`
3. Parsing frontmatter from each `.md` file to extract metadata

## Instructions

When asked about available ai commands:
...
```bash
uv run --env-file .env python -c "
from agentic_inquiry.cli.discover import get_command_info
...
| Command | Description |
|---------|-------------|
| `/ai:search` | Semantic search across code and documentation |
| `/ai:index` | Index a codebase for semantic search |
...
1. **Plugin cache**: `~/.claude/plugins/cache/agentic-inquiry/ai/<version>/commands/`
2. **Local extensions**: `extensions/gemini/ai/commands/`
...
- Use with `/ai:help` to provide up-to-date command listings
- Useful for onboarding new users to ai features
