        ...
        if client.available:
            ...
            if result:
                ...
                if importance == "config":
                    prompt = (
                        f"[agv Memory] Configuration file modified: {os.path.basename(file_path)}. "
                        "If this changes system behavior, store the rationale via /agv:memory save."
                    )
                elif importance == "entry_point":
                    prompt = (
                        f"[agv Memory] Entry point modified: {os.path.basename(file_path)}. "
                        "Store what changed and why via /agv:memory save."
                    )
    except ImportError:
