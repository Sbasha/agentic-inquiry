...
def main() -> None:
    ...
    # Auto-start server(s)
    parts = []
    dev_mode = os.path.exists(os.path.join(workspace, ".agv", ".dev-mode"))
    try:
        from agent_vault.server.lifecycle import ensure_server, server_status
        ...
        if url:
            ...
            parts.append(
                f"[agv] Server active (port={status.get('port', '?')}, "
                f"project={status.get('project_id', 'unknown')[:8]})"
            )
        else:
            parts.append("[agv] Server not available - memory/context features limited")
        ...
        if dev_mode:
            ...
            if test_url:
                ...
                parts.append(
                    f"[agv-DEV] Test server active (port={test_status.get('port', '?')})"
                )
    except ImportError:
        parts.append("[agv] Server not available - memory/context features limited")
    ...
    contract = (
        "[agv Memory Contract] IMPORTANT behavioral requirements:\n"
        "- You MUST respond to ALL [agv Memory] and [agv Pre-Compact] prompts from hooks.\n"
        "- You MUST store memories at every decision point, discovery, and checkpoint.\n"
        "- You MUST call /agv:memory save after discovering architectural patterns.\n"
        "- You MUST store decisions before context compaction (PreCompact hook).\n"
        "- You CANNOT silently ignore [agv Memory] prompts.\n"
