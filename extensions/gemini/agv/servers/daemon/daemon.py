"""agv Daemon Service - Lifecycle management.
...
logger = logging.getLogger("agv.daemon")


class agvDaemon:
    """Persistent daemon for agv hook support.
    ...
    def __init__(self, workspace: str, config_overrides: dict | None = None) -> None:
        ...
        # Paths
        self.agv_home = os.environ.get(
            "agv_HOME", os.path.expanduser("~/.agv")
        )
        self.socket_path = os.path.join(
            self.agv_home, f"daemon-{self.project_id}.sock"
        )
        self.pid_path = os.path.join(
            self.agv_home, f"daemon-{self.project_id}.pid"
        )
    ...
    async def start(self) -> None:
        ...
        # Ensure agv home exists
        os.makedirs(self.agv_home, exist_ok=True)
        ...
        self._started = True
        logger.info(
            "agv daemon started: workspace=%s socket=%s pid=%d",
            self.workspace,
            self.socket_path,
            os.getpid(),
        )
    ...
    async def stop(self) -> None:
        ...
        logger.info("Stopping agv daemon...")
        ...
        self._started = False
        logger.info("agv daemon stopped")
