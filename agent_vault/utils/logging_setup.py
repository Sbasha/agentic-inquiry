"""Logging configuration and setup for the agent-vault library."""
...
class LoggingConfigurator:
    ...
    def setup(config: Optional[LoggingConfig] = None) -> None:
        ...
        try:
            ...
            print(
                f"Error: Failed to configure logging: {e}",
                file=sys.stderr
            )
            print("Continuing without file logging", file=sys.stderr)
    ...
    def _apply_log_levels(config: LoggingConfig) -> None:
        ...
        for service_name, level_str in config.service_levels.items():
            # Construct full logger name (e.g., "agent_vault.parsers")
            logger_name = f"agent_vault.{service_name}"
...
class LogCleanupManager:
    ...
    def cleanup(log_dir: Path, retention_hours: int) -> int:
        ...
        try:
            ...
            print(
                f"Warning: Error during log cleanup: {e}",
                file=sys.stderr
            )
