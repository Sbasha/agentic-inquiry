...
gets its own PID file, lock file, and default port:
  ...
  - "test"    → server-test.pid, port 8766 (agv-dev testing)
"""
...
logger = logging.getLogger("agv.server.lifecycle")
...
def get_agv_home() -> str:
    """Get the agv home directory."""
    return os.environ.get("agv_HOME", os.path.expanduser("~/.agv"))
...
def start_server(
    ...
    """Start the agv server as a background process.
    ...
    try:
        ...
        cmd = [
            sys.executable, "-m", "agent_vault.server.run",
        ...
        if wait:
            ...
            while time.monotonic() < deadline:
                if check_health(actual_port, timeout=1.0):
                    logger.info(
                        "agv server started: env=%s pid=%d port=%d project=%s",
...
def stop_server(env: str = ENV_DEFAULT) -> bool:
    ...
    try:
        ...
        while time.monotonic() < deadline:
            if not is_process_alive(pid):
                remove_pid_file(env)
                logger.info("agv server stopped (env=%s pid=%d)", env, pid)
                return True
            time.sleep(0.2)
        # Force kill
        os.kill(pid, signal.SIGKILL)
        remove_pid_file(env)
        logger.warning("agv server force-killed (env=%s pid=%d)", env, pid)
        return True
...
def all_servers_status() -> list[dict]:
    ...
    """
    agv_home = get_agv_home()
    results = []
    if not os.path.isdir(agv_home):
        return results

    for name in os.listdir(agv_home):
