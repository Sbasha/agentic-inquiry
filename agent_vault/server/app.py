"""agv Server Application Factory.
...
from agent_vault.server.cache import ServerCacheManager
from agent_vault.server.session import SessionState
from agent_vault.server.signals import SignalAnalyzer
from agent_vault.server.versioning import GitVersionManager

logger = logging.getLogger("agv.server")

# Server defaults
DEFAULT_HOST = "127.0.0.1"
DEFAULT_PORT = 8765
SERVER_TITLE = "Agent-Vault Server"
...
async def create_app(
    ...
    Args:
        config: agv Config object (loaded from config.yaml if None)
    ...
    from agent_vault.server.middleware.auth import APIKeyAuthMiddleware
    ...
    from agent_vault.server.routes import health, search, memory, context, session, hooks, index
    ...
    from agent_vault.server.routes.search import _LOCAL_DIFF_EXAMPLE
    ...
    logger.info(
        "agv server created: project=%s, REST at /api/v1, MCP at /mcp, UI at /ui",
        project_id,
    )
...
def _load_daemon_config() -> dict:
    ...
    config_paths = [
        os.path.join(os.getcwd(), "extensions/claude/agv/config/daemon-defaults.yaml"),
        os.path.expanduser("~/.agv/daemon-defaults.yaml"),
    ]
