"""Git Version Manager - Git-linked memory versioning.
...
logger = logging.getLogger("agv.server.versioning")


class GitVersionManager:
    """Git-linked memory versioning and confidence decay."""

    def __init__(self, workspace: str, config: dict | None = None) -> None:
    ...
    def calculate_decay(
        self, memory_file_paths: list[str], commits: list[dict]
    ) -> float:
        """Calculate confidence decay based on commits touching referenced files."""
        total_decay = 0.0
    ...
    def check_staleness(
        self, memories: list[dict], since_commit: str | None = None
    ) -> list[dict]:
        """Check memories for staleness and update confidence."""
