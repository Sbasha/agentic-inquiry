...
class ModelMetadata:
    ...
    def load(cls, path: Path) -> ModelMetadata:
        ...
        required_fields = {
            ...
            "tokenizer_type", "normalize_embeddings", "pooling_mode", "created_at"
        }
...
class ModelLoader:
        ...
        try:
            ...
            if config_path.exists():
                try:
                    ...
                    logger.warning(
                        "Failed to load tokenizer config from %s: %s",
                        config_path,
                        e
                    )
    ...
    def load_model(
        self,
        model_path: Path
    ) -> tuple[Any, Tokenizer, ModelMetadata]:
        ...
        if metadata.format == "onnx":
            ...
            raise ValueError(
                "PyTorch format not yet supported. "
                "Please convert model to ONNX format."
            )
        else:
            raise ValueError(
                f"Unsupported model format: {metadata.format}. "
                f"Supported formats: onnx"
            )
