"""Custom exceptions for Agent-Vault.

This module defines the exception hierarchy for the Agent-Vault system.
All custom exceptions inherit from AgentVaultError, which provides a
common base for error handling and allows callers to catch all
Agent-Vault-specific errors with a single except clause.
...
Usage:
    from agent_vault.exceptions import ConfigurationError, ParsingError
...
class AgentVaultError(Exception):
    """Base exception for all Agent-Vault errors.
    
    All custom exceptions in the Agent-Vault system inherit from this class.
    This allows callers to catch all Agent-Vault-specific errors with a
    ...
    Example:
        try:
            ...
            # Catches all Agent-Vault errors
            logger.error("Processing failed: %s", e)
    """
    pass
...
class ParsingError(AgentVaultError):
...
class ResolutionError(AgentVaultError):
...
class ValidationError(AgentVaultError):
