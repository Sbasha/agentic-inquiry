...
Usage:
    agv index [PATH] [--project PROJECT] [--config CONFIG]
    agv index [PATH] --branch BRANCH_NAME
    agv index --prune [--project PROJECT]
    agv index --compact [--project PROJECT]
    agv index --status [--project PROJECT]
"""
...
async def prune_command(args: argparse.Namespace) -> int:
    ...
    from agent_vault.indexing.branch_discovery import (
        discover_branches,
        _detect_default_branch,
    )
    from agent_vault.indexing.branch_expiry import (
...
async def index_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        from agent_vault.onboard.gate import check_onboard_gate, OnboardGateError
    ...
    try:
        ...
        if getattr(args, "branch", None):
            worktree_dir = tempfile.mkdtemp(
                prefix=f"agv_worktree_{branch_name.replace('/', '_')}_"
            )
