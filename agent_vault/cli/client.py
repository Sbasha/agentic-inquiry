"""Interactive client for Agent-Vault MCP Server."""
...
class agvClient:
    """Client for interacting with a running agv MCP server."""
...
def run_shell(host: str, port: int):
    ...
    if not client.is_server_running():
        ...
        print("Run 'agv setup' to configure or 'agv serve' to start the server.")
        return

    print(f"✅ Connected to Agent-Vault Server at http://{host}:{port}")
    print("Interactive shell (type 'exit' to quit, 'help' for info)")
    
    while True:
        try:
            command = input("agv> ").strip()
            if not command:
                continue
...
def connect_or_setup():
    ...
    if client.is_server_running():
        ...
        print("Agent-Vault Server is not running.")
        print("To start the server: agv serve")
        print("To configure setup:  agv setup")
