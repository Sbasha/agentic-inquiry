"""Environment resolution for Agent-Vault.

This module provides environment detection following a clear discovery order:
1. agv_CONFIG env var (explicit override)
2. agv_ENV env var (named environment like 'agv', 'agv-test', 'agv-prod')
3. Active environment from global registry (~/.agv/)
4. Default fallback

Directory Convention:
- ~/.agv/              : Global - environments, registry, events, logs
- .agv/ (project-local): Testing - test data, local overrides

Naming Convention:
- 'agv' : Default production environment (auto-start proxy if CloudSQL configured)
- 'agv-test' : Test environment (never auto-start)
- 'agv-<custom>' : Custom named environments (auto-start based on config)
"""
...
# Directory names
GLOBAL_DIR_NAME = ".agv"  # ~/.agv/ for environments, registry, logs
LOCAL_DIR_NAME = ".agv"  # .agv/ (project-local) for test data
ENVS_DIR_NAME = "envs"
...
class ResolvedEnvironment:
    ...
    Attributes:
        name: Environment name (e.g., 'agv', 'agv-test', 'gcp-prod')
...
def get_global_dir() -> Path:
    """Get the global agv data directory (~/.agv/).

    This directory stores environments, registry, events, and logs.
    Can be overridden with agv_HOME env var.

    Returns:
        Path to ~/.agv/ directory
    """
    if agv_home := os.environ.get("agv_HOME"):
        return Path(agv_home)
    return Path.home() / GLOBAL_DIR_NAME


def get_local_dir(workspace: Optional[Path] = None) -> Path:
    """Get the project-local agv data directory (.agv/).
    ...
    Returns:
        Path to .agv/ directory within the workspace
    """
...
def get_data_dir(workspace: Optional[Path] = None) -> Path:
    """Get the agv data directory.

    When workspace is provided (e.g., in tests), returns workspace/.agv/
    for isolation. Otherwise returns the global ~/.agv/ directory.

    Args:
        workspace: If provided, returns workspace/.agv/ (for test isolation).
                   If None, returns ~/.agv/ (global).

    Returns:
        Path to the agv data directory
    """
...
def get_env_config_path(env_name: str, workspace: Optional[Path] = None) -> Path:
    ...
    Args:
        env_name: Environment name (e.g., 'agv', 'agv-test')
...
def is_test_environment(env_name: str) -> bool:
    ...
    if env_name.startswith("agv-test"):
        return True
    if env_name == "test":
        return True
...
def resolve_environment(workspace: Optional[Path] = None) -> ResolvedEnvironment:
    """Resolve active agv environment by discovery order.

    Discovery order:
    1. agv_CONFIG env var (explicit config path override)
    2. agv_ENV env var (named environment)
    3. Active environment from registry (if workspace is onboarded)
    4. Default 'agv' environment (if onboarded and agv config exists)
    ...
    # 1. Explicit override via agv_CONFIG env var
    if config_path_str := os.environ.get("agv_CONFIG"):
    ...
    # 2. Named environment via agv_ENV env var
    if env_name := os.environ.get("agv_ENV"):
        # Check workspace-local first, then global ~/.agv/
    ...
    # 3. Check registry (workspace-local .agv/ first, then global ~/.agv/)
    for ws in [workspace, None]:
        ...
        # 3b. Default to 'agv' environment if onboarded
        agv_config = get_env_config_path("agv", ws)
        if not agv_config.exists() and ws is not None:
            agv_config = get_env_config_path("agv", workspace=None)
        if agv_config.exists():
            return ResolvedEnvironment(
                name="agv",
                config_path=agv_config,
                source="onboarded",
                is_test=False,
                auto_start_proxy=should_auto_start_proxy(
                    "agv", has_cloudsql_config=True
                ),
            )

    # 3.5. Cloud context auto-detection (before default fallback)
    try:
        from agent_vault.cli.cloud_detect import detect_cloud_context
...
def load_environment_dotenv(
    ...
    from agent_vault.mcp.utils.validation import PathValidationError, validate_file_path
...
def warn_embedding_device_hatch(env_dir: Path) -> None:
    ...
    and when ``AGV_EMBEDDING_DEVICE`` is already set.
    """
    if platform.system() != "Darwin":
        return
    if os.environ.get("AGV_EMBEDDING_DEVICE"):
        return
    env_file = Path(env_dir) / ".env"
    print(
        f"If embeddings abort on Metal/MPS, uncomment AGV_EMBEDDING_DEVICE=cpu in {env_file}",
        file=sys.stderr,
    )
...
def load_config_for_environment(
    ...
    Example:
        >>> # Uses resolved environment (gcp-prod, agv, etc.)
...
def add_environment_to_registry(
    ...
    Args:
        name: Environment name (e.g., 'agv', 'agv-test')
    ...
    Example:
        ...
        ...     config_path="~/.agv/envs/gcp-prod/config.yaml",
