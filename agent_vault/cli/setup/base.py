"""Base setup class for Agent-Vault setup wizard.
...
class BaseSetup(ABC):
    ...
    def __init__(
        ...
        Args:
            ...
            is_dev: If True, creates a test environment (agv-test-*)
    ...
    def _determine_env_name(self) -> str:
        ...
        Returns:
            Environment name (e.g., "agv", "agv-test", "my-env")
        """
        if self.is_dev:
            # Default dev environment name
            default_name = "agv-test"
        else:
        ...
        if self.is_dev and not is_test_environment(name):
            name = f"agv-test-{name}" if name != "agv-test" else name
