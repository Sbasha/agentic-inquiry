...
Usage:
    agv agent-test [TEST_ID] [--all] [--project PROJECT] [--output-dir DIR]
    agv agent-test list
    agv agent-test status

Examples:
    agv agent-test 01              # Run TEST_01_CORE_FUNCTIONALITY
    agv agent-test --all           # Run all tests in sequence
    agv agent-test list            # List available tests
...
async def list_command(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        print("=" * 60)
        print("\nUsage:")
        print("  agv agent-test 01        # Run TEST_01")
        print("  agv agent-test --all     # Run all tests")
...
async def status_command(args: argparse.Namespace) -> int:
    ...
    Returns:
        Exit code (0 for success)
    """
    results_dir = Path(args.output_dir or "test_results/agv")
...
async def run_test_scenario(
    ...
    """Execute a single test scenario using agv CLI.
...
async def _run_smoke_test(storage, config, project_id: str, output_dir: Path, results: dict) -> dict:
    ...
    try:
        ...
        # Index the agent-vault package as test data
        agv_path = Path(__file__).parent.parent
        if agv_path.exists():
            result = await pipeline.index_directory(path=str(agv_path), wait=True)
    ...
    try:
        ...
        from agent_vault.memory.adapters.postgresql_adapter import PostgresMemoryAdapter
...
async def _run_generic_test(storage, config, project_id: str, parsed: dict, output_dir: Path, results: dict) -> dict:
    ...
    with open(test_log, "a") as f:
        ...
        f.write("followed manually by an AI agent using agv CLI commands.\n")
...
async def run_command(args: argparse.Namespace) -> int:
    ...
    output_dir = Path(args.output_dir or "test_results/agv") / timestamp
    ...
    for test_id in tests_to_run:
        ...
        project_id = f"agv_test{test_id}_{timestamp}"
