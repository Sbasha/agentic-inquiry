"""Dynamic command discovery for Agent-Vault.

This module provides utilities to discover available agv commands
by scanning the plugin cache instead of hardcoding command lists.

Plugin cache structure:
~/.claude/plugins/cache/agent_vault/agv/
├── <version>/
│   ├── commands/
│   │   ├── index.md
│   │   ├── search.md
...
PLUGIN_CACHE_PATHS = [
    Path.home() / ".claude" / "plugins" / "cache" / "agent-vault" / "agv",
    Path.home() / ".claude" / "plugins" / "installed" / "agent-vault" / "agv",
]

# Local extension paths (for development)
LOCAL_EXTENSION_PATHS = [
    Path("extensions/claude/agv"),
    Path("extensions/agv"),
]
...
class CommandInfo:
    ...
    Attributes:
        ...
        full_name: Full command with prefix (e.g., 'agv:search')
...
def discover_agv_commands() -> list[CommandInfo]:
    """Discover available agv commands from plugin cache or local extensions.
...
def get_command_info(command_name: str) -> Optional[CommandInfo]:
    ...
    Args:
        command_name: Command name (with or without agv: prefix)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Discover available agv commands"
    )
    parser.add_argument(
        "--format", "-f",
