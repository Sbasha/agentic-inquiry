...
class OnboardMetadataService:
    ...
    async def from_config(
        ...
        Args:
            config: agv configuration (None uses SQLite with workspace path).
    ...
    async def _create_pg_provider(
        ...
        from agent_vault.onboard.providers.postgresql import (
            PostgresOnboardMetadataProvider,
        )
        from agent_vault.storage.registry import resolve_backend
    ...
    async def _create_sqlite_provider(
        ...
        from agent_vault.onboard.providers.sqlite import (
        ...
        db_path = Path(root) / ".agv" / "onboard_metadata.db"
        # If root itself contains .agv, don't double it
        if ".agv" in str(root):
