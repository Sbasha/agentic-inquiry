...
class EmbeddingService:
    ...
    async def generate_embeddings_batch(
        ...
        try:
            ...
            logger.warning("Batch %s failed, falling back to individual generation: %s",
                exc_info=True,
            )
            ...
            for i, text in enumerate(texts):
                try:
                    ...
                    raise StorageError(
                        "Embedding generation failed for %s", operation_name, e, i, operation_name, inner_e, operation_name) from inner_e
