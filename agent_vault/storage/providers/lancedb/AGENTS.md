...
Default backend for the `vector` and `graph` roles. File-based, single-process, no external service. Shipped as the out-of-the-box `agv setup` choice — optimised for individual dev / small team self-hosting.
...
Adjacent: `agent_vault/database/lancedb_manager.py` (lower-level LanceDB handle wrapper) and `agent_vault/database/tables.py` (schema creation / TableManager).
...
|---|---|---|---|
| `storage.backends.<name>.database_path` | `./.agv/lancedb` (from `agv setup`) | yes | Local path; created on `initialize()` |
...
- **Pool / timeout keys** (`pool_size`, `max_overflow`, `backend_timeouts.transaction_timeout`, `backend_timeouts.migration_lock_timeout`) are accepted in the config schema but LanceDB ignores them. Their presence in `agent-vault.yaml.example` gives the false impression they're tunable here.
