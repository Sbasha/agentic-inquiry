...
if TYPE_CHECKING:
    ...
    from agent_vault.storage.pool import BackendPoolManager
...
class StorageFacade:
    ...
    Agent-Vault. Services should depend on StorageFacade rather than
    ...
    async def from_config(
        ...
        naming the active roles. ``agv setup`` emits this shape; the legacy
        ...
        if provider_name:
            deprecation_message = (
                ...
                "graph_backend. Run `agv setup` to update your config if needed."
            )
        ...
        if not storage_config.backends:
            ...
            if legacy_backend:
                migration_message = (
                    ...
                    "`agv setup` output) to silence this warning."
                )
                ...
                raise ConfigurationError(
                    "No storage backends configured. Run `agv setup` to "
        ...
        # this surface.
        # Tracked: https://github.com/sbasha/agent_vault/issues/131
        ...
        # services, persistence backends — see agent_vault/mcp/tools/search.py,
        # agent_vault/mcp/services/context_builder.py) keep the
