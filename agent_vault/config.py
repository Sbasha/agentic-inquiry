"""Configuration management for Agent-Vault.
...
- Automatic detection of agent-vault.yaml in project root
"""
...
from agent_vault.exceptions import ConfigurationError, StoragePathError
...
class StorageConfig:
    ...
    # https://github.com/sbasha/agent_vault/issues/132
    backend: str = ""
...
# agent_vault.connectors.base - the canonical source for file discovery constants.
...
class ServicesConfig:
    ...
    - 'agv' (default): Auto-start if CloudSQL configured
    - 'agv-test': Never auto-start (test isolation)
...
@dataclass
class Config:
    """Main configuration class for Agent-Vault.

    Configuration is loaded with the following priority (highest to lowest):
    1. Environment variables (AGV_*)
    2. Project root configuration (agent-vault.yaml)
    ...
    def load(
        ...
        1. Environment variables (AGV_*)
        2. Specified config_path or auto-detected agent-vault.yaml
        3. Default configuration (config/default.yaml)

        Args:
            config_path: Optional path to configuration file. If None, automatically
                searches for agent-vault.yaml in project root, then falls back to
                config/default.yaml.
    ...
    def _packaged_config_file(filename: str) -> Optional[Path]:
        ...
        - **Installed wheel** - the file was mapped into the package namespace
          at build time (agent_vault/config_defaults/<filename>) and is found
    ...
    def _find_config_file(cls, config_path: Optional[str] = None) -> Path:
        ...
        2. agv_CONFIG environment variable
        3. agent-vault.yaml in project root
        ...
        # Check agv_CONFIG environment variable
        if agv_config := os.environ.get("agv_CONFIG"):
            path = Path(agv_config)
            if path.exists():
                logger.debug("Using configuration from agv_CONFIG: %s", path)
                return path
            else:
                logger.warning("agv_CONFIG points to non-existent file: %s", agv_config)

        # Try project root agent-vault.yaml
        project_config = Path.cwd() / "agent-vault.yaml"
        if project_config.exists():
            logger.debug("Found project configuration: %s", project_config)
            return project_config
        
        # Try global configuration (~/.agv/config.yaml)
        global_config = Path.home() / ".agv" / "config.yaml"
        ...
        # ships inside the package (agent_vault/config_defaults/) and is found
    ...
    def _validate_config(cls, config_data: Dict[str, Any]) -> None:
        ...
        root_path = storage_config.get('root', './.agv')
        if '\x00' in root_path:
            raise ConfigurationError(
        ...
        try:
            ...
            try:
                from agent_vault.storage.schema_registry import get_merged_storage_schema
    ...
    def _apply_env_overrides(cls, config_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Environment variables use the format: AGV_SECTION_SUBSECTION_KEY
        The convention automatically maps AGV_* variables to configuration paths:
        - AGV_STORAGE_ROOT → config_data['storage']['root']
        - AGV_CACHE_DOCUMENT_CACHE_MAX_SIZE → config_data['cache']['document_cache']['max_size']
        - AGV_SEARCH_DEFAULT_LIMIT → config_data['search']['default_limit']
        - AGV_STORAGE_DEFAULT_PROJECT_ID → config_data['storage']['default_project_id']
        - AGV_STORAGE_FILE_TRACKER_PATH → config_data['storage']['file_tracker']['path']

        Examples:
            AGV_STORAGE_ROOT=./.agv
            AGV_STORAGE_DEFAULT_PROJECT_ID=my_project
            AGV_STORAGE_LANCEDB_PATH=lancedb
            AGV_CACHE_DOCUMENT_CACHE_MAX_SIZE=2000
            AGV_SEARCH_DEFAULT_LIMIT=20
            AGV_EMBEDDINGS_DEFAULT_DIMENSIONS=384

        Special cases:
            - AGV_LOGGING_SERVICE_LEVELS_<SERVICE>=<LEVEL> for per-service log levels
            - AGV_MAINTENANCE_RETENTION_MINUTES → maintenance.cleanup_retention_minutes
            - AGV_TRAVERSAL_LIMIT → mcp.query.traversal_limit
            - AGV_BATCH_SIZE → mcp.query.batch_size
        ...
        # Handle service_levels separately (AGV_LOGGING_SERVICE_LEVELS_<SERVICE>=<LEVEL>)
        service_levels_prefix = "AGV_LOGGING_SERVICE_LEVELS_"
        ...
        for env_key, env_value in os.environ.items():
            ...
            # Convention-based lookup: AGV_SECTION_SUBSECTION_KEY → ["section", "subsection", "key"]
            ...
            if len(key_parts) < 2:
                logger.warning(
                    ...
                    "must have at least section and key (e.g., AGV_STORAGE_ROOT)",
                    env_key
                )
                ignored_count += 1
                continue
