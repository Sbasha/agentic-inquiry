...
class SyntheticDataGenerator:
    ...
    def collect_texts(
        self, extensions: List[str] = [".md", ".py", ".txt"]
    ) -> List[str]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune an embedding model on local data."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        required=True,
        help="Directory containing text data for training",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--mrl", action="store_true", help="Enable Matryoshka Representation Learning"
    )
    parser.add_argument(
        "--quantize", action="store_true", help="Quantize the resulting ONNX model"
    )
    ...
    logger.info(
        "To use this model, configure 'local_model' or 'fastembed' to point to this directory."
    )
    return 0
