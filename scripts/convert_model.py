...
def detect_workspace_root() -> Path:
    ...
    for path in [current] + list(current.parents):
        if (path / "agentic-inquiry.yaml").exists() or (
    ...
    logger.warning(
        "Could not detect workspace root. Using current directory: %s", current
    )
    return current
...
def convert_to_onnx(model_dir: Path, model_id: str, *, quantize: bool = False) -> Path:
    ...
    try:
        ...
        if quantize:
            ...
            try:
                ...
                quantizer.quantize(
                    save_dir=model_dir, quantization_config=quantization_config
                )
                logger.info("  Quantization applied successfully")
            except Exception as e:
                logger.warning(
                    "  Quantization failed, continuing with unquantized model: %s", e
                )
        else:
        ...
        attention_mask_expanded = np.expand_dims(attention_mask, axis=-1).astype(
            np.float32
        )
...
def download_model(
    model_id: str, output_dir: Path, *, force: bool = False, token: str | None = None
) -> bool:
    ...
    if output_dir.exists() and not force:
        ...
        if has_all_files:
            logger.info(
                "Model already exists at %s (use --force to re-download)", output_dir
            )
            return False
        else:
            logger.warning(
...
def main() -> int:
    ...
    try:
        ...
        downloaded = download_model(
            args.model_id, output_dir, force=args.force, token=args.token
        )
        ...
        onnx_path = convert_to_onnx(output_dir, args.model_id, quantize=args.quantize)
