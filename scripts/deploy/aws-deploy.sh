...
#   INQUIRY_RDS_PASSWORD       RDS postgres user password (required)
...
#   INQUIRY_RDS_INSTANCE_CLASS RDS instance class (default: db.t3.micro)
...
#   INQUIRY_DB_USER            Database user (default: postgres)
...
INQUIRY_RDS_INSTANCE_CLASS="${INQUIRY_RDS_INSTANCE_CLASS:-db.t3.micro}"
...
[[ -n "${INQUIRY_RDS_PASSWORD:-}" ]]   || die "INQUIRY_RDS_PASSWORD is required"
...
if step_done "01-prereqs"; then
    ...
    aws sts get-caller-identity --region "${INQUIRY_AWS_REGION}" --output text >/dev/null 2>&1 \
...
if step_done "02-rds"; then
    ...
    log_info "Step 02: Creating RDS PostgreSQL instance '${INQUIRY_RDS_INSTANCE_ID}'..."

    if aws rds describe-db-instances \
        --db-instance-identifier "${INQUIRY_RDS_INSTANCE_ID}" \
        --region "${INQUIRY_AWS_REGION}" \
        --output text >/dev/null 2>&1; then
        log_warn "RDS instance '${INQUIRY_RDS_INSTANCE_ID}' already exists, skipping"
    else
        aws rds create-db-instance \
            --db-instance-identifier "${INQUIRY_RDS_INSTANCE_ID}" \
            --db-instance-class "${INQUIRY_RDS_INSTANCE_CLASS}" \
            --engine postgres \
            --engine-version "15" \
            --master-username "${INQUIRY_DB_USER}" \
            --master-user-password "${INQUIRY_RDS_PASSWORD}" \
            --db-name "${INQUIRY_DB_NAME}" \
    ...
    RDS_ENDPOINT=$(aws rds describe-db-instances \
        ...
        --output text)
    echo "${RDS_ENDPOINT}" > "${INQUIRY_DEPLOY_STATE_DIR}/rds-endpoint.txt"
...
if step_done "03-iam"; then
    ...
    log_info "Step 03: Configuring IAM role '${INQUIRY_IAM_ROLE_NAME}'..."
    ...
    if aws iam get-role --role-name "${INQUIRY_IAM_ROLE_NAME}" --output text >/dev/null 2>&1; then
        log_warn "IAM role '${INQUIRY_IAM_ROLE_NAME}' already exists, skipping creation"
    else
        aws iam create-role \
...
if step_done "04-container"; then
    ...
    if aws ecr describe-repositories \
        --repository-names "${INQUIRY_ECR_REPO}" \
        --region "${INQUIRY_AWS_REGION}" \
        --output text >/dev/null 2>&1; then
        log_warn "ECR repo '${INQUIRY_ECR_REPO}' already exists"
    else
        aws ecr create-repository \
            --repository-name "${INQUIRY_ECR_REPO}" \
            --region "${INQUIRY_AWS_REGION}" \
            --no-cli-pager >/dev/null
    fi

    ECR_URI="${INQUIRY_AWS_ACCOUNT_ID}.dkr.ecr.${INQUIRY_AWS_REGION}.amazonaws.com/${INQUIRY_ECR_REPO}:${INQUIRY_IMAGE_TAG}"

    # Docker login to ECR
    aws ecr get-login-password --region "${INQUIRY_AWS_REGION}" \
        | docker login --username AWS --password-stdin \
          "${INQUIRY_AWS_ACCOUNT_ID}.dkr.ecr.${INQUIRY_AWS_REGION}.amazonaws.com"
    ...
    docker push "${ECR_URI}"
    echo "${ECR_URI}" > "${INQUIRY_DEPLOY_STATE_DIR}/image-path.txt"
...
if step_done "05-app-runner"; then
    ...
    ECR_URI=$(cat "${INQUIRY_DEPLOY_STATE_DIR}/image-path.txt")
    RDS_ENDPOINT=$(cat "${INQUIRY_DEPLOY_STATE_DIR}/rds-endpoint.txt" 2>/dev/null || echo "")

    ROLE_ARN="arn:aws:iam::${INQUIRY_AWS_ACCOUNT_ID}:role/${INQUIRY_IAM_ROLE_NAME}"

    SERVICE_ARN=$(aws apprunner create-service \
        --service-name "${INQUIRY_SERVICE_NAME}" \
        ...
        aws apprunner list-services \
            --region "${INQUIRY_AWS_REGION}" \
            --query "ServiceSummaryList[?ServiceName=='${INQUIRY_SERVICE_NAME}'].ServiceArn" \
            --output text \
            --no-cli-pager)

    echo "${SERVICE_ARN}" > "${INQUIRY_DEPLOY_STATE_DIR}/service-arn.txt"
    ...
    SERVICE_URL=$(aws apprunner describe-service \
        ...
        --output text \
        --no-cli-pager)
    echo "https://${SERVICE_URL}" > "${INQUIRY_DEPLOY_STATE_DIR}/service-url.txt"
...
log_ok "  Account:  ${INQUIRY_AWS_ACCOUNT_ID}"
log_ok "  Region:   ${INQUIRY_AWS_REGION}"
