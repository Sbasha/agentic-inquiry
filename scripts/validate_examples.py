...
DEPRECATED_PATTERNS = [
    (
        r"from agentic_inquiry\.parsers\.base import",
        "Use 'from agentic_inquiry.parsers.models import' instead",
    ),
    (r"\.code_symbols", "Use '.symbols' field instead of '.code_symbols'"),
    (r"DatabaseManager\(", "Use 'LanceDBManager' instead of 'DatabaseManager'"),
    (
        r"from agentic_inquiry\.database\.manager import",
        "Use 'from agentic_inquiry.database import LanceDBManager' instead",
    ),
...
def validate_example(file_path: Path, verbose: bool = False) -> ValidationResult:
    ...
    for line_num, pattern, suggestion in deprecated_issues:
        if line_num > 0:
            result.add_warning(
                f"Line {line_num}: Deprecated pattern '{pattern}' - {suggestion}"
            )
        else:
    ...
    if not check_shebang(file_path):
        result.add_warning(
            "Missing or incorrect shebang (should be #!/usr/bin/env python3)"
        )
    elif verbose:
...
def main():
    ...
    parser.add_argument(
        "--files", nargs="+", help="Specific files to validate (relative to examples/)"
    )
